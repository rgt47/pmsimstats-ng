...
\renewcommand{\stamptime}{2026-08-18 19:31 PDT}
\renewcommand{\stampversion}{\detokenize{b5fd4d1-wip}}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{booktabs}
...
conditional mixed model even necessary. Senn, Julious and Araujo\textsuperscript{\citeproc{ref-senn2014four}{5}} compared four analysis strategies for ordinary (no
...
the companion calibration study\textsuperscript{\citeproc{ref-pmsimstats_calibration2026}{6}} shows
...
\begin{table}
\centering
\caption{\label{tab:tab-s7}Block S7: alternative-paradigm specifications at the
...
retired \texttt{E5}/\texttt{E6} comparison: both new specifications \emph{beat}
Unadjusted at this cell. \texttt{E7} leads by 8.4 points (\(0.832\) versus
\(0.748\)), and \texttt{E9}, the two-stage paired-difference regression with
no repeated-measures model at all, leads by 5.6 points (\(0.804\)).
\texttt{E7}'s AIC selection is imperfect but usefully informative: the mean
...
its immediate neighbor (88\% of replicates choose \(1.0\) or \(2.0\) weeks), so most of the
oracle Exposure-weighted advantage at this cell (Section 3.2,
\(0.860\) at the correctly assumed half-life) survives estimation
rather than assumption. That \texttt{E9}, which discards the repeated-measures
...
echoing Senn, Julious and Araujo's\textsuperscript{\citeproc{ref-senn2014four}{5}} finding that
...
Blocks S7 and S8 found \texttt{E7} and \texttt{E9} both beat Unadjusted at the
Hybrid reference cell under Architecture B, with \texttt{E7} slightly
exceeding Exposure-weighted itself. Block S9 asks whether that
ranking is specific to Hybrid's short, compressed washout, by
refitting \texttt{E1}, \texttt{E7}, and \texttt{E9} under the CO design (off-drug arm
spanning 2.5 to 20 weeks post-discontinuation, Section 2.2) at two
true half-lives.

\begin{figure}
\includegraphics[width=0.75\linewidth]{../../figures/02xs-heatmap-sens-S9} \caption{Block S9: power under the CO design at two true half-lives, same three specifications as block S7.}\label{fig:fig-heatmap-sens-s9}
\end{figure}

It is not, and \texttt{E7} fails outright. At \(t_{1/2} = 1.0\), \texttt{E1} reaches
\(0.834\) while \texttt{E7} collapses to \(0.474\), a 36-point reversal from its
8.4-point \emph{lead} over \texttt{E1} at the Hybrid reference cell; \texttt{E9} also
reverses, to \(0.750\), a smaller 8.4-point deficit. At \(t_{1/2} = 0.5\)
the pattern repeats (\(0.866\) for \texttt{E1} against \(0.552\) for \texttt{E7} and
\(0.778\) for \texttt{E9}). \texttt{E7}'s collapse traces to a specific failure of
...
value, systematically \emph{over}-assuming the half-life. Section 3.6's S2
block already established that over-assumption is the more damaging
direction of half-life error (Section 4.1); \texttt{E7} under CO
demonstrates that failure mode operating on the estimation procedure
itself, not just on a fixed wrong assumption. \texttt{E9}'s milder reversal
...
there than it does under Hybrid. Neither \texttt{E7} nor \texttt{E9}'s Hybrid
advantage is design-general, then, and \texttt{E7} specifically should not
...
\item
  ...
  Julious and Araujo\textsuperscript{\citeproc{ref-senn2014four}{5}}. Both \emph{beat} Exposure-weighted
...
(\texttt{15-run-sensitivity-s10.R}, \texttt{output/15-sensitivity-s10-g.rds}), all
added 2026-08-18 (master seed 20260415, 500 replicates per cell,
Hybrid reference cell except S9, which uses CO). \texttt{E5}/\texttt{E6}
...
and the S7/S8/S9/S10 output \texttt{.rds} files (plus \texttt{diag-s6-cr2.rds} for
\texttt{G8}), and are additional to, not replacements for, the existing
