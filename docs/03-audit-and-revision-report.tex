\documentclass[11pt]{article}
\newcommand{\doctitle}{pmsimstats: Status Report on N-of-1 Trial Simulation Framework}
\newcommand{\shorttitle}{Audit and Revision Report}
\newcommand{\docdate}{2026-03-24}
\input{pmsimstats-preamble}

\begin{document}
\maketitle
\thispagestyle{fancy}
\tableofcontents

\textbf{Ronald G. Thomas, Ph.D.}\\
Department of Family Medicine and Public Health, UC San Diego

\textbf{Collaborator:} Rebecca C. Hendrickson, M.D., Ph.D.\\
VA Puget Sound Health Care System

\bigskip\noindent\rule{\textwidth}{0.4pt}\bigskip

\section{Background and Objective}

The pmsimstats framework provides Monte Carlo simulation-based
power analysis for aggregated N-of-1 clinical trial designs. It
compares four trial designs -- Open-Label (OL), Crossover (CO),
Hybrid N-of-1, and Open-Label with Blinded Drug Cessation
(OL+BDC) -- on statistical power to detect a biomarker-treatment
interaction. The motivating clinical application is the use of
resting blood pressure as a predictive biomarker for prazosin
response in PTSD, based on the Raskind et al. (2013) RCT analyzed
in Murray et al. (in preparation).

The original simulation code was published in Hendrickson et al.
(2020), \textit{Frontiers in Digital Health} 2:13. The current work
represents a comprehensive audit, revision, and consolidation of
the codebase into a single reproducible research repository
(\texttt{pmsimstats-ng}) housed as a zzcollab Docker workspace.

\bigskip\noindent\rule{\textwidth}{0.4pt}\bigskip

\section{Audit and DGP Revisions}

A technical audit of the original publication code identified
seven substantive corrections to the data-generating process
(DGP), each justified by either statistical theory or clinical
plausibility.

\subsection{AR(1) autocorrelation (was compound symmetry)}

The original code used compound symmetry (constant correlation
$\rho$ between all timepoint pairs). The revised code implements
AR(1) decay: $\text{Cor}(X_{t_i}, X_{t_j}) = \rho^{|t_i - t_j|}$.
This change is both more clinically realistic (nearby measurements
are more correlated than distant ones) and numerically beneficial:
it expands the positive-definite-feasible region of the biomarker
correlation parameter $c_{bm}$ from a maximum of 0.25 to 0.49+.

\subsection{Exponential BM-BR correlation decay}

During off-drug periods, the correlation between the biomarker
and the biological response should decay as the drug effect wanes.
The original code used a step function (correlation drops to zero
instantly at discontinuation). The revised code implements
exponential decay tied to the pharmacokinetic half-life:
$\text{Cor}(bm, br_t) = c_{bm} \cdot e^{-\lambda_{cor} \cdot
t_{sd}}$, where $\lambda_{cor} = \ln(2) / t_{1/2}$. This
derivation is presented in the correlation decay derivation white
paper.

\subsection{nlme::lme with corCAR1 analysis model}

The original analysis model used \texttt{lme4::lmer} with a random
intercept only. Under the AR(1) DGP, this misspecified model
absorbs residual autocorrelation into the error term, inflating
Type I error to 13--17\% for CO and OL designs. The revised code
uses \texttt{nlme::lme} with \texttt{corCAR1(form = \~{}t|ptID)}, which models
the continuous-time AR(1) residual structure directly. This
corrects Type I error to the nominal 5\% across all designs. The
analysis model white paper provides the full derivation and
validation.

\subsection{Carryover scale factor removal}

The original code applied a scale factor of 2 to the carryover
half-life parameter, effectively halving the half-life without
documentation. The parameter \texttt{carryover\_t1half} should mean what
it says: the time for the carryover effect to decay to half its
initial value. The scale factor has been removed.

\subsection{Timepoint-1 correlation fix}

The original code guarded the BM-BR correlation assignment with
\texttt{if (p > 1)}, skipping the first timepoint. This was a coding
artifact to avoid an index error in the proportional scaling
formula. Participants are on drug for 2.5--4 weeks at timepoint 1
and should have a biomarker-response correlation. The guard has
been removed.

\subsection{Hendrickson-aligned Gompertz function}

The Gompertz growth function $f(t) = M \cdot e^{-d \cdot
e^{-r \cdot t}}$ does not pass through the origin ($f(0) \neq 0$).
The original publication code applied an offset-rescaling to
guarantee $f(0) = 0$ and preserve the asymptote at $M$. The 2025
tidyverse rewrite inadvertently used a different formula that does
not pass through the origin. This has been corrected: both
pipelines now use the Hendrickson-aligned formula.

\subsection{Updated default parameters}

Autocorrelation base rate reduced from 0.8 to 0.7 (consistent
with AR(1) decay). Biomarker correlation $c_{bm}$ constrained to
$\{0, 0.25, 0.45\}$ (maximum PD-feasible value under AR(1) is
0.49 for OL). Carryover half-life range updated from $\{0, 0.1,
0.2\}$ weeks (too short to produce genuine carryover effects) to
$\{0, 0.5, 1.0\}$ weeks.

\bigskip\noindent\rule{\textwidth}{0.4pt}\bigskip

\section{Resolving the Positive Definiteness Problem}

\subsection{The problem}

The DGP constructs a $26 \times 26$ covariance matrix $\Sigma$
from user-specified correlation parameters and passes it to an
MVN draw. When the resulting matrix is not positive definite (PD),
the code silently projects it to the nearest PD matrix via
\texttt{corpcor::make.positive.definite()}. Instrumentation of the
original publication code revealed that this is not a rare edge
case: 24.7\% of covariance matrices required silent correction at
the publication commit, rising to 63.0\% after subsequent edits.
All 162 matrices in the standard parameter sweep were
ill-conditioned ($\kappa > 100$).

\subsection{Root causes}

The failures are concentrated in the OL+BDC design (83--92\%
failure rate) due to an abrupt expectancy-driven change in the
ER (expectancy response) variance mid-trial. The OL+BDC design
transitions from full expectancy ($e = 1$) during the open-label
phase to half expectancy ($e = 0.5$) during the blinded drug
cessation phase. Because the ER standard deviation is scaled by
$e$, this creates a factor-of-two variance discontinuity within
the same correlation block, violating the implicit assumption
that the specified correlations are consistent with the marginal
variances. The compound symmetry autocorrelation structure
compounds the problem: it requires all same-factor timepoint
pairs to have the same correlation regardless of their temporal
separation, which becomes untenable when variances change
mid-trial.

The OL design never fails because all timepoints have the same
expectancy ($e = 1$), so there is no variance heterogeneity.

\subsection{Resolution}

The revised code resolves the PD problem through two mechanisms:

\begin{enumerate}
  \item \textbf{AR(1) autocorrelation} replaces compound symmetry. Because
    AR(1) correlations decay with temporal distance ($\rho^{|t_i -
    t_j|}$), the correlation matrix has smaller off-diagonal entries
    for distant timepoints, making it substantially easier to
    satisfy the PD constraint. The maximum feasible $c_{bm}$
    increases from 0.25 to 0.49+.

  \item \textbf{Pre-validation with constrained parameters.} The
    \texttt{validateParameterGrid()} function tests all parameter
    combinations for PD before simulation and reports failures
    upfront. The default parameter grid ($c_{bm} \leq 0.45$,
    $\rho = 0.7$) produces zero PD failures across all 162
    combinations for all four trial designs.
\end{enumerate}

The net result: the revised code runs the complete parameter
sweep with zero silent corrections, eliminating the uncontrolled
perturbation that confounded cross-design power comparisons in
the original publication.

\bigskip\noindent\rule{\textwidth}{0.4pt}\bigskip

\section{Resolving the Power Dropoff Problem}

\subsection{The problem}

When carryover is introduced into the DGP (non-zero $t_{1/2}$),
the statistical power to detect the biomarker-treatment
interaction should decline because the carryover effect blurs the
distinction between on-drug and off-drug periods. However, the
magnitude and pattern of the power decline depends on the entire
variable chain from trial design specification through the
analysis model interaction term.

Five derived variables form this chain:

\begin{enumerate}
  \item \texttt{ondrug} -- binary path assignment (design-determined)
  \item \texttt{tod} -- cumulative time on drug (Gompertz input)
  \item \texttt{tsd} -- time since discontinuation (carryover input)
  \item \texttt{Dbc} -- continuous drug indicator in the analysis model
  \item \texttt{bm:Dbc} or \texttt{bm:t} -- the interaction coefficient tested
\end{enumerate}

Carryover enters at step 3 and propagates through steps 4 and 5
to reduce the variance of the interaction term, inflating its
standard error and reducing power.

\subsection{The mechanism}

For designs with both on-drug and off-drug periods (CO, Hybrid,
OL+BDC), the analysis model uses \texttt{Dbc} as the treatment
indicator. On drug, $Dbc = 1$. Off drug, $Dbc = (1/2)^{s \cdot
tsd / t_{1/2}}$, where $s$ is the scale factor and $t_{1/2}$
is the carryover half-life.

Without carryover ($t_{1/2} = 0$), $Dbc$ is effectively binary
(1 vs 0), maximizing the contrast between treatment conditions.
With carryover, $Dbc$ during off-drug periods is strictly positive
(decaying from 1 toward 0), compressing the treatment contrast.
The interaction coefficient \texttt{bm:Dbc} depends on the variance of
$Dbc$, which decreases as carryover increases, inflating the
standard error.

For the open-label design (no off-drug periods), the analysis
model uses \texttt{bm:t} instead of \texttt{bm:Dbc}, so carryover has no
effect on power -- there are no off-drug periods for carryover
to operate on.

\subsection{Resolution}

The revised code addresses the power dropoff problem at two
levels:

\begin{enumerate}
  \item \textbf{Correct DGP-analysis alignment.} The AR(1) autocorrelation
    in the DGP is matched by \texttt{corCAR1} in the analysis model.
    This prevents residual autocorrelation from being absorbed
    into the error term, which would artificially inflate or
    deflate power estimates.

  \item \textbf{Clinically realistic carryover parameters.} The original
    half-life values (0.1--0.2 weeks) produce negligible carryover
    effects that decay to essentially zero within one inter-visit
    interval. The revised values (0.5--1.0 weeks) produce genuine
    carryover that persists across multiple off-drug timepoints,
    enabling meaningful power comparisons between designs that
    differ in their susceptibility to carryover.

  \item \textbf{Conditional analysis formula.} The analysis model adapts
    to the design: \texttt{bm:Dbc} when there is within-subject treatment
    variation (CO, Hybrid, OL+BDC), and \texttt{bm:t} when there is not
    (OL). This prevents the open-label power estimate from being
    confounded by a degenerate treatment indicator.
\end{enumerate}

The 1,000-replicate validation confirms that Type I error rates
are within 2.5--5.8\% across all designs and carryover conditions,
and power estimates decrease monotonically with increasing
carryover half-life as expected.

\bigskip\noindent\rule{\textwidth}{0.4pt}\bigskip

\section{Consolidated Repository}

The pmsimstats-ng repository consolidates the original
publication code (\texttt{orig}, data.table) and the 2025 tidyverse
rewrite (\texttt{2025}) into a single zzcollab Docker workspace with
shared data, documentation, and test infrastructure.

\subsection{Architecture}

The two simulation pipelines coexist independently:

\begin{itemize}
  \item \textbf{orig} (\texttt{R/}, 1973 lines, 10 files): data.table-based
    package functions loaded via \texttt{devtools::load\_all(".")}. Uses
    \texttt{buildSigma()}, \texttt{generateData()}, \texttt{lme\_analysis()},
    \texttt{generateSimulatedResults()}. This is the gold standard.

  \item \textbf{2025} (\texttt{analysis/2025/01-pm-functions.R}, 1023 lines):
    tidyverse-based monolithic file loaded via \texttt{source()}. Uses
    \texttt{build\_sigma\_matrix()}, \texttt{generate\_data()}, \texttt{lme\_analysis()},
    \texttt{generate\_simulated\_results()}. Validated bit-identical to
    orig for the same inputs.

  \item \textbf{simple} (\texttt{simple/simulation.R}, 577 lines): self-contained
    pedagogical simulation using direct mean moderation (not MVN
    correlation) and OLS on phase means (not LME). Serves a
    different purpose and is architecturally independent.
\end{itemize}

\subsection{Validation}

\begin{itemize}
  \item 67 unit tests covering \texttt{cumulative()}, \texttt{modgompertz()},
    \texttt{buildtrialdesign()} (OL, CO, Hybrid, tpb), \texttt{buildSigma()}
    (PD, dimensions, labels, AR(1), lambda\_cor, Cholesky),
    \texttt{generateData()} (output shape, caching, outcome identity),
    and \texttt{lme\_analysis()} (OL, CO, p-value bounds).
  \item 3-level alignment test: Gompertz, sigma matrix, and generated
    data are bit-identical between orig and 2025 for the same seed.
  \item A bug in \texttt{lme\_analysis()} (uninitialized \texttt{op} list when called
    without arguments) was discovered and fixed through the test
    suite.
\end{itemize}

\subsection{Reproducibility infrastructure}

\begin{itemize}
  \item Docker image via zzcollab \texttt{analysis} profile
    (\texttt{rocker/tidyverse:4.5} + renv)
  \item \texttt{renv.lock} capturing all dependencies (nlme, data.table,
    furrr, corpcor, MASS, ggplot2)
  \item \texttt{Makefile} with targets: \texttt{make r} (container shell),
    \texttt{make rstudio}, \texttt{make check-renv}, \texttt{make docker-build},
    \texttt{make test}
  \item GitHub repository: \texttt{rgt47/pmsimstats-ng}
\end{itemize}

\subsection{Dual-Architecture Extension}

The simulation framework now supports two data-generating
process (DGP) architectures for the biomarker-treatment
interaction, controlled by the \texttt{dgp\_architecture}
parameter in \texttt{generateData()}:

\begin{itemize}
\item \textbf{Architecture B} (\texttt{'mvn'}, default):
  The biomarker-treatment interaction is encoded as
  differential correlation in the MVN covariance structure.
  This is the original Hendrickson et al.\ (2020) approach.
  The PD constraint limits $c_{bm} \leq 0.45$ with AR(1)
  $\rho = 0.7$.

\item \textbf{Architecture A} (\texttt{'mean\_moderation'}):
  The interaction is an additive shift in the mean structure,
  applied post-MVN-draw. The BM-BR correlation block is
  omitted from $\Sigma$, removing the PD constraint entirely.
  Power is stable across carryover levels.
\end{itemize}

See \texttt{02-dgp-mean-moderation-vs-mvn.tex} for the
theoretical comparison and
\texttt{19-mean-moderation-implementation-notes.tex} for the
implementation derivation. The dual-architecture framework
resides on the \texttt{feature/mean-moderation-dgp} branch.

\bigskip\noindent\rule{\textwidth}{0.4pt}\bigskip

\section{Collaborator Deliverables}

\subsection{Goal 1: Did prazosin improve PTSD symptoms?}

Implemented as \texttt{analyze\_trial\_extended()} in
\texttt{R/carryover\_analysis.R}. Fits the LME model to actual trial data
and extracts both the main drug effect (\texttt{Dbc} coefficient) and
the biomarker-treatment interaction (\texttt{bm:Dbc}), with 95\%
confidence intervals, variance components, predicted response
curves per participant, and a clinical decision threshold
(the blood pressure value at which predicted benefit exceeds a
specified CAPS improvement). Status: complete.

\subsection{Goal 2a: Characterize carryover in actual data}

Implemented as \texttt{characterize\_carryover()} in
\texttt{R/carryover\_analysis.R}. Sweeps a range of candidate
carryover half-lives (default 0.25 to 8 weeks), fits the LME
model at each half-life with exponentially decaying \texttt{Dbc},
compares model fit via AIC/BIC and log-likelihood, and
identifies the best-fitting half-life. Validated on simulated
data: correctly recovers the true $t_{1/2} = 1.0$ from data
generated with that half-life. Status: complete.

\subsection{Goal 2b: Identify best sequence of response measures}

Conceptual design only. Would require per-timepoint residual
analysis to identify where the carryover signal is strongest
in the observed data. The existing model infrastructure
(\texttt{full\_model\_out = TRUE}) provides all necessary outputs
(fitted values, residuals, random effects per participant).
Status: not yet implemented.

\subsection{Goal 3a: Simulation power calculations}

Complete. The revised simulation produces 1,000-replicate
power estimates with proper Type I error control (2.5--5.8\%
across all designs), zero PD failures across all 162 parameter
combinations, sigma matrix caching for performance (8 minutes
for 200 reps, 47 minutes for 1,000 reps), and parallel
execution via furrr. Status: complete.

\subsection{Goal 3b: Actual analysis output structure}

Implemented in \texttt{analyze\_trial\_extended()}. Returns the full
model object, predicted treatment effect as a continuous
function of biomarker value, per-participant predicted response
curves, and clinical decision thresholds. The function can be
applied directly to the bundled \texttt{CTdata.rda} dataset from the
Raskind et al. trial. Status: complete.

\bigskip\noindent\rule{\textwidth}{0.4pt}\bigskip

\section{White Paper Documentation}

The consolidated repository includes the following technical
white papers documenting the audit findings, theoretical
derivations, and validation results.

\begin{table}[h]
\centering
\small
\begin{tabular}{rll}
\toprule
\# & Document & Content \\
\midrule
01 & Codebase Overview & Package structure, parameters, dependencies \\
02 & DGP Architecture Comparison & Mean moderation vs.\ MVN correlation \\
03 & Audit and Revision Report & Seven DGP corrections, PD resolution, deliverables \\
04 & Revised Power Analysis & Problems, fixes, validated results \\
06 & AR(1) Residual Correlation & lmer to nlme transition with Type I error validation \\
07 & Positive Definiteness Failures & PD failure prevalence and causes \\
08 & BM-BR Correlation Decay & Pharmacokinetic derivation of $\lambda_{\text{cor}}$ \\
09 & Carryover Correlation Artifact & BM-BR correlation artifact analysis \\
10 & Carryover Analysis Assessment & Carryover characterization in actual data \\
13 & Figure 4 Walkthrough & Pre-audit methodology reference \\
14 & Trial Analysis Guide & Collaborator goal implementation \\
15 & Tidyverse Alignment Plan & Alignment between orig and 2025 pipelines \\
16 & Tidyverse Code Review & Code review of 2025 pipeline \\
17 & Simplification Plan & Codebase simplification strategy \\
18 & Response Parameter Sensitivity & Response parameter sensitivity before/after \\
19 & Mean Moderation Implementation & Implementation derivation for Architecture~A \\
\bottomrule
\end{tabular}
\end{table}

Note: document 05 has been removed (duplicate of 04).
Document 11 now resides in \texttt{analysis/report/}.

\bigskip\noindent\rule{\textwidth}{0.4pt}\bigskip

\section{Presentation Outline (7 Slides)}

\subsection{Slide 1: Problem and Motivation}

\begin{itemize}
  \item N-of-1 trial designs for biomarker-treatment interactions
  \item Four designs compared: OL, CO, Hybrid, OL+BDC
  \item Clinical context: prazosin for PTSD, blood pressure as
    predictive biomarker (Raskind et al. 2013, Hendrickson et al.
    2020)
  \item Question: which trial design has the best power to detect
    who responds?
\end{itemize}

\subsection{Slide 2: Audit Findings -- Seven DGP Corrections}

\begin{itemize}
  \item AR(1) autocorrelation (was compound symmetry)
  \item Exponential BM-BR correlation decay (was step function)
  \item nlme + corCAR1 analysis model (was lmer, Type I error
    13--17\% corrected to 5\%)
  \item Scale factor removal, timepoint-1 fix, Gompertz alignment
  \item Updated defaults: rho=0.7, c.bm $\leq$ 0.45, t\_half 0--1.0 wks
\end{itemize}

\subsection{Slide 3: Resolving the PD Problem}

\begin{itemize}
  \item Original: 24.7--63.0\% of covariance matrices silently corrected
  \item Root cause: OL+BDC expectancy-driven variance discontinuity
    under compound symmetry
  \item Solution: AR(1) + parameter constraints = zero PD failures
    across all 162 combinations
  \item Consequence: cross-design power comparisons are now
    unconfounded
\end{itemize}

\subsection{Slide 4: Resolving the Power Dropoff Problem}

\begin{itemize}
  \item Carryover erodes on/off-drug contrast through the variable
    chain: ondrug $\to$ tod $\to$ tsd $\to$ Dbc $\to$ bm:Dbc
  \item Original half-lives (0.1--0.2 wks) too short for genuine
    carryover
  \item Revised half-lives (0.5--1.0 wks) produce clinically
    meaningful power differences across designs
  \item Conditional formula: bm:Dbc (crossover) vs bm:t (open-label)
\end{itemize}

\subsection{Slide 5: Consolidated Framework}

\begin{itemize}
  \item Two validated pipelines: data.table (orig, gold standard) +
    tidyverse (2025, bit-identical)
  \item 67 unit tests, Docker reproducibility, zzcollab workspace
  \item 1,023-line tidyverse pipeline (38\% reduction from cleanup)
  \item GitHub: rgt47/pmsimstats-ng
\end{itemize}

\subsection{Slide 6: Collaborator Deliverables}

\begin{itemize}
  \item analyze\_trial\_extended(): drug effect + interaction + predicted
    curves + clinical thresholds
  \item characterize\_carryover(): sweep half-lives, compare AIC, find
    best fit
  \item 1,000-rep validated power with Type I error 2.5--5.8\%
  \item 7 white papers documenting methodology
\end{itemize}

\subsection{Slide 7: Next Steps}

\begin{itemize}
  \item Goal 2b: per-timepoint residual analysis for carryover signal
    localization
  \item Final figure reproduction with consolidated code and revised
    defaults
  \item Manuscript describing the revised methodology and consolidated
    results
  \item Sensitivity analysis: carryover decay shape (exponential vs
    Weibull), separate correlation decay rate
\end{itemize}

\vfill
\noindent\rule{\textwidth}{0.4pt}
{\footnotesize
Rendered on 2026-03-25 at 19:00 PDT.\\
Source: \textasciitilde/prj/alz/10-pmsimstats-ng/pmsimstats-ng/docs/03-audit-and-revision-report.tex}
\end{document}
