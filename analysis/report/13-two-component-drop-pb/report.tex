...
\renewcommand{\stamptime}{2026-09-09 08:52 PDT}
...
At the reference calibration TV carries a standard deviation of 10
against 8 for BR and 10 for PB, so it contributes approximately 38\%
of the component variance. Removing it reduces the residual variation
against which a fixed interaction is tested; on a variance argument
alone, standard errors shrink by roughly a fifth and power rises
correspondingly.
...
interaction estimand. It carries no biomarker coupling in either
architecture, the package provides no parameter with which to give it
one, and paper 06's omitted-variable identity shows that a component
uncorrelated with the biomarker cannot displace the biomarker slope
regardless of its magnitude. The present results are consistent with
that identity and extend it in one respect: paper 06 established it
under a single architecture, and it now holds under both.
...
drift, a case discussed in Section 4.3. And it contributes realism to
absolute power figures calibrated against a clinical reference
trajectory, so any paper reporting power as a design input rather
than as a comparison should retain it.
...
transparency and cost, not numerical headroom.
...
This bears on the present paper directly. Our conclusion that TV is
inert holds under the DGP as implemented, in which the biomarker-TV
correlation is structurally zero. It would not hold under a DGP
permitting such a coupling; under paper 06's own identity it would
certainly not, since a biomarker correlated with natural-history
drift would displace the one-component slope exactly as a
PB-correlated biomarker does. The inertness of TV is thus a property
of the current implementation rather than of the component, and the
natural next step is to add the missing parameter and test the half
of paper 06's claim that has never been run.
...
\item
  \textbf{TV is a variance term, not a signal term.} It carries no
  biomarker coupling in either architecture, and the package
  provides no means of giving it one. Its removal raises absolute
  power without disturbing the ranking, the direction of the
  carryover effect, or the ordering among designs.
\item
...
\item
  \textbf{The inertness of TV is a property of the implementation.}
