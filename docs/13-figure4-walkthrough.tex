...
\textbf{Historical reference.} This document describes the
Figure~4 methodology as implemented in the original
Hendrickson et al.\ (2020) publication code (commit
\texttt{42ac030}). Several aspects have been superseded by
the revised codebase:

\begin{itemize}[leftmargin=1.5em,topsep=2pt,itemsep=1pt]
\item Compound symmetry correlation has been replaced by
  AR(1) autocorrelation
\item The \texttt{lme4::lmer} analysis model has been
  replaced by \texttt{nlme::lme} with \texttt{corCAR1}
\item The scale factor of 2 on carryover decay has been
  removed
\item The step-function BM-BR correlation has been replaced
  by exponential decay ($\lambda_{\text{cor}}$)
\item The parameter grid ($c_{bm} = 0.6$, $\rho = 0.8$) is
  infeasible under the revised MVN architecture but is now
  evaluable under Architecture~A (mean moderation)
\end{itemize}

\noindent For the revised analysis, see
\texttt{04-revised-power-analysis.tex}. For the
dual-architecture framework, see
