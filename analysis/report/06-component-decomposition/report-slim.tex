...
\fancypagestyle{plain}{%
  ...
  \renewcommand{\headrulewidth}{0pt}%
  \renewcommand{\footrulewidth}{0.4pt}}
\renewcommand{\stampsource}{\textasciitilde{}/\allowbreak{}prj/\allowbreak{}res/\allowbreak{}36-pmsimstats-ng/\allowbreak{}pmsimstats-ng/\allowbreak{}analysis/\allowbreak{}report/\allowbreak{}06-component-decomposition/\allowbreak{}report-slim.Rmd}
\renewcommand{\stamptime}{2026-06-17 10:49 PDT}
...
lumping biases inference, and whether a component decomposition
...
the one-component estimator, which expresses both the lumped
treatment effect and the lumped biomarker slope as sums of
...
and varies the unmodelled-component magnitudes (1,000 replicates per
...
follow. (i) \emph{Biomarker orthogonal to PB and TV, hybrid design:} the
one-component estimator is essentially unbiased across the whole
grid (mean absolute bias \(0.016\) at \(N=35\), \(0.006\) at \(N=150\),
within one Monte Carlo standard error of zero) and strictly more
...
dominated. (ii) \emph{Biomarker coupled to PB, standard hybrid design:}
...
either. (iii) \emph{Biomarker coupled to PB, balanced-placebo design:}
...
contamination range (bias within one Monte Carlo SE of zero,
coverage \(0.94\)--\(0.96\)) where the one-component estimator does not,
at the cost of some power.

\textbf{Conclusions.} The benefit of decomposition is jointly determined
...
or TV, the lumped slope is biased and decomposition is necessary, but
...
Pharmacometrics has long modelled disease progression and drug
effect as separable longitudinal processes\textsuperscript{\citeproc{ref-holford2015}{1},\citeproc{ref-gomeni2009}{2}}; Kaptchuk and colleagues decomposed the placebo
response itself into observation, ritual, and
practitioner-relationship components in a three-arm IBS trial\textsuperscript{\citeproc{ref-kaptchuk2008components}{3}}; neuroimaging has directly dissociated
pharmacological from expectancy-driven analgesia using objective
neural signatures\textsuperscript{\citeproc{ref-wager2013nps}{4},\citeproc{ref-zunhammer2018nps}{5}}. The
...
normalise. The question this paper addresses is not whether the
...
\textbf{The optimal choice is conditional: decompose when the candidate biomarker may correlate with $PB$ or $TV$; use the lumped analysis when domain knowledge supports orthogonality.}
...
established in section 6: the lumped biomarker-treatment slope
...
biomarker orthogonal to it yields an unbiased lumped estimate;
...
regulates dopaminergic signalling in expectancy circuits, and
predicts placebo-response magnitude in IBS and chronic pain
populations\textsuperscript{\citeproc{ref-hall2012comt}{6},\citeproc{ref-wang2022comt}{7}}. A trial using COMT
as a predictive biomarker for prazosin response would face a
lumped slope that conflates pharmacological prediction with
...
does not modulate belief. For both, the lumped pharmacological
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item The identity (section 6) is exact: bias in the lumped slope $= w_{PB}\beta_{bm}^{PB} + w_{TV}\beta_{bm}^{TV}$; it is zero if and only if the biomarker is orthogonal to $PB$ and $TV$, regardless of the magnitude of those components.
\item Study A (section 7.3; 1,000 replicates per alternative cell, 5,000 per null) confirms the orthogonal arm: mean absolute bias in the lumped $\hat\beta_{bm:D_{bc}}$ is $0.016$ against a true value of $-2.25$ at $N=70$, with no trend across $m_{PB} \in \{0,1,3,6,10\}$ or $m_{TV} \in \{-1,0,1,2\}$.
...
phase-augmented LME approximation. Section 6 works through the
...
biomarker-validation decision criterion under the PATH framework\textsuperscript{\citeproc{ref-kent2020path}{8}}. No published N-of-1 trial has applied this
...
sustained exposure to integrate fully\textsuperscript{\citeproc{ref-raskind2013prazosin}{9}}.
...
values from the\textsuperscript{\citeproc{ref-hendrickson2020}{10}} reference dataset are
...
\(E_{\max}\) (Hill) model\textsuperscript{\citeproc{ref-holford2011backstory}{11}} describes the
...
placebo treatments\textsuperscript{\citeproc{ref-tilburt2008}{12}}. As the introduction sets out,
the placebo response is mediated by well-characterised
neurobiological pathways\textsuperscript{\citeproc{ref-finniss2010}{13}--\citeproc{ref-carlino2016benedetti}{17}}:
belief-driven release of endogenous opioids and dopamine,
anticipatory hypothalamic-pituitary-adrenal regulation, and
top-down attentional shifts, dissociated from pharmacology by the\textsuperscript{\citeproc{ref-wager2013nps}{4}} neurologic pain signature\textsuperscript{\citeproc{ref-zunhammer2018nps}{5}} and
shown to have genetic correlates\textsuperscript{\citeproc{ref-hall2012comt}{6},\citeproc{ref-wang2022comt}{7}}.
...
response expectancy in the sense of\textsuperscript{\citeproc{ref-kirsch1985}{18}}, the patient's
...
substantial. Open-label placebo trials\textsuperscript{\citeproc{ref-kaptchuk2010olp}{19}--\citeproc{ref-nurko2022}{21}} demonstrate that a clinically
...
analogue in the placebo-research literature.\textsuperscript{\citeproc{ref-kaptchuk2008components}{3}}
...
\emph{{[}Extended treatment in supplement S2.{]}}

\begin{bullets}

\textbf{Additivity is a modelling assumption; published literature documents subadditivity between pharmacological and belief effects in some contexts; the phase-augmented analysis partly absorbs it as a side benefit.}

\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item The generalised model adds $\gamma \cdot BR_{it} \cdot PB_{it}$; $\gamma < 0$ encodes subadditivity, $\gamma = 0$ recovers the additive form.
...
\item Simulation quantifying bias scaling with $\gamma$ is deferred to the production programme (section 7).
\end{itemize}
...
contexts\textsuperscript{\citeproc{ref-kaptchuk2008components}{3},\citeproc{ref-benedetti2014}{14},\citeproc{ref-bingel2011}{22}}
and can arise from shared neural substrate, ceiling effects, or
belief-dynamics adaptation. A generalised model with
...
\subsubsection{Natural history: existence, testability, and functional form}\label{natural-history-existence-testability-and-functional-form}
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Disease class determines $TV$ sign: positive for acute recovery, near-zero for stable chronic, negative for progressive conditions; selection-driven enrolment adds a positive $TV$ component regardless of class due to regression to the mean [@hrobjartsson2001; @hrobjartsson2010].
...
enrolment selection. Acute conditions trend \(TV\)-positive
...
\(TV\)-negative. Selection-driven enrolment adds a positive \(TV\)
...
apparent improvement in early trial weeks\textsuperscript{\citeproc{ref-hrobjartsson2001}{23},\citeproc{ref-hrobjartsson2010}{24}}. The zero-\(TV\) assumption is testable: a
model including \(TV\) returns \(\hat{m}_{TV}\) with a variance
estimate; lumping \(TV\) into \(\varepsilon\) is worse, because the
...
designs\textsuperscript{\citeproc{ref-colloca2004}{25},\citeproc{ref-rohsenow1981}{26}} that manipulate or record the
...
the vocabulary of the estimands framework\textsuperscript{\citeproc{ref-iche9r1}{27}}, because
...
difficulty Senn has emphasised for individual treatment effects\textsuperscript{\citeproc{ref-senn2004}{28},\citeproc{ref-senn2018}{29}}: a genuine participant-by-component signal
...
now available for repeated-measures and N-of-1 data. Liao et al.\textsuperscript{\citeproc{ref-liao2023}{30}} model carryover through a Bayesian distributed-lag
structure with autocorrelated errors, relaxing the single-rate
exponential assumption at the cost of additional parameters;
Daza\textsuperscript{\citeproc{ref-daza2018}{31}} formalises an average-period treatment-effect
estimand built to accommodate autocorrelation, trend, and slow
carryover jointly; and G\{"a\}rtner et al.\textsuperscript{\citeproc{ref-gartner2023}{32}}
...
\begin{orig}
\emph{{[}Figure 1 here: two-panel trajectory plot. Left panel shows
...
Component contributions are labelled in each panel.{]}}
...
is structured under the\textsuperscript{\citeproc{ref-morris2019}{33}} ADEMP framework, specifying
...
\subsection{Pilot validation}\label{pilot-validation}

\emph{{[}Details in supplement S1.{]}}
...
\item At $N = 35$, rank-deficiency dropped the $D_{bc}{:}\text{phase}$ interaction term, requiring larger $N$ and a richer design in the production programme.
\item The predicted bias manifests in the mean of $\hat\beta_{bm:D_{bc}}$, not the Wald-test rejection rate; the production programme reports coefficient means against true values.
\end{itemize}
...
mean, not the rejection rate, so the production programme
...
\begin{orig}
The production programme follows the\textsuperscript{\citeproc{ref-morris2019}{33}} ADEMP framework and
...
\subsection{Study A: orthogonal-biomarker arm (1,000 replicates per alternative cell)}\label{study-a-orthogonal-biomarker-arm-1000-replicates-per-alternative-cell}
...
\subsection{Study B pilot: coupling-arm simulation (100 replicates per cell)}\label{study-b-pilot-coupling-arm-simulation-100-replicates-per-cell}
...
\subsection{Study C: recovery under a belief-decoupling design}\label{study-c-recovery-under-a-belief-decoupling-design}
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item In the MVN process the biomarker-$BR$ coupling loads on the drug state $D_{bc}$ and the biomarker-$PB$ coupling scales with the expectancy $\eta$; the decomposition $Sx \sim bm + t + D_{bc} + \eta + bm{:}D_{bc} + bm{:}\eta$ isolates $\beta_{bm}^{BR}$ on $bm{:}D_{bc}$ only if $D_{bc}$ and $\eta$ vary independently.
...
collinearity: a balanced-placebo (open-hidden) design\textsuperscript{\citeproc{ref-colloca2004}{25},\citeproc{ref-rohsenow1981}{26}} interposes a covert on-drug phase, in which
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item At $N=150$ the decomposition bias of $\hat\beta_{bm:D_{bc}}$ stays within one Monte Carlo SE of zero across $c_{bm,PB} \in \{0, 0.1, 0.2, 0.3\}$ ($-0.022$, $+0.008$, $+0.021$, $-0.026$; MCSE $\approx 0.02$), while the one-component bias rises to $+0.16$, $+0.34$, $+0.48$.
...
every value within one Monte Carlo standard error (about \(0.02\)) of zero,
...
recovery, and replication under Architecture A coupling remain open.

\end{orig}

\begin{longtable}[]{@{}lllll@{}}
\caption{\label{tab:studyc-power} Rejection rate (power) for the
biomarker-by-treatment interaction at \(\alpha = 0.05\) under the
...
one-component estimate toward the null.}\tabularnewline
\toprule\noalign{}
...
precision-medicine question, formalised in the PATH statement\textsuperscript{\citeproc{ref-kent2020path}{8}}. Whether it depends on the BR-PB-TV decomposition
...
caveat that\textsuperscript{\citeproc{ref-senn2018}{29}} and\textsuperscript{\citeproc{ref-senn2004}{28}} raise for individual
...
discontinuation and randomised-withdrawal designs\textsuperscript{\citeproc{ref-colloca2004}{25},\citeproc{ref-rohsenow1981}{26}}, the crossover with on/off periods, and
...
descends\textsuperscript{\citeproc{ref-holford2015}{1},\citeproc{ref-gomeni2009}{2},\citeproc{ref-chen2021}{34}} operate at the group
