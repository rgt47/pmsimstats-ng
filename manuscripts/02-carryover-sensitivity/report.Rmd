...
**Implementation note.** At the time of writing the tidyverse
collection implements only the exponential form (see
`implementations/tidyverse/R/functions.R`, function
`apply_carryover_to_component`). Adding linear and Weibull
variants is a prerequisite simulation step, tracked as a
prerequisite in `analysis/02-carryover-sensitivity/README.md`.
...
`implementations/original-extended/` (data.table, dual
architecture) for primary runs. Tidyverse runs via
`implementations/tidyverse/` serve as cross-validation.
Reproducibility metadata (R version, `renv.lock` hash,
`implementations/` commit SHA, simulation seed) are recorded in
each output `.rds` file.
