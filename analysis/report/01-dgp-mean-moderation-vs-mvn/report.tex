...
\title{Two Architectures for Simulating Biomarker-Treatment Interactions: Implications for Statistical Power Under Carryover}
...
\setstretch{1.2}
\renewcommand{\rmdsource}{Source: \texttt{\~/Dropbox/prj/alz/10-pmsimstats-ng/pmsimstats-ng/analysis/report/01-dgp-mean-moderation-vs-mvn/report.Rmd}}
\renewcommand{\rendertimestamp}{Rendered: 2026-05-09 12:19 PDT}
...
moderation (Architecture\textasciitilde A), under which the biomarker scales
the treatment effect in the population mean structure; and
differential correlation (Architecture\textasciitilde B, the multivariate
...
timepoint. Under Architecture\textasciitilde B, carryover erodes the
...
B-versus-A contrast), while Architecture\textasciitilde A drops only from
...
matched Architecture\textasciitilde A loss; the Hybrid design produces a
...
Architecture\textasciitilde B uses. The 36-cell production factorial
...
Type\textasciitilde I error sits in \([0.010, 0.074]\), with no
...
(Hybrid), and 2.8\% (OL+BDC). All three values land within the
8-13\% relative-loss range that the broader N-of-1 simulation
literature reports for direct-mean-moderation DGPs at small
sample size.
...
Power declines substantially under Architecture B in the
OL+BDC and Hybrid designs: relative loss from \(t_{1/2} = 0\) to
...
matched Architecture-A OL+BDC loss (2.8\%) and lands on the
lower edge of the 40-60\% relative-loss range that the
broader N-of-1 simulation literature reports for the MVN
differential-correlation DGP. Two-proportion \(z\)-tests on the
...
The Section\textasciitilde3.1 production results are at \(N = 35\), where
...
t_{1/2} \in \{0, 0.5, 1.0\}\) weeks), all on the Hybrid OL+BDC
design, using 8-core \texttt{parallel::mclapply}. Wall-clock
483\textasciitilde s; convergence 100\% across 9\{,\}600 fits.
...
are 9.3 percentage points (Architecture\textasciitilde B; \(z = 7.4\),
\(p < 10^{-12}\)) and 1.8 percentage points (Architecture\textasciitilde A;
\(z = 2.0\), \(p \approx 0.05\)). The qualitative ordering matches
Section\textasciitilde3.1's \(N = 35\) result. The absolute magnitudes are
...
robustness numbers (3\% and 9\% relative loss for Architecture\textasciitilde B
versus 0.4\% and 1.8\% for Architecture\textasciitilde A) requires
acknowledging this saturation effect; the \(N = 35\) production
results in Section\textasciitilde3.1 are the canonical reference for the
narrative magnitude.

\textbf{Type~I error.} Across all 36 null cells of the \(N = 35\)
production run, type\textasciitilde I error sat in \([0.010, 0.074]\), with
mean 0.043 and median 0.041. There is no architecture-specific
...
informative cell of the Section\textasciitilde3.1 narrative.
...
Architecture\textasciitilde B cells maintained mean near \(-0.234\) across
