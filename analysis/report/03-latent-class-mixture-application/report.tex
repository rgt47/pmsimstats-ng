...
\renewcommand{\rmdsource}{Source: \texttt{\~/Dropbox/prj/res/36-pmsimstats-ng/pmsimstats-ng/analysis/report/03-latent-class-mixture-application/report.Rmd}}
...
unit time than a parallel-group randomised trial of equivalent total
...
treatments would let clinicians stratify treatment decisions at the
...
The corresponding statistical estimand is the biomarker-by-treatment
...
(DGP)\textsuperscript{\citeproc{ref-hendrickson2020}{9}}. The present
...
effect is routinely modelled via random slopes on the treatment
indicator and via interaction terms in fixed-effects regression. In its
most economical form, the moderation enters the conditional mean of the
...
long histories of formalisation and identifiability analysis in their
...
performance on cognitive tests was modelled as the sum of a single
...
summarised by factor scores along one or more dimensions.
...
(LCA).\textsuperscript{\citeproc{ref-goodman1974}{17}} placed the LCA
...
probabilities\textsuperscript{\citeproc{ref-daytonmacready1988}{20},\citeproc{ref-bandeenroche1997}{21}}.\textsuperscript{\citeproc{ref-mclachlanpeel2000}{22}}
...
or as discrete.\textsuperscript{\citeproc{ref-bartholomew1987}{23}}
framed this as the choice between `graded' and `unfolding' models of
individual differences and argued that the data themselves rarely settle
the question without strong prior assumptions. The empirical literature
...
forms of psychopathology display latent-class structure consistent with
...
labour-market participation, firm productivity) where standard
regression assumptions of identical agents were known to
fail.\textsuperscript{\citeproc{ref-heckmansinger1984}{26}}, in an
...
non-parametric maximum likelihood estimator anticipated much of what
would later become standard finite-mixture methodology in biostatistics,
and\textsuperscript{\citeproc{ref-lindsay1995}{27}} placed the broader
theory of mixture likelihoods on a unified mathematical footing.
...
response functions and to predict segment membership for new
consumers.\textsuperscript{\citeproc{ref-desarbocron1988}{28}}
formalised this as `clusterwise linear regression', in which
observations were partitioned into clusters with cluster-specific
regression
coefficients.\textsuperscript{\citeproc{ref-wedeldesarbo1995}{29}}
generalised the approach to generalised linear
models.\textsuperscript{\citeproc{ref-jedidi1997}{30}} extended it to
structural equation models with unobserved segments. Within this
econometric-marketing literature, the gating function (the model for the
probability of segment membership as a function of covariates) became a
first-class object, and methodological work focused on whether the
gating model could share covariates with the within-segment regression
model.\textsuperscript{\citeproc{ref-jacobs1991}{31}}, working
...
unobserved heterogeneity as a \emph{nuisance} in many applications, and
was correspondingly attentive to whether the heterogeneity distribution
was identified, weakly identified, or over-fit. The identifiability
concerns raised by\textsuperscript{\citeproc{ref-heckmansinger1984}{26}}
and elaborated by\textsuperscript{\citeproc{ref-lindsay1995}{27}}
translate directly into N-of-1 settings where sample sizes are small and
the heterogeneity is the principal object of inference rather than a
...
factor structure with discrete between-class
structure.\textsuperscript{\citeproc{ref-yung1997}{24}}, in
Psychometrika, developed the factor mixture model as a finite mixture of
confirmatory factor models, where each class had its own factor-loading
and residual covariance
structure.\textsuperscript{\citeproc{ref-arminger1999}{25}} placed
factor mixture models in the broader Mplus-style structural equation
framework.\textsuperscript{\citeproc{ref-lubkemuthen2005}{2}} conducted
the first systematic Monte Carlo evaluation of factor mixture models and
clarified the conditions under which the discrete-class component was
empirically distinguishable from continuous
heterogeneity.\textsuperscript{\citeproc{ref-clark2013}{32}} surveyed
...
characterise developmental trajectories of antisocial behaviour, and has
...
overextraction
hazards.\textsuperscript{\citeproc{ref-bauercurran2003}{1}}, in
...
than evidence of substantive subpopulation
structure.\textsuperscript{\citeproc{ref-bauer2007}{38}} elaborated on
the implications for substantive psychological research and recommended
caution in interpreting class structure without independent
validation.\textsuperscript{\citeproc{ref-lubkemuthen2005}{2}}
...
recoverable.\textsuperscript{\citeproc{ref-nylund2007}{39}} systematised
...
channels.\textsuperscript{\citeproc{ref-verbekelesaffre1996}{40}}, in
the Journal of the American Statistical Association, proposed a linear
...
longitudinal HIV-infection data in which subgroups of patients displayed
qualitatively different CD4 trajectories. The model is essentially a
growth mixture model in biostatistical clothing, with the standard
random-intercept-and- slope linear mixed model augmented by a discrete
latent class
indicator.\textsuperscript{\citeproc{ref-zhangdavidian2001}{41}}
...
and ordinal outcomes, with optional joint modelling of survival outcomes
via shared latent class structure. The \texttt{lcmm} framework has been
deployed in cognitive ageing research, Alzheimer's disease progression
modelling, and clinical-trial subgroup discovery. The \texttt{flexmix}
package in
...
has been used in pharmacokinetic and pharmacodynamic modelling where
...
not been systematically applied to aggregated N-of-1 trial designs. The
published N-of-1 methods literature is dominated by within-participant
effect
...
under the heading of personalised-medicine power
...
of the latent-class formulation. The mixture-formulation question is to
our knowledge unaddressed in the existing N-of-1 methodology literature.
...
all measurements within a participant and therefore in principle
...
Bauer-Curran-Lubke-Muthén identifiability cautions therefore apply with
full force, and the analysis-strategy implications are non-trivial: even
if a latent-class DGP is the biologically correct generative model,
...
existing psychometric simulation literature has not addressed at N-of-1-
relevant sample sizes.

Third, carryover, which enters the analysis as a continuous decay in the
effective drug indicator \(D_{it} = \exp(-\lambda t_{sd,it})\),
...
The remainder of the paper is organised as follows. A notation section
fixes terminology that is used throughout. The latent- class generative
form, its MVN second-moment approximation, and the regimes in which the
approximation breaks down are developed in three subsequent sections.
The implications for power under carryover and the identifiability
constraints that bound mixture analysis at trial-relevant sample sizes
are addressed next. A finite-mixture taxonomy section locates the
relevant model families (LCA, GMM, FMM, regression mixtures); a
biomarker- moderation spectrum section organises the available DGP
encodings moment by moment. Implications for analysis and design, and
software for mixture analyses, are then drawn together. The discussion
section addresses three open methodological questions and references the
five-study simulation programme described in the companion simulation
plan, which constitutes the empirical core of the present line of work
and is the next deliverable in the programme.
...
\(D_{it} = 1\) during on-drug periods, with \(t_{sd,it}\) the time since
drug discontinuation and \(\lambda\) the decay rate. \(Z_i\) denotes an
unobserved class label. The half-life of carryover is
\(t_{1/2} = \ln 2 / \lambda\) and is allowed to differ between the DGP
...
\(\pi(B_i) = \Pr(Z_i = 1 \mid B_i)\) is the class-membership probability
(typically a logistic function of the biomarker), and the
...
Within-class temporal structure follows the same modGompertz form used
in single-class formulations, with class-specific parameters
substituted.
...
the class-conditional mean trajectory does depend on \(D_{it}\): under
full carryover washout (\(D_{it} \to 0\)) the two class-conditional
...
order, by a single multivariate normal with
...
estimating class membership directly, because the class label is itself
...
upper-bounds what a mixture analysis could recover at the same carryover
level; whether this bound is tight in the trial-relevant parameter
regime is an empirical question not addressed by existing simulation
infrastructure. The bound is asymptotic: at finite \(N\) in the
trial-relevant range, finite-sample bias in mixture estimation can drive
the realised gap below the asymptotic upper bound or, in adverse
...
between mixture-DGP-with-MVN-analysis and
mixture-DGP-with-mixture-analysis quantifies the power recoverable by a
faithful analysis model; the difference between the two MVN-analysis
cells quantifies the approximation error inherent to the second-moment
encoding. A null check on MVN-DGP-with-mixture-analysis verifies that
the mixture model does not extract spurious power on data for which no
...
stabilise at values that support confident class extraction in published
psychometric
...
relevant sample sizes may be either underpowered or unidentified, even
when the underlying DGP is genuinely mixture-structured. The question is
therefore not whether mixture formulations are more faithful, but
whether the fidelity gain is recoverable at realistic sample sizes, or
whether analysis-strategy mitigations available within standard
mixed-effects practice (restricted analysis, weighting, within-subject
contrasts; see\textsuperscript{\citeproc{ref-hendrickson2020}{9}})
deliver a better power-per-complexity trade. These mitigations have
better- understood inferential properties at the relevant sample sizes.
Resolving the mixture-versus-mitigation question empirically requires
the simulation programme described in the companion simulation plan
(Study 5 in particular addresses small-\(N\) identifiability and type I
error explicitly).
...
interaction problem. The discussion below introduces each family, its
...
mixture models for cross-sectional data. Each participant belongs to one
of a small number of unobserved classes; the observed variables have
...
(GMM)\textsuperscript{\citeproc{ref-muthenshedden1999}{33}--\citeproc{ref-muthen2004}{35}}
...
random-effects variation. A GMM is a longitudinal linear mixed model in
which the population distribution of random effects is a mixture of
multivariate normals, each corresponding to a distinct trajectory class.
...
written down earlier in this
paper.\textsuperscript{\citeproc{ref-bauercurran2003}{1}}
and\textsuperscript{\citeproc{ref-bauer2007}{38}} document the
...
(FMM)\textsuperscript{\citeproc{ref-lubkemuthen2005}{2},\citeproc{ref-yung1997}{24},\citeproc{ref-arminger1999}{25}}
...
provides a principled way to represent a biomarker as both a noisy class
indicator (via the gating function) and a continuous modulator of
within-class response (via factor loadings on the response factor). The
hybrid representation matches more biological scenarios than either pure
LCA or pure factor analysis, at the cost of correspondingly higher
...
ordinary linear model with a treatment-by-biomarker interaction posits a
...
and the magnitude of the within-responder drug effect. \texttt{flexmix}
in
R\textsuperscript{\citeproc{ref-leisch2004}{43},\citeproc{ref-grunleisch2008}{44}}
...
covariance-varies case is typically encountered only as a constrained
submodel used for testing measurement-invariance
hypotheses\textsuperscript{\citeproc{ref-meredith1993}{49},\citeproc{ref-widamanreise1997}{50}}.
The corresponding biostatistical model has two subgroups with identical
expected outcomes but differing residual variance or within-subject
...
encoding's appeal lies in its tractability for power simulation under
multivariate normality: the entire DGP reduces to drawing from a single
MVN with a constructed covariance. Its cost is that it does not
correspond cleanly to any natural generative mechanism.

\subsection{Combined mean and covariance
moderation}\label{combined-mean-and-covariance-moderation}

\textsuperscript{\citeproc{ref-arminger1999}{25}} develop finite
mixtures of conditional mean- and covariance-structure models in which
both the expected-response vector and the residual covariance matrix
differ between classes and may depend on observed covariates within
class. This is the general psychometric form: it reduces to a
mean-moderation model in the limit of a single class with
...
faithful regime for biomarker-guided psychiatric
trials.\textsuperscript{\citeproc{ref-verbekelesaffre1996}{40}} provide
the analogous treatment for linear mixed-effects models with
heterogeneity in the random-effects population, the more direct
generalisation of the repeated-measures structure used in N-of-1 trial
simulation.

\subsection{Location-scale moderation}\label{location-scale-moderation}

Generalised additive models for location, scale, and shape
(GAMLSS)\textsuperscript{\citeproc{ref-gamlss2005}{51}} and the related
...
location effect, which is Architecture A) and altered response
...
\textsuperscript{\citeproc{ref-verbekelesaffre1996}{40}},\textsuperscript{\citeproc{ref-zhangdavidian2001}{41}},
and\textsuperscript{\citeproc{ref-proustlima2017}{42}} develop linear
mixed-effects models in which the random-effects distribution is itself
a finite mixture, allowing class-specific random-intercept and
...
structure at the fixed-effect level. The \texttt{lcmm} R
...
substantive scientific question, not a parameterisation convenience, and
that recognising this choice opens both a sensitivity-analysis programme
(the spectrum of biomarker- moderation specifications above) and an
analysis-strategy programme (class-aware mixture analyses where
...
simulation programme that would empirically resolve the
...
line of work and is the next deliverable in the programme. Second, the
...
indicator \(D_{it} = \exp(-\lambda t_{sd,it})\) has a clean
...
A natural reaction to the simulation programme above is that the DGP is
structured (a three-component decomposition into biological response,
placebo-belief response, and time-variant natural history under the
standard pmsimstats parameterisation; or a latent-class mixture under
the alternatives in this paper) while the analysis model is a four-term
linear mixed model
...
effects) is impractical for trial-relevant N for four reasons: the trial
design must supply the contrasts that identify each component
...
moderation estimand more than it reduces residual confounding; the non-
linear fitting machinery has higher convergence-failure rates and is
...
dgp-mean-moderation-vs-mvn companion).
...
\texttt{Sx\ \textasciitilde{}\ bm\ +\ t\ +\ Dbc\ +\ phase\ +\ Dbc:phase\ +\ bm:Dbc\ +\ bm:Dbc:phase\ +\ (1\ \textbar{}\ ptID)}.
...
once the primary cells are run. The pedagogy doc at
...
in Study 1 is a natural mechanism for generating \textbf{apparent
subadditivity} in a downstream one-component analysis, even when no
direct \(BR \times PB\) interaction is specified in the
class-conditional generative model. The companion methods paper at
...
The first mechanism is \textbf{direct mechanistic interaction}: shared
neural substrate, ceiling effects on the symptom scale, or
belief-dynamics adaptation each produce a true non-additive coupling
between drug and placebo response. The second mechanism is
\textbf{emergent subadditivity from latent- class structure}: if the
trial population is a mixture of responder and non-responder classes
(the formulation in §3 of the present paper) and class membership
correlates with both \(BR\) and \(PB\) strength (responder-class
participants have both higher \(BR\) and higher \(PB\) on average), then
the marginal expected value of \(BR \cdot PB\) over the unobserved class
label has a non-zero covariance term whose magnitude depends on the
gating function \(\pi(B)\). From the additive-extension analysis this
looks exactly like a \(\gamma\)-weighted interaction.

The Study 1 pre-registration (see
\texttt{analysis/scripts/latent-class-mixture-application/00-ademp-\ pre-registration.md})
accordingly includes a \texttt{pb\_class\_correlation} axis that varies
...
paper is therefore not just methodological convenience but a direct
route to falsifying or confirming a class-aware versus mechanistic-
interaction interpretation of any apparent subadditivity finding.
...
Crowther\textsuperscript{\citeproc{ref-morris2019}{56}} in
\texttt{analysis/scripts/latent-class-mixture-application/00-ademp-pre-registration.md};
...
\textbf{Pre-registration.} ADEMP tables for all five
studies\textsuperscript{\citeproc{ref-morris2019}{56}} are committed at
\texttt{analysis/scripts/latent-class-mixture-application/\ 00-ademp-pre-registration.md},
with cell counts (\textasciitilde2,300 cells across the programme),
...
\textbf{Class-aware analysis wrapper.} An \texttt{lcmm\_analysis()}
wrapper around \texttt{lcmm::hlme} is committed at
\texttt{01-lcmm-wrapper.R} with the same input-output contract as the
existing \texttt{lme\_analysis()}. The wrapper uses a two-stage fit (a
...
with logistic gating in the standardised biomarker and class- specific
...
marginal \(\mathrm{Cor}(B, BR) = 0.63\) that the linear-mixed bm:Dbc
test detects (\(p = 0.002\)) without class awareness; the lcmm fit
returns a posterior class- assignment entropy of 0.86 and a BIC 170
...
\texttt{output/03-study5-pilot.rds}. Per-replicate timing is 0.05 s for
the linear-mixed baseline and 1.57 s for the class-aware analysis (32:1
cost ratio, consistent with the smoke-test estimate). lcmm convergence
rate is 96\%; the four non-converging replicates encountered numerical
issues during the gridsearch initialisation.
...
standard deviation of the gating coefficient across reps (sd = 63.9) is
roughly fifty times the median within-replicate Wald SE (0.86), and the
empirical sd of the z-statistic is 1.47 against a nominal 1.0. The
gating coefficient is centred at zero (median -0.10, sign balance 46/54)
and shows no systematic bias; the inflation is driven by the conditional
Wald SE under-estimating the true variability of
...
forced to fit \texttt{ng=2}, the optimiser latches onto random patterns
to define a class boundary, and the resulting Wald SE does not absorb
the labelling uncertainty.
...
ch.~6). The bootstrapped LRT (BLRT)
...
\textbf{BIC-conditional gating procedure}: fit ng=1 and ng=2, prefer
ng=2 only if BIC supports it (\(\Delta_{\text{BIC}} > 0\) or, more
...
of replicates, and zero of those reps also reject the gating Wald,
giving the conditional procedure a Type I error of 0 at the pilot's
resolution. The conditional procedure is therefore usable as a primary
moderation test at trial-relevant \(N\). Whether the same conditional
procedure has adequate power to detect a true two-class DGP is the
central question for the rest of Study 5; pilots at non-null cells will
...
registered cell counts (Study 1 has 768 cells × 1,000 replicates =
768,000 fits per analysis arm), the class-aware arm of Study 1 is
approximately 340 hours of serial CPU time at the pilot's mean fit time.
Parallelisation across 8-16 cores reduces this to 22-44 hours per arm.
Studies 2 and 3 require similar budgets; the Study 5 production run
(5,000 replicates × three sample sizes × three class counts × three DGP
families) is the largest single block at approximately 60 hours per arm
on 8 cores. The full programme is feasible on a single workstation over
a week of background compute, with the calibration sub-study and Study 5
pre-flight running first to finalise effect-size axes and
...
baseline. The expected pattern is that the BIC-conditional procedure
outperforms the linear-mixed bm:Dbc test at large class separations
(where ng=2 BIC consistently wins) but underperforms it at moderate
...
lcmm gating Wald unconditional, BIC-conditional gating). Wall time 558
s; convergence 92.9\% under the null and 98.3 to 98.8\% at non-null
cells. Per-replicate output at
\texttt{analysis/data/quick-sim/03-latent-replicates.rds}; Morris
summary at \texttt{03-latent-summary.txt}. Monte Carlo standard error at
...
heterogeneous-random-slopes DGP at \(N = 70\), the BIC step selects ng =
1 in essentially every replicate and the conditional procedure
approximates a never-reject rule. The non-monotonicity (gs = \(1.5\)
below gs = \(1.0\)) suggests that as the heterogeneous-slopes signal
strengthens, a single-class lme captures it well enough that BIC
continues to favor ng = 1 and the conditional procedure does not fire.

The lme \texttt{bm:Dbc} test holds nominal type I (0 of 240 under the
null) and reaches \(0.542\) power at gs = \(1.5\), dominating both lcmm
tests at this sample size and DGP regime. We therefore conclude, on the
...
under-powered. We note that a canonical run at 1,000 replicates per cell
with a wider gating-slope grid (and at \(N \in \{35, 70, 100\}\)) is
required to map the regime in which the BIC-conditional procedure does
become competitive, plausibly at large class separations rather than
under heterogeneous random slopes.
...
(4-core \texttt{mclapply} for the lcmm-bottlenecked run),
...
from the 240-replicate Study 5 pilot are checkpointed at
