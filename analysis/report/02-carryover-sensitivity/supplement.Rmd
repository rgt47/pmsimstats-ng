...
```{r fig-heatmap-sens-s1-g9, echo = FALSE, fig.cap = "Block S1 extended to all nine G1-G9 specifications ($n_{sim} = 500$).", out.width = "80%"}
...
```{r fig-heatmap-sens-s2-g9, echo = FALSE, fig.cap = "Block S2 extended to all nine G1-G9 specifications ($n_{sim} = 500$).", out.width = "95%"}
...
```{r fig-heatmap-sens-s4-g9, echo = FALSE, fig.cap = "Block S4 extended to all nine G1-G9 specifications ($n_{sim} = 500$).", out.width = "80%"}
...
Figures 1-2 (`21-run-tier1-hendrickson-g9-archA.R`, $n_{sim} = 500$;
sanity checks passed: no missing values, power spanning 0.02-0.98,
non-convergence 0-1.6%). Panel C (`G1`-`G3` only, decay-shape axis)
is from a second dedicated 30-cell simulation at the same
$n_{sim} = 500$, matching the more extreme Weibull shape range
($k = 0.25, 0.5, 2.0, 4.0$) the main manuscript's Figure 3 uses
(`24-run-decay-shape-sensitivity-archA.R`), rather than the shared
production grid (`02-grid-summary.rds`, still at the earlier
$k = 0.7, 1.0, 1.5$).

```{r fig-hendrickson-a-archA, echo = FALSE, fig.cap = "Architecture A counterpart to Figure 1: biomarker-effect axis, all nine G1-G9 specifications, power by trial design (columns), $N$ (row blocks), and analysis specification (rows within block), across $c_{bm} \\in \\{0, 0.30, 0.45\\}$ at $t_{1/2} = 1.0$ weeks, exponential DGP, Architecture A ($n_{sim} = 500$).", out.width = "95%"}
knitr::include_graphics("../../figures/02xs-heatmap-hendrickson-a-archA.pdf")
```

```{r fig-hendrickson-b-archA, echo = FALSE, fig.cap = "Architecture A counterpart to Figure 2: carryover-axis, all nine G1-G9 specifications, power by trial design (columns), $N$ (row blocks), and analysis specification (rows within block), across $t_{1/2} \\in \\{0, 0.5, 1.0\\}$ weeks at $c_{bm} = 0.45$, exponential DGP, Architecture A ($n_{sim} = 500$).", out.width = "95%"}
knitr::include_graphics("../../figures/02xs-heatmap-hendrickson-b-archA.pdf")
```

```{r fig-hendrickson-c-archA, echo = FALSE, fig.cap = "Architecture A counterpart to Figure 3: decay-shape axis, power by trial design (columns), $N$ (row blocks), and analysis specification (rows within block), across the five DGP decay-shape levels (exponential, Weibull $k \\in \\{0.25, 0.5, 2.0, 4.0\\}$) at $c_{bm} = 0.45$, $t_{1/2} = 1.0$ weeks, Architecture A ($n_{sim} = 500$).", out.width = "95%"}
...
$0.52$ / `G1` $0.85$ under Architecture B at the same cell). At
...
every specification is near ceiling there (`G1` $0.980$, `G3`
$0.962$, `G4` $0.962$, `G9` $0.964$), a compression that likely
...
($0.936$) edges out `G3`/`G4`/`G9` ($0.894$/$0.860$/$0.864$) by
4 to 8 points. Readers should treat this section as a first look,
