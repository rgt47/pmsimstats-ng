...
carryover (A1); an exposure-weighted continuous indicator that
assumes a parametric decay and half-life (A2, the current default);
and the binary indicator augmented by a lagged just-off-drug term
(A3, the classical crossover device). We embed them in an
...
**Results.** The exposure-weighted specification A2 does not
...
blinded-discontinuation phase, where its advantage over A1 is about
10 percentage points (e.g.\ 0.860 vs 0.766 at the reference cell).
Under the classical crossover design A2 is markedly inferior (0.488
vs 0.830 for A1), and under the mean-moderation architecture (A) it
is marginally dominated throughout; across the grid A2 is the
most powerful specification in only 19% of cells. Where A2 does
lead, the lead is robust to decay-form and half-life
mis-specification. The model-based interaction test is mildly
conservative (mean null rejection 0.039), an artefact of the
...
where A2 with a cluster-robust standard error is recommended, but
...
binary (A1), exposure-weighted (A2), and lagged-augmented (A3) --
across decay forms and both architectures.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item A1 ignores carryover; A2 commits to a parametric decay and
half-life; A3 estimates carryover non-parametrically via a lagged
term.
...
carryover (**A1**, the strict repeated-measures analogue); an
exposure-weighted continuous indicator that commits to a parametric
decay form and half-life (**A2**, the current default); and the
binary indicator augmented by a lagged just-off-drug term that
estimates carryover non-parametrically (**A3**, the classical
...
\item A1, A2, A3 differ only in how $X$ is constructed.
\end{itemize}
:::
...
::: {.bullets data-latex=""}
\textbf{A2 uses a continuous exposure-weighted predictor committing
...
::: {.bullets data-latex=""}
\textbf{A3 adds a lagged just-off-drug term but, because that term
is only a nuisance main effect, it is structurally A1 plus a
control variable.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
...
to A1.
\item A3 therefore differs from A1 only through variance $L$
...
references.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item A2 is effectively a different model for each $\hat{t}_{1/2}$.
\item The mis-specification risk falls entirely on off-drug points.
\item A1 and A3 carry no half-life parameter, so they are invariant
...
A2-vs-A1 can be compared trial by trial (a paired test).
...
This is why we can later compare A2 and A1 with a *paired* test --
...
\item Bias/MSE/coverage are scored against A2's target
$\theta_{\text{true}} = -c_{bm}\sigma_{BR} = -3.6$, but A1/A3
estimate a different coefficient ($\text{bm:}D_{it}$).
\item So A1/A3's apparent bias and under-coverage are an
...
::: {.bullets data-latex=""}
\textbf{The matched-decay baseline shows A2 power declining from
0.988 to 0.860 as carryover grows, reproducing Hendrickson's 0.82.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Matched cell (exp DGP, A2, Architecture B, $N = 70$,
...
grows, but A2's lead is confined to Architecture B's non-crossover
designs.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item All three are near-identical at $t_{1/2} = 0$.
\item A2's advantage emerges at $t_{1/2} = 0.5$ and grows under
Architecture B in the Hybrid and OL+BDC designs; A1 and A3 are
indistinguishable.
\item The advantage does not appear under the CO design (where A2 is
...
specification A2 retains a modest but consistent advantage
...
through $t_{1/2} = 1.0$ weeks; A1 and A3 are effectively
indistinguishable. This advantage is confined to those strata: it
does not appear under the crossover (CO) design, where A2 is in
fact dominated (Section 5), nor under Architecture~A. The figure
therefore shows that the A2 advantage, where present, is not an
artefact of the single reference cell, not that it holds
everywhere.
:::
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Matched reference (exp DGP $\times$ A2): power
$0.860 \pm 0.016$.
\item Same row, other columns: penalty of A1/A3 under an exponential
DGP.
\item A2 column, other rows: cost of assuming exponential when the
...
over-estimated SE; block S6's CR2 restores nominal size without
...
the specification ranking. Table \@ref(tab:tab-type1-cr2) makes the
...
preserving A2's $\approx 0.06$ advantage at every $\rho$.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Power rises monotonically with $\rho$ for all three.
\item A2's advantage holds near 0.06 across $\rho \in [0.5, 0.9]$.
...
::: {.bullets data-latex=""}
\textbf{S2: A2 keeps its advantage even when the assumed half-life
is wrong by a factor of two or four.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item A2 averages 0.868 over eight assumed/true half-life cells
(range 0.766-0.964).
\item A1 and A3 are invariant to the assumed half-life (average
0.840).
\item A2's gentle curvature makes it a defensible default under an
...
narrows A2's advantage as off-drug observations are dropped.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item At 30\% MCAR all three fall to about 0.12-0.15, effectively
tied.
\item The A2$-$A1 gap shrinks from $\approx 0.10$ (no dropout) to
$\approx 0.03$ (30\% MCAR).
\item A2's signal lives in off-drug points, so dropping them erodes
...
::: {.bullets data-latex=""}
\textbf{S4: the power curve is sigmoidal; A2's advantage peaks at
intermediate effect sizes and vanishes toward the ceiling.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Type I error at $c_{bm} = 0$ is conservative (0.026-0.030).
\item A2$-$A1 peaks at $+0.102$ at $c_{bm} = 0.45$, shrinking to
$+0.042$ at $c_{bm} = 0.60$.
...
::: {.bullets data-latex=""}
\textbf{S5: the A2 advantage is constant across the
$\rho \times t_{1/2}$ grid, confirming additive (not interactive)
effects.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item A2$-$A1 $= 0.096$ at both $\rho = 0.5$ and $\rho = 0.8$ when
$t_{1/2} = 1.0$.
...
::: {.bullets data-latex=""}
\textbf{A higher-precision paired-McNemar rerun confirms A2's lead
is real (p $= 6.6 \times 10^{-17}$ at the strongest cell).}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item 24-cell OL+BDC rerun at 600 trials/cell, all converging.
\item Paired McNemar (same trials) at $N = 70$, half-life 1.0: A2
beats A1 by 13.7 points, $p = 6.6 \times 10^{-17}$.
\item A3 recovers only part of A2's advantage; Type I stayed within
$[0.003, 0.053]$.
\end{itemize}
:::
...
half-life 1.0) A2 beats A1 by 13.7 points (0.737 vs 0.873) with
McNemar $p = 6.6 \times 10^{-17}$: the lead is real, not noise. The
shape-free method A3 recovers only part of A2's advantage, and only
...
## Fixing the slightly-conservative test (Block S6)

::: {.bullets data-latex=""}
\textbf{Block S6 re-fits A2 with a CR2 cluster-robust SE to correct
...
fixes the calibration. Table \@ref(tab:tab-s6) re-fits A2 with both
...
the A2 advantage is a comparison among correctly-sized tests.
\end{itemize}
:::
...
where all three specifications hold nominal $\alpha$, the A2
...
\item The A2$-$A1 gap is held or widened, at the cost of far fewer
...
power. Crucially, the A2-minus-A1 gap is held or slightly widened,
...
the crossover design or Architecture~A where A2 does not lead.
:::
...
::: {.bullets data-latex=""}
\textbf{A2's advantage is real but conditional: highest in only
19\% of cells, dominated under CO and under Architecture A.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Best cell (Arch B, Hybrid): A2 beats A1/A3 by $\approx 10$
points ($\approx 5$ MCSEs).
\item A2 is the best method in only 68 of 360 non-null cells; under
CO it is much worse (0.488 vs 0.830).
\item Mechanism: the crossover's within-subject correlation already
carries the signal A2 targets, so A2 only loses there.
\end{itemize}
:::
...
::: {.orig data-latex=""}
The headline is that A2's advantage is real but **conditional**.
At its best cell (Architecture B, Hybrid design, $N = 70$,
$c_{bm} = 0.45$, $t_{1/2} = 1.0$) A2 beats A1 and A3 by about 10
percentage points of power (0.860 vs 0.766 and 0.770) -- about five
MCSEs, so not luck. But A2 is the best method
in only 68 of the 360 cells with a real effect (19%). In the
crossover design it is much *worse* (0.488 vs 0.830 for A1), and
...
A2's decaying predictor is designed to recover, so A2 has nothing
left to gain and its down-weighting of off-drug points only hurts.
The ranking "A2 best" therefore holds only under Architecture B in
...
::: {.bullets data-latex=""}
\textbf{Where A2 leads, the lead survives all three robustness
checks.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Decay shape: A2 never drops below A1/A3 even when the assumed
form is wrong (worst case the linear truth).
\item Assumed half-life: a two- to four-fold error moves A2 only
from 0.83 to 0.77, never below A1/A3.
\item Autocorrelation: lifts all three equally, so A2's 5-6 point
...
method falls to 0.12-0.25 and they become indistinguishable. A2
...
::: {.bullets data-latex=""}
\textbf{A2's advantage exists only under Architecture B, because
only there does carryover erode a correlation for A2 to recover.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Architecture B is more carryover-sensitive under every decay
form and Weibull shape.
\item Under Architecture A, A2 is marginally negative everywhere
(e.g.\ Hybrid, $t_{1/2} = 1.0$: 0.960 vs A1 0.972).
\item Under A the effect stays proportional, so there is no
attenuated correlation to recover and the binary A1 is preferable.
\end{itemize}
:::
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item A2 power 0.860 reproduces Hendrickson's 0.82; the new content
is the A1/A2/A3 comparison.
\item The Jones-Kenward expectation that a shape-free A3 beats A2
...
::: {.orig data-latex=""}
Our matched-decay (A2) power at the headline cell, $0.860 \pm
...
of A1, A2, and A3, which Hendrickson did not do. The crossover
tradition of Jones and Kenward [-@jones2014crossover] predicted
that a shape-free lagged term (A3) should beat a parametric
predictor (A2) when the decay shape is guessed wrong; our S2 result
does not support this in the range we tested, because A2's penalty
for a wrong half-life is too small for A3 to overtake it. Senn's
...
1. **A2 helps, but only conditionally.** The exposure-weighted
   predictor gives the highest power -- about 10 points over A1 --
   *only* under Architecture B in the Hybrid and OL+BDC designs,
   ...
   Architecture A, A2 has no advantage and A1 is at least as good.
   Where A2 is used, report it with a CR2 cluster-robust standard
   error (Section on Block S6).

2. **A guessed half-life is fine (where A2 is used).** Getting the
   half-life wrong by a factor of two to four barely moves A2's
   power, so a rough pharmacokinetic estimate suffices.

3. **The shape-free method A3 is essentially A1.** Because its
   lagged term is only a nuisance control, A3 tracks A1 closely and
   never overtakes A2 in the settings we tested.
