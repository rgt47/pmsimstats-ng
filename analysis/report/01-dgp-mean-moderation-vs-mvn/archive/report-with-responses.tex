...
\usepackage{framed}
\definecolor{addblue}{RGB}{0,80,180}
...
\makeatother
\hypersetup{
  pdftitle={Two Architectures for Simulating Biomarker-Treatment Interactions: Implications for Statistical Power Under Carryover --- with reviewer responses},
  pdfauthor={pmsimstats team},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{Two Architectures for Simulating Biomarker-Treatment Interactions: Implications for Statistical Power Under Carryover --- with reviewer responses}
\author{pmsimstats team}
\date{2026-04-09 (response draft 2026-05-06)}
...
This is a response draft of the manuscript. It carries every word of
the original alongside proposed revisions, so that the reviewer can
see what is added, what is removed, and what is rewritten. The
conventions are:

\begin{itemize}
  \item \textcolor{delred}{\sout{Red strikethrough}} marks text proposed for removal.
  \item \textcolor{addblue}{Blue text} marks text proposed for insertion.
  \item Combinations indicate a replacement: deleted then inserted.
\end{itemize}

A point-by-point response to each reviewer comment is given in the
next section, followed by the manuscript body with tracked changes
applied inline.
...
\textbf{Reviewer:} Rebecca C. Hendrickson (RH), co-author.

We thank Dr.~Hendrickson for the careful and detailed read. The
comments cluster around three themes: framing of the lead, two
specific points of definitional or scope clarification, and a
substantive single observation (Comment\textasciitilde9) which we treat as the
strongest signal in the set. Below we quote each comment, give our
response, and identify the location of the corresponding revision.

\subsection*{C1. Abstract closing --- future directions}

\begin{quote}
\textit{``context.''} --- Add future directions of generalization?
\end{quote}

\textbf{Response.} Agreed. The abstract closes too narrowly on the
two-architecture distinction. We add a closing sentence pointing to
generalization directions taken up in the companion papers.

\textbf{Revision.} Inserted at the end of the abstract (\S,1).

\subsection*{C2. Lead framing --- qualitative/quantitative
interaction division}

\begin{quote}
\textit{``The''} --- Is this actually the division we want to
highlight front and center?
\end{quote}

\textbf{Response.} A fair question. The Rothwell/Kent qualitative-
versus-quantitative taxonomy is the standard heterogeneity-of-
treatment-effect framing in the clinical-trials literature, and
grounding the paper in it gives readers a familiar entry point. Our
framing is not that we are \textit{replacing} this taxonomy but
that we are introducing an \textit{orthogonal} axis (DGP
architecture) which sits one layer below the analysis-model
taxonomy that PATH and the Rothwell tradition operate within. We
propose to keep the paragraph and add a sentence making this
orthogonality explicit at the point of first contact.

\textbf{Revision.} One-sentence orthogonality clarification added
after the qualitative/quantitative paragraph in \S,1.

\subsection*{C3. ``Not a research-program convention but a
paper-specific design choice''}

\begin{quote}
Would skip this part of framing.
\end{quote}

\textbf{Response.} Agreed; the phrasing is more pointed than
necessary. We strike the editorialization and retain only the
substantive observation that other simulation work uses
Architecture\textasciitilde A.

\textbf{Revision.} Strikethrough in \S,1; replacement phrasing in
blue.

\subsection*{C4. Origin story for the architectural distinction}

\begin{quote}
Would seem more straightforward to simply explain that the impact
of carryover on power was greater than expected in this
implementation versus other similar projects, and an exploration of
why identified the dynamic explored here.
\end{quote}

\textbf{Response.} Agreed. The current ``comprehensive audit
revealed'\,' narrative reads as detective story; the reviewer's
framing is direct and honest. We replace the paragraph.

\textbf{Revision.} Strikethrough of the audit-narrative paragraph
in \S,1; replacement paragraph in blue.

\subsection*{C5. Treatment of a hybrid architecture}

\begin{quote}
Question of whether we 1)~implement and include in comparisons the
hybrid model, 2)~identify the hybrid as future area of work, or
3)~only note in passing.
\end{quote}

\textbf{Response.} We propose option (2) for the present paper,
with a forward reference to the latent-class-mixture-application
companion manuscript scaffolded in the pmsimstats-ng repository.
Implementing a hybrid mixture DGP and characterizing its
identifiability under the N-of-1 design family is a substantial
addition that risks diluting the present paper's narrower message
about the architecture choice. The companion paper takes it up
properly.

\textbf{Revision.} Sentence added at the end of \S,1 introducing
the companion paper as the future-work locus for the hybrid
architecture.

\subsection*{C6. Notation --- ``biomarker''}

\begin{quote}
Baseline biomarker value, I assume?
\end{quote}

\textbf{Response.} Yes. We clarify in the \S,2 Notation paragraph.

\textbf{Revision.} Inline replacement of \texttt{biomarker\ value\textquotesingle{}\textquotesingle{}\ with}baseline biomarker value, measured at trial entry'\,'.

\subsection*{C7. Default exponential-decay form --- why?}

\begin{quote}
Dumb question, but is this just the default because we wrote it
that way? Is this what you added as part of fixing the positive
definite problem? Is it actually structurally required, or could
this decision be dissociated?
\end{quote}

\textbf{Response.} Not a dumb question; it deserves an explicit
answer. The exponential decay form is motivated by first-order
pharmacokinetics (so that
\(\lambda = \ln 2 / t_{1/2}\) ties cleanly to the drug elimination
rate), not by positive-definiteness considerations. It does ensure
the BM-BR correlation never crosses zero during washout, which
helps PD in some regions of the parameter space but is not strictly
required. Linear and Weibull decay forms are implemented in the
tidyverse collection and are explored empirically in the
carryover-sensitivity manuscript.

\textbf{Revision.} Footnote added in \S,2.2 stating the
pharmacokinetic motivation and pointing to the alternative-form
sensitivity work.

\subsection*{C8. ``Carryover effects in the DGP inflate the BR
means during off-drug periods''}

\begin{quote}
The way this is written I don't understand how it's specific to
this model --- isn't this what carryover of a drug effect always is?
\end{quote}

\textbf{Response.} Correct: this property is shared by both
architectures and so does not by itself discriminate them. We
revise the bullet to make the architecture-specific consequence
explicit, namely the second-moment consequence of the mean
inflation: correlation-structure compression.

\textbf{Revision.} Bullet rewritten in \S,2.2.

\subsection*{C9. ``the noise structure is independent of drug
state''}

\begin{quote}
This makes the most sense to me as a clear concrete difference ---
how much is lost if I think of this as the central difference?
\end{quote}

\textbf{Response.} This is the strongest single comment in the set.
We agree that ``noise structure is independent of drug state'\,' is
the cleanest one-sentence characterization of the architectural
difference. We add a paragraph at the end of \S,3.2 making this
explicit and stating that a reader who keeps only this distinction
loses very little of the substance of the paper. We do not promote
it to the abstract or section title in the present revision because
that is a larger restructuring decision; it is offered here as the
natural takeaway of \S,3.2.

\textbf{Revision.} Paragraph added at end of \S,3.2.

\subsection*{C10. Two-stage random slopes --- for our data?}

\begin{quote}
Should we be considering this for our data?
\end{quote}

\textbf{Response.} Yes, plausibly. The feasibility hinges on
whether the prazosin-PTSD data provide enough on-drug observations
per participant to identify a participant-specific drug effect (the
random slope \(u_{1i}\)). The OL+BDC participants do; the
parallel-arm participants do not. We add a methods note flagging
this as a candidate analysis and identifying the data-feasibility
constraint.

\textbf{Revision.} Sentence added at end of \S,6.4.
...
selecting the appropriate DGP for a given clinical context. \textcolor{addblue}{Generalizations to latent-class mixture formulations, copula-based joint models, and richer carryover-decay families remain open methodological directions; some are taken up in companion papers in the pmsimstats-ng project.}
...
this heterogeneity. \textcolor{addblue}{Note that the qualitative-versus-quantitative interaction distinction operates at the analysis-model layer, classifying the shape of the moderating relationship after a model has been fit. The architectural distinction introduced in this paper is orthogonal to it: the same qualitative interaction can be generated by either Architecture~A or Architecture~B, and the practical consequences for power estimation under carryover differ between architectures regardless of where the result lands in the qualitative/quantitative taxonomy.}
...
differential-correlation parameterization \textcolor{delred}{\sout{is not a research-program convention but a paper-specific design choice in Hendrickson et al. (2020). To the authors' knowledge}},\textcolor{addblue}{is uncommon. To the authors' knowledge}, no other clinical trial
...
\textcolor{delred}{\sout{Both approaches were encountered in the development of the pmsimstats simulation framework for N-of-1 clinical trials (Hendrickson et al. 2020). The original publication code uses the MVN differential correlation approach, an unusual choice that, as shown here, has substantive consequences for power estimation under carryover. A comprehensive audit revealed that a parallel tidyverse reimplementation of the same simulation had inadvertently adopted the standard direct mean moderation approach. Both implementations produced plausible power estimates, but they yielded qualitatively different conclusions about how carryover effects influence statistical power. Tracing this discrepancy to its root cause revealed the architectural difference described in this paper, which has not, to the authors' knowledge, been explicitly discussed in the simulation methodology literature.}}

\textcolor{addblue}{In our application of the Hendrickson framework to the prazosin-PTSD reanalysis, the impact of carryover on power was substantially greater than the impact reported by comparable simulation studies of N-of-1 designs. Tracing this discrepancy to its source revealed an architectural difference in how the biomarker-treatment interaction is encoded in the data-generating process. The two simulation frameworks were producing internally consistent power estimates from two different data-generating processes that had not been distinguished in the literature. This paper makes the distinction explicit and characterizes its consequences for power estimation under carryover.}
...
\textcolor{addblue}{A hybrid architecture combining elements of A and B (latent-subtype membership generating Architecture B-like covariance structure within classes whose means are governed by Architecture A-style moderation) is conceptually appealing and is treated separately in the latent-class-mixture-application companion manuscript scaffolded in this repository. We do not attempt that here.}
...
\textcolor{delred}{\sout{biomarker value of participant $i$}},\textcolor{addblue}{baseline biomarker value of participant $i$, measured at trial entry}, \(D_{it}\) the binary on-drug
...
exponential-decay form is the default; \textcolor{addblue}{the choice is motivated by first-order pharmacokinetics rather than by positive-definiteness considerations: with $\lambda = \ln 2 / t_{1/2}$ the decay matches the drug's elimination rate, giving the model parameter a transparent biological interpretation. Linear and Weibull decay forms are implemented in \texttt{implementations/tidyverse/} and explored empirically in the carryover-sensitivity manuscript.} the limiting cases
...
\item \textcolor{delred}{\sout{Carryover effects in the DGP inflate the BR means during off-drug periods, making them resemble on-drug means. This reduces the \textbf{differential} in the correlation structure between treatment states, weakening the signal that the analysis model detects.}}\,\textcolor{addblue}{Carryover inflates BR means during off-drug periods, a property shared by both architectures. What is specific to Architecture~B is the second-moment consequence of this mean inflation: by shrinking the contrast between on-drug and off-drug states, carryover compresses the treatment-state-dependent differential in the BM-BR correlation that defines the architecture. The signal-bearing component of the joint distribution is therefore weakened even though the mean structure remains intact.}
\end{itemize}
...
every level of exposure. The biomarker-treatment signal
...
Architecture\textasciitilde B embeds it in the off-diagonal \(c_{bm}\) entries of
\(\Sigma\), whereas Architecture\textasciitilde A omits those entries and adds a
...
\textcolor{addblue}{Cleanest one-sentence characterization: under Architecture~A the noise structure is independent of drug state and the biomarker-driven heterogeneity lives in the mean; under Architecture~B the noise structure depends on drug state, and the entire interaction signal lives in this dependency. A reader who keeps only this distinction will lose little of the substance of the present paper. Whether the headline framing of the architecture comparison should be reorganized around this single statement is a larger restructuring question we leave open for the journal-format distillation.}
...
consequence is that Architecture\textasciitilde B as implemented here captures
...
biomarker-treatment interaction, and that even modest
...
\textcolor{addblue}{For the prazosin-PTSD reanalysis, the two-stage approach is a candidate analysis. Its feasibility hinges on whether the trial provides enough on-drug observations per participant to identify a participant-specific drug effect ($u_{1i}$): the OL+BDC participants in the dataset do, but the parallel-arm participants do not. Implementing the two-stage estimator would therefore restrict the analysis sample to the OL+BDC subgroup or require a hybrid scheme that combines OL+BDC random slopes with a parallel-arm fixed-effects estimate.}
...
interacts with biomarker-treatment effect signals. The
...
randomized controlled trials: importance, indications, and
...
personalized medicine. \textit{Stat Med}. 2016;35(7):966-977.
