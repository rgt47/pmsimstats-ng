...
(Architecture~A), in which the biomarker scales the treatment
effect in the conditional mean, and differential correlation
(Architecture~B, the multivariate-normal approach), in which the
...
\textbf{Methods.} We define Architecture~C (combined), which
carries independent parameters $c_{bm,A}$ (mean channel) and
$c_{bm,B}$ (covariance channel), and which reduces algebraically to
pure Architecture~A when $c_{bm,B} = 0$ and to pure
Architecture~B when $c_{bm,A} = 0$. We show analytically that the
biomarker-by-treatment signal delivered to the fixed analysis model
...
::: {.bullets data-latex=""}
\textbf{The companion paper contrasts two DGP architectures; this
paper treats the case where both operate simultaneously.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Architecture A encodes the interaction in the mean;
  Architecture B encodes it in the covariance.
\item A biomarker could act through both pathways at once.
\item Architecture C carries independent weights on the two channels
...
power loss under carryover. Under Architecture~A the biomarker
scales the treatment effect in the conditional mean; under
Architecture~B the interaction is carried by
...
data-generating model Architecture~C (combined). It is not a third
...
argument identifies the additivity of the biomarker-by-treatment
...
::: {.bullets data-latex=""}
\textbf{Architecture C applies both interaction mechanisms in
sequence: a differential-correlation MVN draw, then an on-drug mean
shift.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item The MVN covariance encodes BM-BR differential correlation with
  weight $c_{bm,B}$ (the Architecture B channel).
...
Architecture~B: the on-drug correlation is $c_{bm,B}$, decaying to
...
pure Architecture~A; setting $c_{bm,A} = 0$ removes the mean-shift
channel and recovers pure Architecture~B. This combined form is the
mean-and-covariance-structure model of Arminger et al.
[-@arminger1999], the general psychometric parameterization within
which Architectures~A and~B are constrained submodels; a companion
...
Architecture~C: a correctly implemented combined driver must
...
(Architecture~B) reflects a different process: on-drug periods
...
correlation structure that Architecture~B encodes in the covariance
...
::: {.bullets data-latex=""}
\textbf{The biomarker-by-treatment signal delivered to the analysis
...
channel. Under the covariance channel alone (Architecture~B), the
on-drug joint distribution sets
\[
\mathrm{Cov}(B,\, BR \mid \text{on-drug})
  = c_{bm,B}\, \sigma_B \sigma_{BR}.
\]
Under the mean channel alone (Architecture~A), the on-drug shift
...
\]
The biomarker-by-treatment signal delivered to the analysis model
...
\item A2 analysis specification; 1000 replicates per cell.
\end{itemize}
:::
...
the A2 analysis specification. The channel-weight panel is
...
\item The $c_{bm,B} = 0$ column must reproduce pure Architecture A;
  the $c_{bm,A} = 0$ row must reproduce pure Architecture B.
...
companion paper's Architecture~A and Architecture~B baselines, and
...
\begin{table}[h]
\small\centering
\caption{Architecture C empirical power: $3 \times 3$ channel-weight
panel at carryover $t_{1/2} = 1.0$ week, A2 analysis specification,
1000 replicates per cell. Rows index $c_{bm,A}$ (mean channel);
columns index $c_{bm,B}$ (covariance channel). Boundary column
$c_{bm,B} = 0$ corresponds to pure Architecture A; boundary row
$c_{bm,A} = 0$ corresponds to pure Architecture B. Values are at the
...
distinguish the channels. Architecture~C should not be the default
...
response. The recommendation against Architecture~C as a default is
...
signaling-pathway interpretation). We regard Architecture~C as the
...
the biomarker-by-treatment signal and the convexity of the power
