...
<!--
  ...
  the linear DGP decay form, and drops the A1 (binary on-drug)
  analysis specification, keeping only A2 (exposure-weighted) and
  A3 (lagged-treatment). See report.Rmd / report-slim.Rmd for the
  ...
  1-3. Tier 1 and Tier 2 production data and summarisation are
  ...
  4. Render the filtered Tier 1 figures (Architecture B, no linear
     decay, A2/A3 only; writes analysis/figures/02xs-*.pdf):
...
**Results.** A2 attains the highest power only in the two designs
...
conservative (mean null rejection 0.039), an artefact of the working
...
exposure-weighted (A2) and lagged-treatment (A3) -- across decay
forms under Architecture B.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item A2 commits to a parametric decay and half-life; A3 estimates
...
commits to a parametric decay form and half-life (**A2**, the
...
(**A3**, the classical crossover device of Jones and Kenward
...
::: {.bullets data-latex=""}
\textbf{The aim is to identify, under Architecture B, which of A2 and
A3 is the more robust carryover-mitigation strategy across DGP decay
forms and trial designs.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Quantify the power cost of analysis-model carryover
mis-specification under Architecture B.
\item Identify which of A2 and A3 is more robust across the DGP
...
carryover-mitigation strategies, A2 or A3, is more robust across the
...
\item Sensitivity of A2 to assumed-versus-true mismatch is evaluated
in Tier 2 block S2.
\end{itemize}
:::
...
formula is unchanged for any $\hat{t}_{1/2}$, so A2 is effectively a
...
::: {.bullets data-latex=""}
\textbf{A3 augments the binary indicator with a lagged-treatment term
...
under A3 remains $\text{bm:}D_{it}$, the same coefficient as under
the binary on-drug specification (A1, not evaluated in this
manuscript). A3 is best understood as A1 with a carryover nuisance
control rather than as a fundamentally different interaction model;
unlike A2, it uses no half-life parameter and is therefore invariant
...
A1 shrinks the grid to 216 cells, one-tenth of the full-scope
manuscript's 540.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
...
\item Common random numbers across A2 and A3 within each cell enable
paired contrasts.
\item Data are the same production run summarised in
\texttt{analysis/data/02-grid-summary.rds} (540 cells), filtered here
to Architecture B, non-linear decay forms, and A2/A3.
\end{itemize}
:::
...
and the A2/A3 specifications. Within each cell, the same simulated
dataset is used to fit both remaining specifications (common random
numbers), so the A2$-$A3 contrast is estimated with reduced Monte
...
Architecture B, exponential/Weibull decay, and A2/A3.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
...
and coverage are calibrated to A2's target and are not directly
comparable to A3's.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
...
$\theta_{\text{true}} = -c_{bm}\sigma_{BR} = -3.6$; A3 estimates a
different coefficient ($\text{bm:}D_{it}$).
\item A3's apparent bias and under-coverage reflect an estimand
...
```{r fig-power-by-spec, echo = FALSE, fig.cap = "Power versus carryover half-life under the exponential DGP, Architecture B, stratified by analysis specification. A2 retains a modest advantage over A3 in the Hybrid and OL+BDC designs at $t_{1/2} = 1.0$ weeks, but is dominated by A3 under the crossover (CO) design.", out.width = "95%"}
knitr::include_graphics("../../figures/02xs-power-by-spec.pdf")
```

::: {.bullets data-latex=""}
\textbf{The two specifications coincide at zero carryover and diverge
as it grows, but A2's lead is confined to the two non-crossover
designs.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Both are near-identical at $t_{1/2} = 0$.
\item A2's advantage emerges at $t_{1/2} = 0.5$ and grows in the
Hybrid and OL+BDC designs.
\item Under the CO design A2 is dominated by A3 (Section 5).
\end{itemize}
:::
...
progressively as carryover half-life increases. A2 retains a modest
...
crossover (CO) design, where A2 is in fact dominated (Section 5): in
a crossover the within-subject correlation already carries most of
the signal A2's decaying predictor is designed to recover, so A2 has
...
$0.860 \pm 0.016$.
\item Same row, A3 column: penalty of the lagged specification under
an exponential DGP.
...
the A2 analysis column is the matched reference
(power $0.860 \pm 0.016$). The same row's A3 column quantifies the
power penalty of the lagged specification when the DGP is
exponential; the A2 column's other rows quantify the cost of
assuming exponential decay in the analysis when the true DGP is
Weibull. Even under the heaviest-tailed Weibull DGP ($k = 0.7$), A2's
advantage over A3 narrows but does not reverse; the Discussion
...
lab <- c('Reference, $N=70$', 'Small $N=35$',
         ...
         '30\\% MCAR dropout')
## The pre-computed s6$gaps object is the A2-A1 gap inherited from
## the full-scope (report.Rmd) diagnostic run; this manuscript's
## comparison is A2 vs A3, so the A2-A3 gap is computed directly
...
tab <- s6$summary |>
  ...
  transmute(
    ...
    `A2$-$A3 model`   = sprintf('%+.3f', gap_mod_a3),
    `A2$-$A3 CR2`     = sprintf('%+.3f', gap_cr2_a3),
    `CR2 df`          = sprintf('%.0f', cr2_df))
knitr::kable(tab, booktabs = TRUE, escape = FALSE,
  caption = 'Cluster-robust recalibration of A2 (Power columns) and
...
points; the A2-vs-A3 ranking is held or widened at four of five S6
...
for A2; A3's calibration ratio is comparable, Table
\@ref(tab:tab-type1-cr2)).
\item The A2$-$A3 gap widens under CR2 at the reference, small-$N$,
...
\@ref(tab:tab-s6) reports the A2$-$A3 gap -- this manuscript's own
comparison, computed directly from the per-specification power at
each S6 cell rather than reused from the A2-vs-A1 contrast reported
...
($+0.052 \to +0.036$, both favoring A2); and at 30% MCAR dropout it
collapses from a small A2 lead under the model-based test ($+0.014$)
...
as a restatement of the dropout finding in block S3 below: A2's
...
the crossover design, where A2 does not lead.
:::
...
$\rho = 0.5$, A2 power is $0.798 \pm 0.018$ versus A3
$0.738 \pm 0.020$; at $\rho = 0.9$, A2 reaches $0.956 \pm 0.009$
versus A3 $0.910 \pm 0.013$ (point estimate $\pm$ MCSE,
$n_{sim} = 500$). The A2 advantage over A3, approximately 0.06 in
...
```{r fig-sens-s2, echo = FALSE, fig.cap = "Power of A2 and A3 as the analyst's assumed half-life departs from the true DGP half-life. A3 is invariant to the assumed half-life by construction; A2's power degrades with mis-specification.", out.width = "80%"}
...
(range 0.766-0.964).
\item A3 is invariant to the assumed half-life (average 0.840).
...
::: {.orig data-latex=""}
A2 is markedly robust to analyst-truth half-life mismatch. Across the
...
from 0.008 to 0.019. A3 is invariant to the analyst-assumed half-life
...
of the assumed half-life by a factor of two does not erode A2's
advantage over A3 at either DGP half-life, suggesting A2 remains
...
A2's advantage as off-drug observations are dropped.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item At 30\% MCAR both fall to about 0.12-0.15, effectively tied.
\item The A2$-$A3 gap shrinks from $\approx 0.10$ (no dropout) to
$\approx 0.03$ (30\% MCAR).
...
MCAR baseline ($N = 70$, dropout = 0) gives A2 $0.866 \pm 0.015$,
A3 $0.764 \pm 0.019$; at MCAR rate 0.30 these collapse to A2
$0.148 \pm 0.016$, A3 $0.124 \pm 0.015$. A2's advantage narrows under
heavy dropout: the absolute A2$-$A3 gap is approximately 0.10 at
...
($\approx 0.004$ at rate 0.30). The pattern is consistent with A2's
...
dropped, A2's advantage contracts toward A3's. At low-to-moderate
dropout (rate $\leq 0.20$) the A2 advantage is preserved.
:::
...
\item A2$-$A3 peaks at $+0.102$ at $c_{bm} = 0.45$, shrinking to
$+0.042$ at $c_{bm} = 0.60$.
...
nominal at $0.028 \pm 0.007$ (A2) and $0.030 \pm 0.008$ (A3); power
rises through the mid-range and saturates at $c_{bm} \geq 0.45$: at
$c_{bm} = 0.6$, A2 reaches $0.958 \pm 0.009$ and A3
$0.916 \pm 0.012$. The A2 advantage is largest at intermediate effect
...
::: {.bullets data-latex=""}
\textbf{A high-precision paired-McNemar rerun finds A2 strictly ahead
of A3 only at the highest-leverage cell ($+7.2$ points,
$p = 1.2 \times 10^{-7}$); elsewhere the two are operationally tied.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item 24-cell OL+BDC rerun at 600 trials/cell, all converging.
\item Paired McNemar (same trials): A2 beats A3 by 7.2 points at
...
because A2 and A3 see the *same* trials, they can be compared with a
...
McNemar on A2 versus A3 finds A2 strictly dominant only at the
...
the A2-versus-A3 gap at the intermediate cells with statistical
...
::: {.bullets data-latex=""}
\textbf{A2's advantage over A3 is real but conditional: confined to
the Hybrid and OL+BDC designs, and reversed under the crossover
design.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Best cell (Hybrid): A2 beats A3 by $\approx 10$ points
($\approx 5$ MCSEs).
\item Under CO, A2 is much worse (0.488 vs 0.830 for A3).
...
::: {.orig data-latex=""}
The headline is that A2's advantage over A3 is real but
**conditional**. At its best cell (Hybrid design, $N = 70$,
$c_{bm} = 0.45$, $t_{1/2} = 1.0$) A2 beats A3 by about 10 percentage
points of power (0.860 vs 0.770) -- about five MCSEs, so not luck.
In the crossover design it is much *worse* (0.488 vs 0.830 for A3).
...
The ranking "A2 best" therefore holds only in the two non-crossover
...
checks.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Decay shape: A2 never drops below A3 even when the assumed
form is wrong, up to the heaviest-tailed Weibull.
\item Assumed half-life: a two- to four-fold error moves A2 only
from 0.83 to 0.77, never below A3.
\item Autocorrelation: lifts both equally, so A2's 5-6 point lead is
$\rho$-independent.
\end{itemize}
:::
...
A2 still does not drop below A3, within the decay-shape range tested
here. **Assumed half-life (Fig. \@ref(fig:fig-sens-s2)).** A3 does
not use a half-life, so it is unaffected; A2 does, yet getting the
half-life wrong by a factor of two or four moves its power only from
about 0.83 to 0.77, never below A3. **Autocorrelation
(Fig. \@ref(fig:fig-sens-s1)).** Stronger serial correlation raises
both methods by about the same amount, so A2's lead stays near 5-6
...
become indistinguishable. A2 cannot protect against the loss; it only
...
more power than the choice between A2 and A3.
:::
...
specification A1, or linear decay without consulting the full-scope
manuscript.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Full grid (report.Rmd / report-slim.Rmd) shows A2's advantage
does not extend to Architecture A, where A2 is marginally dominated
throughout.
\item A2 power 0.860 reproduces Hendrickson's 0.82; the Jones-Kenward
expectation that shape-free A3 beats A2 under mis-specification is
not supported here.
...
Architecture B, non-linear decay, and the A2/A3 contrast, to bring a
...
the A2 advantage documented here does not extend to Architecture A
(direct mean moderation), where A2 is marginally dominated in every
design, nor is the binary on-drug specification A1 dominated by A2
outside the Hybrid/OL+BDC-under-Architecture-B cells studied here;
readers who need the cross-architecture or A1 comparison should
consult that manuscript rather than extrapolate from this one. Within
the narrower scope, our matched-decay (A2) power at the headline
...
a shape-free lagged term (A3) should beat a parametric predictor (A2)
when the decay shape is guessed wrong; our S2 result does not support
this in the range we tested, because A2's penalty for a wrong
half-life is too small for A3 to overtake it. Two further
...
1. **A2 helps, but only conditionally.** The exposure-weighted
   predictor gives the highest power -- about 10 points over A3 --
   *only* in the Hybrid and OL+BDC designs, and the lead survives
   wrong decay-shape and wrong half-life assumptions there. Under
   the crossover design A2 has no advantage and A3 is at least as
   good. Where A2 is used, report it with a CR2 cluster-robust
...
4. **The test is slightly conservative, and fixable.** Both methods
   ...
   recovering a few points of power. The A2-vs-A3 gap is held or
...
5. **This is a narrowed slice.** Architecture A, the binary
   specification A1, and linear decay are outside this manuscript's
