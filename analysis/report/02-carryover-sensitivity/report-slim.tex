...
target is the biomarker-by-treatment interaction, and a recurring
...
has not been characterised. This paper is written as a tutorial:
we introduce the design and the modelling choices from first
...
crossing three data-generating decay forms (linear, exponential,
Weibull) with the three specifications, under two biomarker
data-generating architectures (mean moderation and
multivariate-normal differential correlation), three designs, two
...
\textbf{Results.} The exposure-weighted specification A2 does not
...
inference.
\item The estimand is the biomarker-by-treatment interaction:
whether a baseline covariate $B_i$ modifies the treatment effect.
\item Four designs (CO, OL+BDC, Hybrid, pure open-label) differ in
how on- and off-drug occasions are arranged.
\end{itemize}
...
N-of-1 setting, so we set out the design before the modelling. In a
...
estimand of interest is the biomarker-by-treatment interaction,
...
occasions are arranged: a traditional crossover (CO), an open-label
...
proportionality survives washout.
...
specification, leaving the joint sensitivity uncharacterised.
\end{itemize}
...
proportionality survives washout, and carryover costs only 1-3\% of
...
carryover (\textbf{A1}, the strict repeated-measures analogue); an
exposure-weighted continuous indicator that commits to a parametric
decay form and half-life (\textbf{A2}, the current default); and the
binary indicator augmented by a lagged just-off-drug term that
estimates carryover non-parametrically (\textbf{A3}, the classical
...
measure accompanied by its Monte Carlo standard error. Section 3
...
companion manuscript. The companion also formalises a combined
Architecture C, which activates both the mean-moderation and
differential-correlation channels simultaneously; Architecture C
...
when only a complete-washout time is known.}
...
interaction, realised by the $c_{bm} = 0$ grid row.}
...
\item Models no residual drug effect during washout.
...
\textbf{$n_{sim} = 500$ keeps the MC SE on power below 0.022; the
50-replicate scale is for pipeline validation only.}

\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Production: $n_{sim} = 500$, MC SE $\leq 0.022$ at $p = 0.5$.
\item Development: $n_{sim} = 50$, MC SE $\approx 0.07$.
...
This is why we can later compare A2 and A1 with a \emph{paired} test --
...
\textbf{All results come from the 540-cell production run; MC SE on
power is at most 0.022, so smaller differences are noise.}

\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item 500 replicates per cell; MC SE $\leq \sqrt{0.25/500} \approx
...
narrows A2's advantage as off-drug observations are censored.}
...
$\approx 0.03$ (30\% MCAR).
\item A2's signal lives in off-drug points, so censoring them erodes
...
\subsection{Fixing the slightly-conservative test (Block S6)}\label{fixing-the-slightly-conservative-test-block-s6}
...
the crossover design or Architecture\textasciitilde A where A2 does not lead.
...
\begin{orig}
The headline is that A2's advantage is real but \textbf{conditional}.
...
Monte Carlo standard errors, so not luck. But A2 is the best method
in only 68 of the 360 cells with a real effect (19\%). In the
crossover design it is much \emph{worse} (0.488 vs 0.830 for A1), and
...
cannot protect against the loss; it only organises whatever data
...
for a wrong half-life is too small for A3 to overtake it. Senn's\textsuperscript{\citeproc{ref-senn2016mastering}{6}} variance-component reasoning is consistent
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Numbers come from one prazosin/PTSD parameterisation, so
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{A2 helps, but only conditionally.} The exposure-weighted
...
\item
  \textbf{A guessed half-life is fine (where A2 is used).} Getting the
  half-life wrong by a factor of two to four barely moves A2's
  power, so a rough pharmacokinetic estimate suffices.
\item
  \textbf{The shape-free method A3 is essentially A1.} Because its
...
\emph{Rendered on 2026-06-18 at 08:09 PDT.}\\
