## quick-sim-prototype.R
##
## Small-scale prototype simulation for paper 02-carryover-sensitivity.
## Compares three analysis-side carryover specifications under a fixed
## exponential-decay DGP carryover (t1/2_DGP = 1.0 wk).
##
## Specifications:
##   A1: analysis t1/2 = 0     (binary on/off Db)
##   A2: analysis t1/2 = 1.0   (exposure-weighted continuous Dbc, matched)
##   A3: simplecarryover=TRUE  (lagged-treatment via tsd as covariate)
##
## NOTE: The user prompt described the trial design as 'hybrid OL+BDC'.
## The literal code snippet provided would build a pure OL design, in
## which every participant is on drug at every timepoint. Under pure OL,
## tsd = 0 throughout and Db is constant at 1, so all three analysis
## specifications collapse onto an identical bm:t fallback model and
## the comparison is degenerate. This driver therefore uses the OL+BDC
## preset from analysis/scripts/carryover-sensitivity/simulation-core.R,
## which is what 'hybrid OL+BDC' refers to in this codebase. See the
## report at the end of this script for elaboration.
...
## discontinuation), N = 35 split across two paths.
## --------------------------------------------------------------
...
## --------------------------------------------------------------
## Helper: simulate one trial dataset for a given c.bm / seed
## --------------------------------------------------------------
...
## --------------------------------------------------------------
## Main simulation loop with wall-clock guard
## --------------------------------------------------------------
...
for (cb in c_bm_levels) {
  ...
  for (rep_idx in seq_len(n_reps_target)) {
    ...
    seed_i <- 1000L * which(c_bm_levels == cb) + rep_idx
    dat <- tryCatch(
...
## --------------------------------------------------------------
## Figure: power vs analysis spec, faceted by c.bm
## --------------------------------------------------------------
