...
\setstretch{1.2}
\renewcommand{\rmdsource}{Source: \texttt{\~/Dropbox/prj/alz/10-pmsimstats-ng/pmsimstats-ng/analysis/report/05-nof1-design-sensitivity/report.Rmd}}
\renewcommand{\rendertimestamp}{Rendered: 2026-05-08 08:36 PDT}
...
Morris et al.\textsuperscript{\citeproc{ref-morris2019using}{1}}.
...
machinery to support population-level inference\textsuperscript{\citeproc{ref-lillieNof1ClinicalTrial2011}{2},\citeproc{ref-kravitzSinglepatientNof1Trials2014}{3}}. Several recent simulation
...
literature: effect size\textsuperscript{\citeproc{ref-blackstonComparisonAggregatedNof12019}{4}},
carryover half-life\textsuperscript{\citeproc{ref-blackstonComparisonAggregatedNof12019}{4},\citeproc{ref-hendricksonOptimizing2020}{5}}, decay-form
misspecification\textsuperscript{\citeproc{ref-gaertnerCarryover2023}{7}}, between-patient
heterogeneity\textsuperscript{\citeproc{ref-sennSampleSizeConsiderations2019}{8},\citeproc{ref-zuckerIndividualTrialsPopulation2010}{9}}, within-patient variance\textsuperscript{\citeproc{ref-wangPowerDesignIssues2019}{10}}, cycles per patient\textsuperscript{\citeproc{ref-sennSampleSizeConsiderations2019}{8}}, total participant count\textsuperscript{\citeproc{ref-blackstonComparisonAggregatedNof12019}{4},\citeproc{ref-sennSampleSizeConsiderations2019}{8}}, AR(1) serial correlation\textsuperscript{\citeproc{ref-wangPowerDesignIssues2019}{10}}, period length\textsuperscript{\citeproc{ref-wangPowerDesignIssues2019}{10}}, biomarker interaction strength
...
Monte Carlo standard errors below 1\% per Morris et al.\textsuperscript{\citeproc{ref-morris2019using}{1}}.
...
\item
  \textbf{Architecture A (\texttt{dgp\_architecture\ =\ "mean\_moderation"}).}
...
profiles under carryover, as reported in\textsuperscript{\citeproc{ref-hendricksonOptimizing2020}{5}}. The two architectures are crossed
...
random intercept per participant, and \texttt{corCAR1(form\ =\ \textasciitilde{}t\ \textbar{}\ ptID)}
...
\item
  ...
  baseline-symptom SD; precedents Senn 2018\textsuperscript{\citeproc{ref-sennSampleSizeConsiderations2019}{8}} and Zucker 2010\textsuperscript{\citeproc{ref-zuckerIndividualTrialsPopulation2010}{9}}.
\item
  \textbf{S5} varies the within-patient bio-response SD; precedent
  Wang and Schork 2019\textsuperscript{\citeproc{ref-wangPowerDesignIssues2019}{10}}.
\item
...
\item
  ...
  and Schork 2019\textsuperscript{\citeproc{ref-wangPowerDesignIssues2019}{10}}.
\item
...
target coefficient is the \textbf{main drug effect} (the \texttt{Dbc}
coefficient in the \texttt{nlme::lme} fixed-effects formula).

The \textbf{primary N-of-1 design} is the Hendrickson 2020\textsuperscript{\citeproc{ref-hendricksonOptimizing2020}{5}} hybrid (open-label titration
...
Blackston 2019\textsuperscript{\citeproc{ref-blackstonComparisonAggregatedNof12019}{4}}, whose
...
2023\textsuperscript{\citeproc{ref-gaertnerCarryover2023}{7}}: misspecification of the
...
In the Blackston 2019\textsuperscript{\citeproc{ref-blackstonComparisonAggregatedNof12019}{4}}
framing, the hybrid achieves an equivalent power target at
approximately one-third to one-half the sample size of the
parallel RCT. This is directly comparable to the Carrozzo 2025\textsuperscript{\citeproc{ref-carrozzoTwoArm2025}{6}} result that the meta-analytic N-of-1
...
and the \texttt{corCAR1} residual structure absorbs the correlation;
...
when \texttt{corCAR1} is fitted, because the fit cannot manufacture
the missing within-subject contrasts. This matches Wang and
Schork 2019\textsuperscript{\citeproc{ref-wangPowerDesignIssues2019}{10}}: under correct
...
\textbf{increases} monotonically with period length: at period =
...
interaction-target recommendation: \textbf{longer periods are
preferred when the target is the main drug effect}, subject to
carryover not exceeding the period length by more than
approximately 50\%. Wang and Schork 2019\textsuperscript{\citeproc{ref-wangPowerDesignIssues2019}{10}} note this tradeoff explicitly,
...
high-AR(1) cells are \textbf{inflated above nominal} (0.112-0.126),
...
treatment contrast is within-subject. Blackston 2019's\textsuperscript{\citeproc{ref-blackstonComparisonAggregatedNof12019}{4}} observation that
moderate-to-strong carryover inflates Type I error when not
modeled is not reproduced here for the hybrid (because the
decaying \texttt{Dbc} in the analysis absorbs carryover correctly)
...
\item
  \textbf{Sample-size advantage of the hybrid N-of-1 over the
  ...
  approximately one-third matches Blackston 2019's\textsuperscript{\citeproc{ref-blackstonComparisonAggregatedNof12019}{4}} and Carrozzo 2025's\textsuperscript{\citeproc{ref-carrozzoTwoArm2025}{6}} reported N-of-1 efficiency advantage.
\item
  ...
  range. This reproduces the Senn 2018\textsuperscript{\citeproc{ref-sennSampleSizeConsiderations2019}{8}} variance-decomposition
  prediction exactly.
\item
...
\item
  \textbf{AR(1) serial correlation affects the two designs
...
RCT are \textbf{both highly competent designs} at the
...
Hendrickson is (i) in its Morris\textsuperscript{\citeproc{ref-morris2019using}{1}} MCSE
...
Wang and Schork\textsuperscript{\citeproc{ref-wangPowerDesignIssues2019}{10}} provide analytical
...
\texttt{corCAR1} at the aggregated level. Our parallel RCT power drops
...
\item
  \textbf{S3, S6, and S9 use the alternating A/B/A/B variant rather
...
\item
  ...
  complementary with-vs-without-\texttt{corCAR1} factorial would
  require adding an \texttt{op\$use\_corCAR1} option to \texttt{lme\_analysis}
...
\item
  ...
  analysis that fits \texttt{corCAR1} but with the small sample
...
N-of-1\textsuperscript{\citeproc{ref-zuckerIndividualTrialsPopulation2010}{9}}, the open-label-only
and OL-plus-BDC variants from Hendrickson 2020\textsuperscript{\citeproc{ref-hendricksonOptimizing2020}{5}}, full multi-cycle crossover with more
than two periods (Blackston 2019 used three cycles\textsuperscript{\citeproc{ref-blackstonComparisonAggregatedNof12019}{4}}), Williams-square / Latin
square / complete N-of-1 designs, sequential-monitoring variants
with adaptive stopping\textsuperscript{\citeproc{ref-jiangDesignAnalysis2025}{11},\citeproc{ref-selukarFrameworkSequential2025}{12}}, and randomized-block N-of-1.
...
summary at \texttt{05-design-summary.txt}. With 500 replicates per
...
\texttt{corCAR1} at small per-subject timepoint counts.
