...
\renewcommand{\stamptime}{2026-06-16 15:46 PDT}
\renewcommand{\stampversion}{\detokenize{16bffb2-wip}}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{lineno}
\linenumbers
...
\(N=35\) and \(0.006\) at \(N=150\), within one Monte Carlo standard
error of zero), with near-nominal coverage and type I error. The
...
reaching \(+0.51\) at a biomarker-PB correlation of \(0.45\) (\(N=150\),
some 30 Monte Carlo standard errors from zero), and the operative
...
lumped slope is biased in proportion to the biomarker's correlation
...
the unbiased pharmacological slope (bias within one Monte Carlo SE
of zero across the feasible contamination range at N=150) while the
...
lumped bias grows with the biomarker-\(PB\) correlation. The operative
quantity is the biomarker's correlation with the individual-level
...
breakpoint families and analysed them under the standard linear-mixed
...
fell within two Monte Carlo standard errors of sampling noise) and to
type I error (0.025 to 0.036 across null cells, with no family-specific
inflation). The conclusions drawn here are therefore not artefacts of
...
difficulty Senn has emphasised for individual treatment effects\textsuperscript{\citeproc{ref-senn2004}{49},\citeproc{ref-senn2018}{54}}: a genuine participant-by-component signal
...
Gompertz parametric form. Where both the lumped and the
...
Daza\textsuperscript{\citeproc{ref-daza2018}{55}} formalises an average-period treatment-effect
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item The realised Study A run (section 7.3) is the empirical core of the manuscript; this pilot precedes it.
...
\begin{orig}
The production simulation programme is the empirical core of this
manuscript; its first realised run, Study A, is reported in section
...
\begin{orig}
The production programme follows the\textsuperscript{\citeproc{ref-morris2019}{57}} ADEMP framework and
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item The true biomarker-by-$BR$ effect is $-2.25$; mean absolute bias is $0.016$ at $N=35$, $0.016$ at $N=70$, $0.006$ at $N=100$, and $0.006$ at $N=150$, in every case within one Monte Carlo SE of zero.
...
\(m_{PB} \in \{0,1,3,6,10\}\), \(m_{TV} \in \{-1,0,1,2\}\), and
\(N \in \{35,70,100,150\}\), plus four null cells) returns a result
...
channel, so the true biomarker-by-treatment effect is the
...
one Monte Carlo standard error (respectively about \(0.028\),
\(0.019\), \(0.009\), and \(0.007\)) of zero, with no monotone trend
across \(m_{PB}\) or \(m_{TV}\). The predicted placebo-attribution
bias of the biomarker-by-treatment estimate does not appear.
...
\begin{orig}
Two conclusions follow for the Study A programme. First, the
...
\item At $N=150$, mean $\hat\beta_{bm:D_{bc}}$ shifts from
  ...
  30 Monte Carlo SEs from zero), confirming the identity's
  prediction.
...
30 Monte Carlo standard errors from the true \(-2.25\), establishing
...
bias: at the contaminated cells (\(c_{bm,PB} \geq 0.30\), \(N=150\))
...
\item A trial where the biomarker covaries with optimism,
...
that predict placebo response -- optimism, prior treatment
experience, suggestibility, expectancy at enrolment -- the
...
the \(N=70\) pattern, and characterise type I error under
...
\item The decomposition $Sx \sim bm + t + D_{bc} + \eta + bm{:}D_{bc} + bm{:}\eta$ then isolates $\beta_{bm}^{BR}$ on the $bm{:}D_{bc}$ term while $bm{:}\eta$ absorbs the placebo channel.
\end{itemize}
...
design. The mechanism is that, in the MVN data-generating process, the
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item At $N=150$, the decomposition's bias of $\hat\beta_{bm:D_{bc}}$ stays within one Monte Carlo SE of zero across $c_{bm,PB} \in \{0, 0.1, 0.2, 0.3\}$ ($-0.022$, $+0.008$, $+0.021$, $-0.026$; MCSE $\approx 0.02$), while the one-component bias rises monotonically to $+0.16$, $+0.34$, $+0.48$.
...
timepoints, \(N \in \{70, 150\}\), \(c_{bm,PB} \in \{0, 0.1, 0.2, 0.3\}\))
confirms the recovery at full resolution. At \(N=150\) the mean estimated
...
0.3\) respectively, every value within one Monte Carlo standard error
...
(about \(0.66\) versus \(0.93\) at \(N=150\)), the precision cost of the
...
\item Open questions remaining: recovery beyond the positive-definite range, the participant-level component recovery of the original Study B identifiability aims, and replication of the pattern under Architecture A (mean-moderation) coupling.
\end{itemize}
...
biomarker-by-treatment interaction separately identifiable. Three
questions remain open. Recovery beyond the positive-definite contamination
range is not characterised, since the MVN architecture cannot represent
...
analysis here; and whether the same recovery holds under Architecture A
...
discontinuation and randomised-withdrawal designs\textsuperscript{\citeproc{ref-colloca2004}{51},\citeproc{ref-rohsenow1981}{52}}, the crossover with on/off periods, and
...
\emph{Rendered on 2026-06-16 at 15:46 PDT.}\\
