...
\textbf{Methods.} We formalise two architectures: direct mean moderation
...
reference cell (\(N = 35\), \(c_{bm} = 0.45\), OL+BDC design) across
\(t_{1/2} \in \{0, 0.5, 1.0\}\) weeks, Architecture\textasciitilde B
power drops from \(0.777\) to \(0.539\) (a 30.6\% relative loss;
\(z = 7.64\), \(p < 10^{-13}\) on the B-versus-A contrast), while
Architecture\textasciitilde A drops only from \(0.751\) to \(0.730\) (a
2.8\% relative loss). The B-versus-A absolute power-loss gap of 21.7
percentage points at OL+BDC, \(t_{1/2} = 1.0\) is approximately 11 times
the matched Architecture\textasciitilde A loss; the Hybrid design
produces a smaller but still significant gap (\(z = 3.96\),
\(p < 10^{-4}\)); the traditional crossover design produces no
detectable gap because its within-subject AR(1) structure dominates the
carryover-mediated correlation channel that
...
convergence; MCSE on cell power is 0.013-0.016. Type\textasciitilde I
error sits in \([0.010, 0.074]\), with no architecture-specific
inflation.
...
scientific question rather than a parameterisation convenience. We
...
Hendrickson\textsuperscript{\citeproc{ref-hendrickson2020}{1}} parameter
grid under Architecture\textasciitilde A are documented in
...
where \(D_{it}\) is the treatment indicator (or a continuous measure of
...
interaction is explicit in the mean: participants with higher biomarker
values have proportionally larger treatment effects.
...
al.\textsuperscript{\citeproc{ref-hendrickson2020}{1}} publication code
...
al.\textsuperscript{\citeproc{ref-raskind2013}{2}}, whose SBP-stratified
secondary analysis is the subject of ongoing reanalysis by the
pmsimstats team. The biomarker is resting systolic blood pressure (SBP)
and the outcome is CAPS (Clinician-Administered PTSD Scale) score
change.
...
(1) compressed variance of the treatment indicator \texttt{Dbc} when
...
regresses the estimated random deviations (or equivalently, the BLUPs of
the participant-specific effects) on the biomarker:
...
Table\textasciitilde{}\ref{tab:sec6-strategies} summarises the
...
`Hypothesised benefit' column are qualitative; no simulation evidence is
...
\texttt{furrr} with \texttt{future\_multicore}
(Architecture\textasciitilde C production run), \texttt{data.table} (the
\texttt{original-extended} simulation collection), \texttt{purrr} (the
\texttt{tidyverse} simulation collection), and \texttt{rmarkdown} for
...
independently per randomisation path, then concatenates paths before
...
is 140 subjects. The Architecture\textasciitilde C driver
(\texttt{01-run-combined-factorial.R}) uses \(N = 35\) as the total
number of subjects, divided equally across paths, so that
Section\textasciitilde3.4 figures refer to a 35-subject trial. The two
parameterisations are not directly comparable; Section\textasciitilde3.4
power values are uniformly lower than their Section\textasciitilde3.1
...
\texttt{100000 * cell\_index + rep\_index}. The
Architecture\textasciitilde C driver sets \texttt{set.seed(20260610)}
and uses \texttt{furrr\_options(seed = 20260610L)}.
...
Section\textasciitilde3.4 (Architecture\textasciitilde C): per-replicate
outputs at
\texttt{analysis/data/dgp-combined/01-combined-replicates.rds};
cell-level summaries at \texttt{01-combined-summary.rds}; driver at
\texttt{analysis/scripts/dgp-combined/01-run-combined-factorial.R}. The
...
\emph{Rendered on 2026-06-14 at 09:11 PDT.}\\
\emph{Source:
\texttt{\textasciitilde{}/prj/res/36-pmsimstats-ng/pmsimstats-ng/analysis/report/01-dgp-mean-moderation-vs-mvn/report.Rmd}}
