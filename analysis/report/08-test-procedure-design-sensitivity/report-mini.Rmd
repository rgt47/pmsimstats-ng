...
The estimand of interest in a biomarker-validation N-of-1 programme is
the biomarker-by-treatment interaction: the degree to which a
...
manuscripts characterise this estimand by simulation across test
...
expression,
the efficiency loss that motivates modelling the autocorrelation
...
randomises each participant to a treatment sequence (for example AB or
...
before any aggregation [@guyatt1986; @zucker2010combining]. Pooling
...
when a biomarker is measured, the biomarker-by-treatment interaction
...
continuous, centred baseline biomarker $b_i$ (so $\sum_i b_i = 0$).
...
biomarker-by-treatment interaction; the two-path Hendrickson hybrid,
...
where $\theta$ is the biomarker-by-treatment interaction (the
...
the response scale, $\theta = c_{bm}\,\sigma_{BR}$ (for the
...
ordinary least squares is fully efficient (generalised least squares
...
generalised-least-squares fit recovers the small remaining efficiency,
...
dichotomising the biomarker at its median replaces $s_b^2$ in
