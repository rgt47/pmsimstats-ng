...
\renewcommand{\stamptime}{2026-08-19 11:01 PDT}
...
is stored under an \texttt{E}-prefixed code (\texttt{G1}, \texttt{G3}, \ldots); this
...
fit with the lowest AIC\textsuperscript{\citeproc{ref-akaike1974new}{5}}. This mirrors
...
the paired t-test that Senn, Julious and Araujo\textsuperscript{\citeproc{ref-senn2014four}{7}}
...
First, within every design by \(N\) block, \texttt{G1} and \texttt{G2} (Unadjusted,
...
Second, the Exposure-weighted family (\texttt{G3}, \texttt{G4}, and their CR2
variants \texttt{G8}, \texttt{G9}) visibly separates from \texttt{G1}/\texttt{G2}/\texttt{G6}/\texttt{G7} only
...
cell (\(c_{bm} = 0.45\), \(t_{1/2} = 1.0\)), for example, \texttt{G4} reaches
0.48 against 0.86 to 0.89 for \texttt{G1}/\texttt{G2}/\texttt{G6}/\texttt{G7}. This is the
design-dependence of Section 3.4, read directly off the grid now
across all nine specifications rather than three.

Third, \texttt{G9} and \texttt{G8} lead, in a near-tie, at the Hybrid and OL+BDC
reference cells (both around 0.88 at Hybrid, 0.77 to 0.74 at
OL+BDC). \texttt{G5} (Paired-difference) is the strongest specification at
CO that is not \texttt{G1}/\texttt{G2} or a CR2 variant of them (0.77, versus 0.48
to 0.51 for the Exposure-weighted family there). This is consistent
with \texttt{G5} discarding the carryover representation that costs the
...
\item
  ...
  default: it has been through the full robustness battery \texttt{G4}/\texttt{G5}
  have not (next point).
\item
...
\texttt{02-grid-summary.rds} Tier 1 summary as the headline table, by
\texttt{12-hendrickson-heatmap.R}, and retains the original three
