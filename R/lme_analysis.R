...
lme_analysis<-function(trialdesign_set,dat,op){
  ...
  if((op$carryover_t1half>0)&(op$simplecarryover==TRUE)){
    stop("Don't know what it means to have two different carryover models, so disallowing simplecarryover and a half-life based carryover at the same time for now.")
  }
  ...
  if(!is.null(fit)){
    # Flag near-singular random-effects variance (random intercept
    # estimated at effectively zero). nlme::lme does not expose an
    # isSingular() primitive, so we inspect VarCorr directly.
    issingular<-tryCatch({
      vc<-nlme::VarCorr(fit)
      sd_val<-suppressWarnings(as.numeric(vc["(Intercept)","StdDev"]))
      isTRUE(!is.finite(sd_val) || sd_val<1e-6)
