...
\begin{itemize}
  \item \del{Red strikethrough} marks text proposed for removal.
  \item \add{Blue text} marks text proposed for insertion.
...
substantive single observation (Comment~9) which we treat as the
...
\textbf{Revision.} Inline replacement of ``biomarker value'' with
``baseline biomarker value, measured at trial entry''.
...
selecting the appropriate DGP for a given clinical context. \add{Generalizations to latent-class mixture formulations, copula-based joint models, and richer carryover-decay families remain open methodological directions; some are taken up in companion papers in the pmsimstats-ng project.}
...
this heterogeneity. \add{Note that the qualitative-versus-quantitative interaction distinction operates at the analysis-model layer, classifying the shape of the moderating relationship after a model has been fit. The architectural distinction introduced in this paper is orthogonal to it: the same qualitative interaction can be generated by either Architecture~A or Architecture~B, and the practical consequences for power estimation under carryover differ between architectures regardless of where the result lands in the qualitative/quantitative taxonomy.}
...
differential-correlation parameterization \del{is not a research-program convention but a paper-specific design choice in Hendrickson et al. (2020). To the authors' knowledge}\,\add{is uncommon. To the authors' knowledge}, no other clinical trial
...
\add{A hybrid architecture combining elements of A and B (latent-subtype membership generating Architecture B-like covariance structure within classes whose means are governed by Architecture A-style moderation) is conceptually appealing and is treated separately in the latent-class-mixture-application companion manuscript scaffolded in this repository. We do not attempt that here.}
...
\del{biomarker value of participant $i$}\,\add{baseline biomarker value of participant $i$, measured at trial entry}, $D_{it}$ the binary on-drug
