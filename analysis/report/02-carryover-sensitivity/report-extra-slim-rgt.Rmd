...
<!--
  ...
  scope (Architecture B, exponential and Weibull decay, A2 versus
  A3), the same data, the same tables and figures, but with the
  ...
  4. Filtered Tier 1 figures (Architecture B, no linear decay, A2/A3
...
the interaction test. But how should residual exposure be
represented in the linear predictor? Two answers are in common use,
and the question we take up here is which of them recovers more of
the power that carryover would otherwise cost.
...
A3, performs better across the grid of data-generating decay forms
...
identical regardless of $\hat{t}_{1/2}$. That is, A2 is in effect a
...
[-@jones2014crossover]. One structural feature of A3 bears directly
...
actually tested under A3 remains $\text{bm:}D_{it}$, the same
coefficient tested under the binary on-drug specification (which we
call A1 and do not evaluate further in this manuscript). A3 is best
understood, then, as A1 with a carryover nuisance control added,
rather than as a fundamentally different model of the interaction.
Because it carries no half-life parameter, A3 is invariant to the
analyst's assumed half-life by construction, in contrast to A2. This
...
where A2 is in fact dominated, a point we return to in Section 4.
...
A2 appears fairly robust to a mismatch between the analyst's assumed
...
erode A2's advantage over A3 at either true half-life. It would
appear, then, that A2 remains a defensible choice even when the true
...
The principal finding is that A2's advantage over A3 is real, but
conditional. On the one hand, at its best cell (Hybrid design,
$N = 70$, $c_{bm} = 0.45$, $t_{1/2} = 1.0$), A2 exceeds A3 by about
...
design A2 performs considerably worse (0.488 versus 0.830 for A3).
...
signal that A2's decaying predictor was designed to recover, so A2
...
recommendation that A2 is to be preferred therefore holds only in
...
true form is Weibull, A2 still does not fall below A3, within the
range of decay shapes we examined. On the assumed half-life (Figure
\@ref(fig:fig-sens-s2)): A3 does not use a half-life and is
therefore unaffected, while A2 does, yet mis-specifying the
half-life by a factor of two or four moves A2's power only from
about 0.83 down to 0.77, never below A3. On autocorrelation (Figure
...
between 0.12 and 0.25 and become essentially indistinguishable. A2
...
choice between A2 and A3 ever could.
...
A2-versus-A3 contrast, in the interest of a self-contained
...
lead one to expect that a shape-free lagged term such as A3 should
outperform a parametric predictor such as A2 once the decay shape is
mis-specified. Our S2 result does not support that expectation over
the range we examined, because A2's penalty for an incorrect
half-life appears too small for A3 to overtake it.
...
1. **A2 helps, but conditionally.** The exposure-weighted predictor
   achieves the highest power, about 10 points over A3, only in the
   ...
   those designs. Under the crossover design, A2 confers no
   advantage, and A3 is at least as good. Where A2 is used, we
   recommend reporting it with a CR2 cluster-robust standard error
   (Block S6).

2. **An approximate half-life is adequate, where A2 is used.**
   Mis-specifying the half-life by a factor of two to four moves
   A2's power only modestly, so a rough pharmacokinetic estimate is
