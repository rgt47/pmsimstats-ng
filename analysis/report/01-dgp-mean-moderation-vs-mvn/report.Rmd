...
\emph{[Placeholder: 3$\times$3 power table for CO design,
$N = 35$, $t_{1/2} = 1.0$, A2 spec.
Insert from \texttt{analysis/data/dgp-combined/02-power-grid-co.csv}
once the production run completes.]}
...
\begin{orig}
\emph{[Orig placeholder: narrative of 3$\times$3 panel results.
To be completed once the production simulation
(\texttt{01-run-combined-factorial.R}) finishes and
\texttt{02-summarise-combined.R} produces the summary tables.]}
\end{orig}
