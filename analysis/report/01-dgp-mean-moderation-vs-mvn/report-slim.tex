...
(differential correlation, the multivariate normal (MVN) approach): the
...
the joint distribution rather than in the mean; on this reading it is
the reduced-form second-moment encoding of a latent responder-class
mechanism (Section
...
value can still respond differently. Architecture A cannot represent
this, because it assumes the biomarker fixes each patient's drug effect
exactly; Architecture B represents it by letting biomarker and response
track each other only while the drug acts, as a tendency rather than a
fixed rule. That tendency fades once the drug washes out, which is why
carryover costs more power under Architecture B than under Architecture
A (Section 4 gives clinical examples).
...
absent during off-drug periods, producing the differential correlation
...
\(N = 35\) total subjects per cell (randomized equally across paths);
...
form. These boundary cells confirm that the combined parameterization
...
sample size, whereas \S3.1 uses \(N = 35\) per randomization path
...
are: (i) power for \(\beta_{bm:D}\) at \(\alpha = 0.05\), with MCSE
...
\begin{orig}
Under Architecture B the power loss has two sources: the drug exposure
variable's variance shrinks, and the biomarker-response correlation
...
Sargent\textsuperscript{\citeproc{ref-renfro2017}{11}}, and the standard
...
the PATH statement\textsuperscript{\citeproc{ref-kent2020path}{6}}.
