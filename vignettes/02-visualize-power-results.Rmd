...
This is the second part of a pair of vignettes designed to walk through the steps used to generate and visualize simulated N-of-1 trial power results with the pmsimstats package, of the kind reported in the package's companion manuscripts under `analysis/report/`. The steps fall into four basic steps:
...
p2<-ggplot(data=d2[c.bm>0])+geom_point(aes(y=tpval,x=test,color=censorparamset,shape=censorparamset),size=2)+
  ...
  xlab("Estimated bias in model coefficient")+ylab("Signifiance (p-value)")+
  labs(color="Censoring",shape="Censoring")+
  facet_grid(c.bm~trialdesign)+ggtitle("B. Bias compared to full data set without censoring")
plots<-list(p1,p2)
