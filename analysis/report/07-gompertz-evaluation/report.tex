...
\fancypagestyle{plain}{%
  ...
  \renewcommand{\headrulewidth}{0pt}%
  \renewcommand{\footrulewidth}{0.4pt}}
\renewcommand{\stampsource}{\textasciitilde{}/\allowbreak{}prj/\allowbreak{}res/\allowbreak{}36-pmsimstats-ng/\allowbreak{}pmsimstats-ng/\allowbreak{}analysis/\allowbreak{}report/\allowbreak{}07-gompertz-evaluation/\allowbreak{}report.Rmd}
\renewcommand{\stamptime}{2026-09-07 11:36 PDT}
...
\textbf{Methods.} We evaluate the modified Gompertz family against the
...
\textbf{Results.} We ran a 1\{,\}000-replicate-per-cell simulation across
...
is well-established in tumor growth dynamics\textsuperscript{\citeproc{ref-laird1964dynamics}{2},\citeproc{ref-norton1988gompertzian}{3}}, microbial growth
...
than assumed away. The methodological N-of-1 corpus\textsuperscript{\citeproc{ref-hendrickson2020}{1},\citeproc{ref-lillie2011}{8}--\citeproc{ref-vohraCONSORTExtensionReporting2015}{10}} is, to the best of our knowledge, silent on
...
Bertalanffy growth curve\textsuperscript{\citeproc{ref-vonbertalanffy1957}{14}}, and a
...
\subsection{What the trajectory template is for}\label{what-the-trajectory-template-is-for}

A power simulation for an aggregated N-of-1 trial has to turn a
\emph{design} into a \emph{distribution}. The design is a schedule: a sequence
...
{\centering \includegraphics[width=1\linewidth]{report_files/figure-latex/fig-families-1} 

}

\caption{The trajectory-family manipulation, at the calibration used throughout this paper. Panel (a): the biological-response mean curve under each of the four families, against cumulative time on drug, matched at the asymptote and at the week-5 anchor (dashed line). Panel (b): each family's deviation from the Gompertz reference. Dotted vertical lines and plotted points mark the six distinct on-drug durations the OL+BDC design actually samples (weeks 4, 8, 12, 16, 17, 18). The families are separated at the first two on-drug visits and have effectively converged by the third.}\label{fig:fig-families}
\end{figure}
...
1.32 CAPS-IV points at the first two visits,
against a BR residual standard deviation of
8, and falls to
0.35 points from week 12 onward. Two features
...
\subsection{How the Hendrickson framework treats the template, and what it estimates}\label{how-the-hendrickson-framework-treats-the-template-and-what-it-estimates}

The framework this paper evaluates\textsuperscript{\citeproc{ref-hendrickson2020}{1}} treats the
...
\(\mathrm{maxr}_{BR} = 10.99\) CAPS-IV total
severity points, \(\mathrm{disp}_{BR} = 5\),
and \(\mathrm{rate}_{BR} = 0.42\) per week;
...
\emph{parameters} of that family, the rates and the maxima in particular,
...
that the treatment effect \emph{varies with} the biomarker, not power to
...
broader clinical-trial methodology literature\textsuperscript{\citeproc{ref-morris2019simulation}{15}}, applied one level down in the
...
tradition, where its use traces to Laird's tumor dynamics work\textsuperscript{\citeproc{ref-laird1964dynamics}{2}} and to Zwietering's microbial growth
...
The CONSORT extension for N-of-1 trials\textsuperscript{\citeproc{ref-vohraCONSORTExtensionReporting2015}{10}}
...
reference work building on\textsuperscript{\citeproc{ref-raskind2013}{18}}) inherit the
...
with results reported in the Morris-White-Crowther\textsuperscript{\citeproc{ref-morris2019simulation}{15}} ADEMP framework already adopted as the
...
The simulation borrows the comparative methodology of\textsuperscript{\citeproc{ref-benzekry2014classical}{11}} (eight-model tumor-growth comparison).
The sloppiness diagnostic of\textsuperscript{\citeproc{ref-jagadeesan2023sloppiness}{16}} motivates the identifiability framing
in this Introduction but is not itself computed in the present
16-cell focused submission; it is Study 2 of the ADEMP
pre-registration (\texttt{00-ademp-pre-registration.md}) and remains
...
\(\mathrm{maxr}_{BR} = 10.99\) CAPS-IV total
severity points, \(\mathrm{disp}_{BR} = 5\), and
\(\mathrm{rate}_{BR} = 0.42\) per week for the biological
response, with a within-timepoint residual standard deviation of
8 points. The expectancy and natural-history components are
...
\subsection{Sampling distribution of the biomarker-treatment interaction estimate}\label{sampling-distribution-of-the-biomarker-treatment-interaction-estimate}
