...
\renewcommand{\stamptime}{2026-09-06 19:06 PDT}
\renewcommand{\stampversion}{\detokenize{0a7b6e2-wip}}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{booktabs}
...
read. In the reference implementation
(\texttt{generateData}), the biological response BR is
evaluated at \texttt{tod}, the cumulative time the participant has
spent on active drug; the expectancy or placebo-belief component PB
is evaluated at \texttt{tpb}, the accumulated time under a nonzero
expectancy, and is then scaled by the visit's expectancy weight; and
the time-varying natural-history component TV is evaluated at
calendar time since baseline. The observed score is their sum plus a
correlated error. That the three components read three different
clocks is what makes them separable by design rather than by
assumption: a blinded-discontinuation block advances calendar time
and belief time while freezing time on drug, so the components come
apart in a way that no single-clock specification could represent.
Absent a trajectory template there is no clock to read, and being on
...
clinical-trial methodology literature\textsuperscript{\citeproc{ref-morris2019simulation}{15}}, applied
...
and empirical standard error of the biomarker-treatment interaction
estimate under each cell, with conventional MCSEs. The simulation borrows the comparative
methodology of\textsuperscript{\citeproc{ref-benzekry2014classical}{11}} (eight-model tumor-growth
comparison). The sloppiness diagnostic of\textsuperscript{\citeproc{ref-jagadeesan2023sloppiness}{16}} motivates the identifiability framing
