...
(RCTs) for detecting moderate treatment main effects. However,
their power profiles depend on a substantial set of parameters
...
through S12) against the pmsimstats-ng tidyverse pipeline. We used
...
correlation. Each sweep records per-replicate results, so that we
can compute Monte Carlo standard errors (MCSE) at render time per
Morris et al. [-@morris2019simulation].

**Results.** At a baseline of $N = 35$ participants per design, the
hybrid achieves comparable or higher power than the parallel RCT
under within-patient and between-patient variance perturbations. It
is robust to AR(1) serial correlation that inflates RCT Type I
error, and it tolerates carryover up to the within-subject period
length before power erodes. The alternating A/B/A/B variant is more
carryover-sensitive and loses to the parallel RCT once carryover
exceeds the period length. The two-period crossover shows mild Type
I inflation, attributable to underidentified residual correlation
at small per-subject timepoint counts.

**Conclusions.** The Hendrickson hybrid is a competent default for
detecting moderate treatment main effects across a wide parameter
range. Design ordering favors the parallel RCT only when carryover
exceeds the within-subject period length. That ordering is
directionally different for biomarker-treatment interaction
...
individual patients across repeated crossover periods.
Patient-level estimates are then combined through meta-analytic or
mixed-effects machinery to support population-level inference
[@lillie2011; @kravitzSinglepatientNof1Trials2014]. Several recent
simulation studies report that aggregated N-of-1 procedures can
match or exceed the statistical power of parallel-group randomized
...
magnitude of this efficiency advantage varies across studies,
however, and the conditions under which it holds or reverses remain
incompletely characterized. That is, the published literature
offers encouraging headline comparisons but relatively sparse
guidance on the parameter axes that actually govern the ordering of
designs.
:::
...
specific hybrid aggregated-N-of-1 design. The design combines an
open-label titration phase, a blinded discontinuation phase, and a
brief crossover phase, following the proposal of Hendrickson and
colleagues [@hendricksonOptimizing2020] for predictive-biomarker
validation in chronic neurobehavioral conditions. The sensitivity
sweeps are conducted against the pmsimstats-ng tidyverse pipeline
that operationalizes this design in R. Three parallel comparators
are included: an alternating A/B/A/B aggregated N-of-1 variant, a
two-period crossover, and a parallel-group RCT. The target estimand
throughout is the main drug effect, which we distinguish from the
biomarker-treatment interaction examined in the original
...
straightforward. Published simulation comparisons typically vary
one or two parameters at a time while holding the remainder fixed
at calibration defaults. This leaves open the question of which
axes actually drive the N-of-1-versus-RCT power difference. We have
endeavored to address this gap by reporting twelve sensitivity
...
comparison along twelve parameter axes, specifically for the hybrid
design family.
:::
...
::: {.rgt data-latex=""}
The simulation study has three aims. First, we aim to characterize
the power profile of the Hendrickson 2020 hybrid aggregated-N-of-1
design [@hendricksonOptimizing2020] under systematic perturbation
of the parameter axes implicated in the published
simulation-comparison literature. Second, we aim to identify the
parameter regimes in which the hybrid is preferred over a
parallel-group RCT, and the regimes in which that ordering
reverses. Third, we aim to verify Type I error control under null
and near-null conditions, with sufficient replication to hold MCSE
below 1% per Morris et al. [-@morris2019simulation].
:::
...
participants per design (the Hendrickson 2020 minimum) and 8 weekly
timepoints, unless explicitly varied by the sweep in question.
Per-replicate sample size is $N$ participants for each design under
comparison. We note that participants are matched on the per-design
count rather than on observation total, following the convention
adopted by Blackston [-@blackstonComparisonAggregatedNof12019],
...
[@hendricksonOptimizing2020]. We cross the two architectures in
sweep S10 below; the remaining eleven sweeps use the
covariance-moderation architecture as the default.
:::
...
::: {.rgt data-latex=""}
The curve is evaluated at the appropriate cumulative time index,
...
$\max(0, (1 - t / (3 t_{1/2}))^p)$. The decay family chosen on the
data-generating side need not match the one assumed on the analysis
side; sweep S3 quantifies the cost of that mismatch.
...
The conversion is one week to seven days, so those papers' grid
corresponds to $\{0, 3.5, 7\}$ days here, and this paper's 3-day
baseline corresponds to roughly 0.43 weeks. Timepoints $t$ are
weekly under both conventions.
:::
...
inference is the main drug effect, that is, the \texttt{Dbc} coefficient,
...
\texttt{set.seed(42 + sweep\_id)}, and constructs its parameter grid. It
then calls \texttt{generate\_simulated\_results()} with furrr-based
...
\texttt{run-all.R} runs all twelve sweeps in the order specified in the
plan. We write per-replicate rows rather than pre-aggregated
summaries, so that power, bias, MSE, empirical SE, and Type I
error can all be computed at render time with Morris MCSE.
...
wall-clock time on an 8-core machine, using the furrr multisession
backend. Per-cell summary metrics are computed from the RDS
artifacts at render time; the numerical values quoted below are read
directly from \texttt{analysis/data/derived\_data/sensitivity/}. The
target coefficient throughout is the main drug effect, that is, the
\texttt{Dbc} coefficient in the \texttt{nlme::lme} fixed-effects formula.
:::
...
[@hendricksonOptimizing2020] hybrid, that is, open-label titration
followed by blinded discontinuation and a brief crossover, labeled
'Nof1' in the sweep artifacts and figures below. We retain the
alternating A/B/A/B aggregated N-of-1 (labeled 'Nof1-alt') as a
sensitivity comparison at sweeps that vary the cycle count (S6) and
period length (S9), where the hybrid's fixed eight-timepoint
...
competently. The hybrid reaches 80% power at a smaller sample size,
under larger within-patient variance, and under higher serial
correlation than the parallel RCT. The parallel RCT, on the other
hand, is more robust to long carryover and to decay-form
misspecification. Type I error is controlled near nominal under the
null for the hybrid, the alternating variant, and the parallel RCT,
with modest conservatism at small $n$. The two-period crossover,
however, shows mild Type I inflation of approximately 0.10 to 0.13,
a finding that warrants caution when that design is used at small
per-subject timepoint counts.
:::
...
at $N = 35$, $c_{bm} = 0.3$, and a carryover half-life of 3 days.
...
$\delta = -1$, and 0.996 at $\delta = -1.5$, saturating at 1.000
...
the 80% power target at approximately $\delta = -1$, while the
parallel RCT reaches the same target only at approximately
...
parallel RCT (MCSE 0.006, [0.022, 0.046]). The hybrid runs
slightly above nominal and the parallel RCT slightly below,
...
when a study is powered near the 5% boundary.
:::
...
A/B/A/B variant is more carryover-sensitive, with power dropping
from 0.896 at $t_{1/2} = 0$ to 0.778 at $t_{1/2} = 3$ to 0.304
at $t_{1/2} = 14$. The parallel RCT is essentially immune to
carryover at the scale examined here, with power flat at 0.92 to
0.96 across the grid. The two-period crossover is likewise
...
treatment-accumulated signal that is not erased by the
post-discontinuation off-drug period. The alternating design, by
contrast, relies on repeated within-subject transitions, each of
which sheds signal in proportion to the unwashed residual. The flat
...
at the within-period scale. We note that the loss of effective
sample size in within-subject designs, once $t_{1/2}$ exceeds
the period length, is the primary mechanism behind the
hybrid-versus-RCT power-ordering crossover that begins at
approximately $t_{1/2} = 10$ days.
:::
...
decay in constructing the continuous drug indicator
\texttt{Dbc}. We use the alternating A/B/A/B N-of-1 variant for this
sweep because the hybrid's specific multi-phase \texttt{tsd} pattern
interacts poorly with the 'power' form, producing
non-positive-definite covariance matrices at certain cells. The
hybrid is, in any case, less informative for decay-form
...
The Weibull $k=1.5$ form yields 0.852. This 15-percentage-point
range across decay forms is smaller than the range observed under
the interaction target examined in earlier work, which suggests
that main-effect estimation is relatively less sensitive to
decay-form misspecification than interaction estimation. The
parallel RCT is essentially flat, at 0.93 to 0.95 across all
forms, consistent with the finding of Gärtner 2023
[@gaertnerCarryover2023] that misspecification of the carryover
decay form carries a design-specific cost affecting only
...
baseline-symptom SD, that is, the \texttt{BL} factor standard deviation,
the $\tau$ component in the Senn 2018 decomposition, across
...
across the six levels. The parallel RCT, on the other hand,
degrades substantially with $\tau$: from 0.998 at $\tau = 0.5$ to
0.994 at 0.75, 0.978 at 1.0, 0.958 at 1.25, 0.946 at 1.5, and
0.874 at 2.0 (the last with MCSE 0.015). The RCT loses
approximately 12 percentage points of power as between-patient
...
parallel-RCT variance, since $\tau^2$ is part of
...
within-patient variance. At a smaller effect size, or a larger
$\sigma$ (see S5), we would expect the N-of-1 advantage over the
RCT to widen rather than flatten as $\tau$ increases.
:::
...
within-patient variance reduces the power of both designs, with
the N-of-1 design absorbing that variance more efficiently
because it sums over $k$ paired contrasts per patient. The
hybrid's quasi-saturation indicates that the eight-timepoint
structure, together with the open-label titration phase, provides
enough signal accumulation to tolerate substantial within-patient
noise. At smaller effect sizes the $\sigma$ dependence would be
steeper, and this saturation should not be extrapolated beyond
...
per patient $k$ across $\{2, 4, 6, 8, 12, 16\}$. We use the
alternating A/B/A/B N-of-1 variant for this sweep because the
hybrid's eight-timepoint structure does not naturally scale with
$k$. Under the alternating N-of-1 design, main-effect power
...
RCT, since each additional timepoint contributes one more
between-subject observation, whereas the alternating-N-of-1
paired contrasts are already largely averaged in by $k = 8$.
This result highlights that the hybrid design's primary power
advantage over the alternating variant comes from the open-label
...
of 35, which anchors the cross-paper comparison with the
companion clinical manuscript. Under the hybrid design,
main-effect power is high even at modest sample sizes and
saturates quickly as $N$ grows; the parallel RCT follows a
similar but offset trajectory.
:::
...
the parallel RCT reaches the same threshold only somewhere between
...
estimated $\beta$ shrinks for the RCT as well, from $-1.01$ at
...
designs, and this asymmetry warrants a closer look. For the
hybrid, strong AR(1) produces more coherent within-subject
...
$\{0.5, 3, 7\}$ days. We use the alternating A/B/A/B variant for
this sweep. Under the alternating N-of-1 design, main-effect power
increases monotonically with period length: at a period of 3
days, power is 0.40 to 0.44 across the three half-lives; at 5
days, 0.72 to 0.76; at 7 days, 0.88 to 0.91; at 10 days, 0.993
to 0.997; and at 14 days, 0.997 to 1.00. At fixed period length,
...
more than roughly 50%. Wang and Schork 2019
...
hybrid, alternating N-of-1, and parallel RCT. Under the
...
RCT power ranges from 0.914 to 0.958.
:::
...
is expected, and it is reassuring rather than surprising. The
main drug effect is determined by the Gompertz
$\mathrm{max}[\mathrm{br}]$ and the trial's on-drug
accumulation, not by the biomarker coupling. This finding
provides a robustness check on the DGP implementation: when the
...
for the hybrid N-of-1 falls within 2 MCSE of the nominal 0.05
(0.036 to 0.056 across the four cells). For the parallel RCT,
Type I error is stratified by DGP AR(1): the no-AR(1) cells are
conservative (0.018 to 0.024), while the high-AR(1) cells are
inflated above nominal (0.112 to 0.126), with MCSE 0.014 to
0.015, so this inflation is statistically robust. Under the
alternative, the hybrid is saturated at 1.000 in all four cells;
parallel RCT power runs 0.986 to 0.990 when no AR(1) is present,
and drops to 0.766 to 0.772 when AR(1) $\rho = 0.7$ is present.
:::
...
::: {.rgt data-latex=""}
The inflated RCT Type I error under high DGP AR(1) is an
important caveat that merits comment. When the DGP carries
strong serial correlation and the parallel-group analysis fits
\texttt{corCAR1}, the variance estimator still treats each participant
as providing approximately independent observations at the
between-subject level, which understates the true standard error
of the group contrast. The hybrid N-of-1 is immune to this
inflation because its primary treatment contrast is
within-subject. We note that Blackston 2019's
[@blackstonComparisonAggregatedNof12019] observation, that
moderate-to-strong carryover inflates Type I error when not
modeled, is not reproduced here for the hybrid, since the
decaying \texttt{Dbc} in the analysis absorbs carryover correctly. It
is, however, analogous in mechanism to the AR(1) inflation we
observe here for the parallel RCT.
:::
...
2,000 replicates per design, for an MCSE near 0.005. Across the
sixteen design-configuration cells, empirical Type I error
rates are as follows: hybrid N-of-1 in [0.056, 0.062] (four
cells, all within 2 MCSE of nominal); alternating N-of-1 in
[0.057, 0.064] (four cells, all within 2 MCSE of nominal);
parallel RCT in [0.029, 0.060] (four cells, three conservative
and one nominal); and two-period crossover in [0.100, 0.130]
(four cells, all modestly inflated, with MCSEs of 0.006 to
0.007, so the inflation is statistically robust at 2,000
replicates).
:::
...
to the small number of timepoints, two per subject, which
produces an underidentified \texttt{corCAR1} residual correlation.
Two-period crossovers should be analyzed without a
serial-correlation structure, either via a paired $t$-test on
change scores or via an lme with an unstructured residual.
Fitting \texttt{corCAR1} at only two within-subject timepoints is
problematic and, as we see here, inflates the rejection rate by
a practically meaningful amount.
:::
...
whereas the parallel RCT reaches the same target only somewhere
between $N = 24$ and $N = 35$ (S7). This sample-size ratio of
...
Third, sensitivity to carryover depends on the design variant.
The hybrid's four-week open-label titration phase provides
...
$t_{1/2} = 14$ range. The parallel RCT, by contrast, is
...
0.018 to 0.126 under a high-$\rho$ DGP (S11). This is consistent
...
0.95 (hybrid). The hybrid approaches CO power while still
permitting an 8-week open-label titration phase. A central
Hendrickson finding is that, at the biomarker-interaction target,
even short carryover half-lives erode hybrid and OL+BDC power
...
Hendrickson is, first, in its Morris [-@morris2019simulation]
MCSE reporting for every performance measure; second, in
sensitivity S3, which crosses DGP decay form with the
analysis-model assumption of exponential decay; and third, in
sensitivities S2, S6, and S9, which quantify carryover and
period-length effects for the main drug effect. A key
substantive divergence is that our main-effect findings show the
hybrid saturated for carryover half-lives up to five days, while
Hendrickson's biomarker-interaction findings show hybrid power
collapsing within a fraction of a week. Both findings are
correct. They simply describe different estimands, and the
reader should be careful not to conflate the two.
:::
...
$\rho = 0.75$, a roughly linear erosion as serial correlation
increases. For $40p \times 2i$, power runs 0.851, 0.745, 0.674,
...
in which parallel-RCT Type I error inflates to 0.11 to 0.13 under
...
$N = 35$, a caveat we take up below under Limitations.
:::
...
The hybrid N-of-1 saturates in many sweeps. It reaches 100%
power in S4 (all $\tau$ values), much of S5 ($\sigma$), S8
...
dropping baseline power into the 0.50 to 0.80 range, where
design differences are most clearly discriminated. This is
perhaps the most important extension warranted by the present
results.
...
nominal Type I error. We note also that the S11 RCT Type I range
...
data-generating-mechanism regimes, not inconsistent estimates of
a common quantity.
:::
...
numbers reported here, so our results serve as an upper bound
...
power at or above the numbers reported here, so our results
serve as a lower bound on those designs under matched
...
roughly three MCSE, suggestive but not decisive. At
$N = 50$, the swing is 0.072 (range 0.266 to 0.338, peak at
$c_{br} = 0.45$). The peaks at $N = 35$ and $N = 50$ do not
coincide on $c_{br}$, which is the pattern we would expect if the
...
80% power; reaching that target requires a substantially larger
...
Hendrickson hybrid, that is, open-label titration plus blinded
discontinuation plus a brief crossover, is a competent default for
...
robust Type I inflation observed in S12 warrants a change to
...
axes for the hybrid design family. Taken together, the hybrid's
robustness across most axes, with the two specific exceptions
noted above, supports its use as a default candidate in trial
planning for chronic neurobehavioral conditions where moderate
treatment heterogeneity and short-to-moderate carryover are
expected.
:::
...
zzcollab Docker container, with the image hash recorded in
\texttt{Dockerfile} and the package set captured in \texttt{renv.lock}. The
principal packages used by the simulation pipeline are
\texttt{nlme} (continuous-time
...
index. We note that these two seedings operate at distinct stages
of the pipeline and do not collide.
:::
...
cell-level summaries are at the companion \texttt{05-design-summary.txt}.
