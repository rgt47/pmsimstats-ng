...
consequences.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item HTE literature (Rothwell 2005; Kent et al. 2020)
...
near-universal standard; the Hendrickson [@hendrickson2020] MVN approach
is apparently unique.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Every major simulation framework uses direct mean
  moderation: Simon [@simon2010] for stratified designs; Freidlin and
  Korn [@freidlin2014] for adaptive enrichment; Renfro and Sargent
  (2017) for basket/umbrella trials; all N-of-1 frameworks.
\item Wang and Schork [@wang2019] and Schork [@schork2022] also use
...
\item The multiplicative form maps directly to the standard
  additive regression (Kent et al. 2020 effect modeling
  equation).
...
\textbf{30.6\% (OL+BDC)}. The OL+BDC loss is approximately
...
erosion. Two-proportion $z$-tests on the
B-versus-A absolute power-loss gap give $z = 7.64$
($p < 10^{-13}$) at OL+BDC and $z = 3.96$ ($p < 10^{-4}$) at
Hybrid; the CO contrast is not statistically distinguishable
from zero ($z = 0.29$) because the CO design's within-subject
AR(1) structure dominates the carryover-mediated correlation
decay channel that Architecture B uses to encode the
...
are 9.3 percentage points (Architecture~B; $z = 7.4$,
$p < 10^{-12}$) and 1.7 percentage points (Architecture~A;
$z = 2.0$, $p \approx 0.05$). The qualitative ordering matches
...
with the covariance channel governing the carryover sensitivity.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Interior cells ($c_{bm,A} > 0$, $c_{bm,B} > 0$) show
  power strictly between the boundary-row and boundary-column
  values.
...
Second, interior cells are uniformly super-additive relative to
the boundary-row and boundary-column averages. At ($c_{bm,A} =
0.22$, $c_{bm,B} = 0.22$) in the Hybrid design, power is 0.567,
whereas the arithmetic mean of the corresponding pure-architecture
values (0.198 for pure A, 0.152 for pure B) is only 0.175. The
joint signal compounds rather than aggregates.
...
\begin{bullets}
\textbf{Senn [@senn2016mastering] cautions that apparent personalized
response is often a variance artifact; Architecture B
presupposes a real, stable latent subtype, which may not hold.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Senn [@senn2016mastering] argues that apparent biomarker-by-treatment
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Earlier single-site Raskind trials [@raskind2003;
  @raskind2007; @raskind2013] reported benefit on nightmares and
  PTSD symptoms.
\item PACT [@raskind2018] failed to replicate; this has been
...
\begin{bullets}
\textbf{Hendrickson et al. [@hendrickson2020] correctly uses Architecture B
...
simulations.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item ADEMP (Morris et al. 2019) prescribes five elements:
...
design. We regard this as a direction for future work.
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Rothwell [@rothwell2005] establishes the predictive/prognostic
  biomarker distinction; Kent et al. [@kent2020path] synthesize it into
  the PATH Statement.
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Simon [@simon2010], Freidlin and Korn [@freidlin2014], Renfro and Sargent
  (2017), Haller and Ulm [@haller2018], and the platform trial
  literature all use Architecture A.
\item N-of-1 simulation studies (Zucker et al. 1997; Araujo et
  al. 2016; Duan et al. 2013) use hierarchical random-effects
  models, a variant of Architecture A.
\item Wang and Schork [@wang2019] and Schork [@schork2022] share authorship
...
et al. (2020). It appears, then, that the differential-
...
Rizopoulos [@rizopoulos2012], Bayesian joint modeling frameworks, and
copula-based simulation studies. In the clinical trial
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Senn [@senn2002], Jones and Kenward [@jones2014crossover], and Sturdevant and
  Lumley [@sturdevant2021carryover] treat carryover as a nuisance parameter; none
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Simon [@simon2010] and Freidlin and Korn [@freidlin2014] frame power
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Renfro and Sargent [@renfro2017] discuss power calculations that
...
\item This recommendation parallels the advice in Section 5.3
  and is consistent with the PATH Statement (Kent et al. 2020).
...
consistent with the PATH Statement (Kent et al. 2020), which
...
\texttt{tidyverse} simulation collection), and \texttt{rmarkdown}
plus \texttt{bookdown} for the manuscript build.

\textbf{Random-number generator.} The simulation driver sets
\texttt{set.seed(20260507)} at the top of the replicate loop and
uses \texttt{parallel::mc.set.seed} inside \texttt{mclapply}.
...
\texttt{analysis/data/quick-sim/01-dgp-replicates.rds}; Morris
ADEMP cell-level summaries are at the companion
\texttt{01-dgp-summary.txt}. The simulation driver is at
\texttt{analysis/scripts/quick-sim/01-dgp-prototype.R}. The
manuscript source, bibliography, and SIM CSL file are at
...
evaluate statistical methods. \textit{Stat Med}.
