...
\renewcommand{\stamptime}{2026-07-30 17:24 PDT}
\renewcommand{\stampversion}{\detokenize{c530e71-wip}}
\usepackage{amssymb}
\usepackage{booktabs}
...
\begin{figure}
\includegraphics[width=0.95\linewidth]{../../figures/02xs-power-by-spec} \caption{Power versus carryover half-life under the exponential DGP, Architecture B, stratified by analysis specification. E2 retains a modest advantage over E3 in the Hybrid and OL+BDC designs at $t_{1/2} = 1.0$ weeks, but is dominated by E3 under the crossover (CO) design.}\label{fig:fig-power-by-spec}
\end{figure}
...
directory \texttt{analysis/scripts/carryover-sensitivity/output/}.
Cell-level summaries are \texttt{02-grid-summary.rds} and
\texttt{02-sensitivity-summary.rds} under \texttt{analysis/data/}. Six scripts
under \texttt{analysis/scripts/carryover-sensitivity/} generate everything
used here:

\begin{itemize}
\tightlist
\item
  \texttt{01-run-factorial.R} (Tier 1 principal factorial),
\item
  \texttt{04-run-sensitivity-blocks.R} (Tier 2 sensitivity blocks),
\item
  \texttt{02-summarise-grid.R} (cell-level summaries),
\item
  \texttt{03-render-figures-extra-slim.R},
\item
  \texttt{04-sensitivity-figures-extra-slim.R}, and
\item
  \texttt{07-type1-cr2-figure-extra-slim.R} (the three Architecture B
  filtered figure scripts).
\end{itemize}

The same directory holds \texttt{simulation-grid-plan.md}, which documents
