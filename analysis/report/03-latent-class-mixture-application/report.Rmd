...
time than a parallel-group randomised trial of equivalent total sample
...
trials is the biomarker-by-treatment interaction term.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
...
statistical estimand is the biomarker-by-treatment interaction term in
...
biomarker-by-treatment interaction inference under realistic carryover,
...
treatment effect is routinely modelled via random slopes on the
...
\item Methodology from outside biostatistics -- with mature
  formalisation and identifiability analysis -- is directly applicable.
\end{itemize}
\end{bullets}
...
have long histories of formalisation and identifiability analysis in
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Observed indicators are modelled as linear combinations of
  unobserved continuous factors plus residual noise.
\item Between-person heterogeneity is summarised by factor scores along
...
which performance on cognitive tests was modelled as the sum of a
...
individual differences are summarised by factor scores along one or
more dimensions.
\end{orig}
...
\item Local independence within classes is the founding assumption of
  LCA; Goodman derived identifiability conditions and ML estimation.
...
\item Cognitive ability favours continuous factors; certain
  psychopathologies favour discrete subtypes.
...
\begin{bullets}
\textbf{The marketing-science literature formalised covariate-dependent
...
\item Jacobs et al. formalised the same structure as 'mixtures of
...
DeSarbo and Cron [-@desarbocron1988] formalised this as 'clusterwise
...
DeSarbo [-@wedeldesarbo1995] generalised the approach to generalised
...
machine learning, formalised an essentially identical structure under
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Yung developed the FMM as a finite mixture of confirmatory factor
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item GMMs posit class-specific trajectories with within-class
...
primarily in criminology to characterise developmental trajectories of
antisocial behaviour, and has been widely applied in epidemiology,
public health, and clinical research where trajectory heterogeneity is
the principal scientific question.
\end{orig}

\begin{bullets}
\textbf{Overextraction hazards tempered early optimism: GMMs can
...
\item Nylund et al. systematised class enumeration via Monte Carlo
  evaluation of information criteria and BLRT; the cumulative
...
apparent classes are artefacts of distributional non-normality rather
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item The model augments a standard random-intercept-and-slope LMM
...
\item Their motivating application was qualitatively distinct CD4
  trajectories in HIV-infected patient subgroups.
...
data in which subgroups of patients displayed qualitatively different
CD4 trajectories. The model is essentially a growth mixture model in
...
\item \texttt{lcmm} implements joint latent-class mixed models for
  longitudinal outcomes with optional joint survival modelling.
\item \texttt{flexmix} provides a general regression-mixture framework
  ...
  specifications.
\item Both have been used in pharmacokinetic, cognitive ageing, and
  clinical subgroup discovery applications.
\end{itemize}
\end{bullets}
...
and ordinal outcomes, with optional joint modelling of survival
outcomes via shared latent class structure. The \texttt{lcmm} framework has
been deployed in cognitive ageing research, Alzheimer's disease
progression modelling, and clinical-trial subgroup discovery. The
...
pharmacokinetic and pharmacodynamic modelling where individual-level
...
the heading of personalised-medicine power calculation [@gabler2011;
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Under continuous heterogeneity, carryover attenuates $D_{it}$
...
the effective drug indicator $D_{it} = \exp(-\lambda t_{sd,it})$,
...
$D_{it}$. In the latent-class formulation, carryover erodes the
...
\begin{bullets}
\textbf{The paper has four aims: formalise latent-class DGPs for N-of-1
trials, characterise estimation strategies at small $N$, benchmark
power via simulation, and re-analyse the prazosin-PTSD dataset.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Aims 1--2 are methodological: formal specification and
  estimation/diagnostic characterisation with explicit identifiability
  conditions.
\item Aim 3 is computational: a factorial simulation benchmarking
  mixture-DGP against continuous-DGP power under carryover.
\item Aim 4 is applied: re-analysis of the @hendrickson2020
...
\begin{orig}
This paper has four aims. The first is to formalise the latent-class
...
characterise estimation strategies (expectation-maximisation, Bayesian
...
\begin{bullets}
\textbf{The paper moves from notation and DGP specification through MVN
approximation, identifiability analysis, model taxonomy, and spectrum
organisation to analysis implications and a discussion of open
questions.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
...
\item The finite-mixture taxonomy and biomarker-moderation spectrum
  organise the model landscape systematically.
\item The discussion addresses three open questions and situates the
  companion five-study simulation programme.
\end{itemize}
\end{bullets}
...
\begin{orig}
The remainder of the paper is organised as follows. We begin in the
...
section organises the available DGP encodings moment by moment.
...
simulation programme described in the companion simulation plan, which
constitutes the empirical core of the present line of work and is the
next deliverable in the programme.
\end{orig}
...
biological-response component, $D_{it}$ the continuous-decay drug
...
\item $D_{it} = \exp(-\lambda t_{sd,it})$ off-drug and $1$ on-drug;
  $\lambda = \ln 2 / t_{1/2}$.
...
$PB_{it}$. $D_{it}$ denotes drug state at time $t$ in the
analysis-side continuous-decay form $D_{it} = \exp(-\lambda t_{sd,it})$
during off-drug periods and $D_{it} = 1$ during on-drug periods, with
$t_{sd,it}$ the time since drug discontinuation and $\lambda$ the decay
rate. $Z_i$ denotes an unobserved class label. The half-life of
...
\item $Z_i$ is fixed across drug states; it carries participant-level
  ...
  attenuated.
\item Under full carryover washout, class-conditional means converge
  but $Z_i$ remains recoverable from the full trajectory.
\item Under MVN-covariance encoding the carryover erosion of the
  covariance is direct and no residual signal survives washout.
\end{itemize}
\end{bullets}
...
Second, the class-conditional mean trajectory does depend on $D_{it}$:
under full carryover washout ($D_{it} \to 0$) the two class-conditional
...
\item The approximation yields elevated correlation on-drug and
  shrinkage toward zero off-drug after carryover washout.
...
where they coincide (off-drug after carryover washout). For power
...
\item At finite $N$ in the trial-relevant range, finite-sample bias
  can drive the realised gap below the bound or reverse its sign.
\end{itemize}
\end{bullets}
...
finite $N$, finite-sample bias can drive the realised gap below the
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Posterior entropy stabilisation requires hundreds to thousands
...
to several thousand) to stabilise at values that support confident
...
programme described in the companion simulation plan; Study 5 in
...
\begin{bullets}
\textbf{LCA and LPA are cross-sectional mixture models estimating
class membership from observed indicators; in the trial context they
characterise responder classes from response trajectories.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
...
\item The biological hypothesis is distinct responder subtypes with
  additional continuous heterogeneity within each subtype.
...
distinct responder subtypes and that, within subtype, additional
...
ordinary linear model with a treatment-by-biomarker interaction posits
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item The spectrum is organised by which moments the biomarker
  moderates: mean, covariance, both, or higher moments.
\item Covariance-only is the Hendrickson formulation; mean-and-
  covariance and higher-moment forms are the generalisations.
...
moment-by-moment organisation of the spectrum (biomarker moderating the
...
\item This is the core DGP in @hendrickson2020.
\end{itemize}
\end{bullets}
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item FMM and GMM parameterisations typically allow class-specific
...
subgroups with identical expected outcomes but differing residual
...
psychometric parameterisation, subsuming both pure mean moderation and
...
population, which is the more direct generalisation of the
...
\item GAMLSS generalises this to arbitrary distributional shape.
\end{itemize}
\end{bullets}
...
variance; GAMLSS generalises this to arbitrary distributional shape.
\end{orig}
...
of this restrictive case and need not generalise to the
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Random-slopes parameterisation extends the DGP in a direction
...
form rather than a full FMM. The random-slopes parameterisation
...
engaged; recognising the encoding choice opens both sensitivity-analysis
and analysis-strategy programmes.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
...
\item The continuous-versus-discrete choice is a substantive
  scientific question, not a parameterisation convenience.
\end{itemize}
\end{bullets}
...
scientific question, not a parameterisation convenience, and that
recognising this choice opens both a sensitivity-analysis programme
(the spectrum of biomarker-moderation specifications surveyed above)
and an analysis-strategy programme (class-aware mixture analyses where
...
five-study simulation programme has not been executed, and the
...
First, the simulation programme that would empirically resolve the
...
of work and is the next deliverable in the programme. Second, the
...
immediate target; the simulation programme should run in parallel with
...
\item The continuous-decay $D_{it}$ has a clean single-component
...
\item If a latent-class DGP is biologically more plausible, designs
  minimising carryover may gain power the standard hybrid design
  surrenders.
\end{itemize}
\end{bullets}
...
continuous-decay drug indicator $D_{it} = \exp(-\lambda t_{sd,it})$
...
maximise the per-participant contrast and minimise carryover may gain
power that the standard hybrid design surrenders.
\end{orig}

\begin{bullets}
\textbf{The five-study simulation programme addresses the open
...
\begin{orig}
The five-study simulation programme described in the companion plan is
...
biomarker-interaction papers in the wider research programme.
\end{orig}
...
\begin{orig}
A natural reaction to the simulation programme described above is that
...
under the standard pmsimstats parameterisation; or a latent-class
...
biomarker-by-treatment interaction was partly biomarker-by-PB rather
...
adequate. The simulation programme fixed in the companion plan uses the
...
subadditivity in a one-component analysis; the $\gamma > 0$ finding is
ambiguous between direct mechanistic interaction and emergent class
structure.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
...
The five-study production programme is pre-registered under the ADEMP
...
cells across the programme), seeded random-number control, an
...
\item The heterogeneous-random-slopes DGP adds a Bernoulli class-
  membership channel with logistic gating in the standardised
  biomarker.
\end{itemize}
\end{bullets}
...
programme, seeded random-number control, an effect-size calibration
...
the standardised biomarker and class-specific multipliers on the BR
...
hybrid OL design) is reported at \texttt{output/03-study5-pilot.rds}.
...
Type I error at $\alpha = 0.05$ (Monte Carlo standard error 0.022)
...
identifiability artefact: the conditional Wald SE underestimates true
variability because the optimiser latches onto random class boundaries.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Empirical SD of the gating coefficient (63.9) is fifty times
  the median within-replicate Wald SE (0.86).
\item The gating coefficient is centred at zero with no systematic
  bias; inflation is in the SE, not the estimate.
\item This is the overextraction artefact predicted by @bauercurran2003
  and @lubkemuthen2005.
\end{itemize}
\end{bullets}
...
nominal 1.0. The gating coefficient is centred at zero (median
...
$N$. This is a finite-sample identifiability artefact predicted by the
...
\texttt{ng=2}, the optimiser latches onto random patterns to define a
class boundary, and the resulting Wald SE does not absorb the labelling
uncertainty.
\end{orig}

\begin{bullets}
\textbf{The naive LRT is anti-conservative because the true
...
\item The @nylund2007 bootstrapped LRT is the standard calibration
...
\begin{bullets}
\textbf{The full five-study programme is feasible on a single
...
arm on 8 cores. The full programme is feasible on a single workstation
...
well enough that BIC continues to favour \texttt{ng=1} and the
...
programme has not yet been run). The pre-registered ADEMP plan is at
`analysis/scripts/latent-class-mixture-application/
