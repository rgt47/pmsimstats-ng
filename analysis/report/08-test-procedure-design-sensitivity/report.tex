...
\textbf{Background.} Inference about the biomarker-treatment interaction
...
reported under the\textsuperscript{\citeproc{ref-morris2019simulation}{1}} ADEMP framework. The
...
Mancl-DeRouen standard error agrees with the \texttt{geesmv} reference to
within 2.5\%.
...
design\textsuperscript{\citeproc{ref-hendrickson2020}{2}}, dominates the strict classical
...
The degrees-of-freedom corrections\textsuperscript{\citeproc{ref-huynh1976}{4},\citeproc{ref-greenhouse1959}{5}}
...
treatment effect (ATE). The PATH framework\textsuperscript{\citeproc{ref-kent2020path}{7},\citeproc{ref-kent2020pathee}{8}} makes the methodological distinction precise:
...
CONSORT extension reporting standards\textsuperscript{\citeproc{ref-vohraCONSORTExtensionReporting2015}{14},\citeproc{ref-shamseer2015cent}{15}} codify
reporting quality but do not address the analytic-model decision
in any detail, and\textsuperscript{\citeproc{ref-kwasnicka2019}{16}} identifies the analytic-method
...
models is well established:\textsuperscript{\citeproc{ref-ma2012beyond}{17}}, for instance, report
...
Mallinckrodt and colleagues\textsuperscript{\citeproc{ref-mallinckrodt2003}{18}--\citeproc{ref-prakash2008}{21}} applies to parallel-group longitudinal trials with
monotone missingness, not to aggregated N-of-1 designs. The GEE
methodology of\textsuperscript{\citeproc{ref-liang1986}{22}} with sandwich variance estimation has
been adapted to small-cluster settings via bias-corrected
estimators\textsuperscript{\citeproc{ref-wang2016geesmv}{23},\citeproc{ref-pan2001}{24}}, but the small-sample
concerns are acute at trial-relevant N-of-1 cluster counts:\textsuperscript{\citeproc{ref-tong2024stepwedge}{25}} report that in stepped-wedge cluster trials
...
test procedure.\textsuperscript{\citeproc{ref-blackstonComparisonAggregatedNof12019}{28}} compare
...
procedure.\textsuperscript{\citeproc{ref-schork2023precision}{29}} and\textsuperscript{\citeproc{ref-zucker2010}{30}} synthesize the
...
with \texttt{corCAR1}) against a design-parameter grid (cycles, period
...
exclude placebo responders before randomization.\textsuperscript{\citeproc{ref-berger2003runin}{32}}
...
methodology\textsuperscript{\citeproc{ref-senn1994crossover}{9},\citeproc{ref-senn2002}{33}} to the
biomarker-stratified case and clarifies the connection to the
contemporary sphericity-violation literature\textsuperscript{\citeproc{ref-haverkamp2017sphericity}{3},\citeproc{ref-blanca2023sphericity}{6}}.
...
reported in the\textsuperscript{\citeproc{ref-morris2019simulation}{1}} ADEMP framework adopted as
...
derivation specializes classical split-plot machinery\textsuperscript{\citeproc{ref-maxwelldelaney2018}{35}} to the biomarker-stratified case in the
...
namely the mixed-effects model for repeated measures\textsuperscript{\citeproc{ref-mallinckrodt2008recommendations}{36}}, generalized estimating
...
repeated-measures design\textsuperscript{\citeproc{ref-maxwelldelaney2018}{35}}.
...
the split-plot literature\textsuperscript{\citeproc{ref-maxwelldelaney2018}{35}} and anchors the
...
\eqref{eq:rmanova-f} generalizes directly, with degrees of
...
\eqref{eq:rmanova-f-22} to the pmsimstats \texttt{nlme::lme} Wald
\(t\)-statistic on the \texttt{bm:Dbc} coefficient. The two are
...
compound-symmetric, and (iii) \texttt{Dbc} is binary, the E1
...
binding one. Equation \eqref{eq:rmanova-f-22} presupposes
...
open-label (OL) designs, whereas switching to \texttt{nlme::lme} with
\texttt{corCAR1} returns it to nominal. We stress that this
...
sphericity. Either way, the linear-mixed analysis with \texttt{corCAR1}
...
used in the companion calibration manuscript\textsuperscript{\citeproc{ref-pmsimstats-paper10}{38}})
...
(\texttt{stats::pf} in R with the \texttt{ncp} argument; \texttt{PROC\ POWER} in SAS).
For sample-size calculation, equation \eqref{eq:rmanova-ncp}
...
and on/off symmetry around the\textsuperscript{\citeproc{ref-hendrickson2020}{2}} hybrid reference
in \texttt{docs/27-cycle-period-design-sweep-plan.md}; it constitutes the
...
The simulation program follows the ADEMP framework of\textsuperscript{\citeproc{ref-morris2019simulation}{1}}. The primary estimand is the
...
the two-sample \(t\)-test of equation \eqref{eq:rmanova-t}); the
...
\textbf{Units.} Carryover half-lives \(t_{1/2}\) are quoted in days in this
...
\texttt{generateData()} and the standard \texttt{lme\_analysis()} fit, rather
than on the reduced \texttt{implementations/simple} substrate. The
...
linear-mixed with \texttt{corCAR1}, GEE with naive sandwich, GEE with
...
the Mancl-DeRouen correction inline (no \texttt{geesmv}-style external
...
coefficient agrees with \texttt{geesmv::GEE.var.md} to within
approximately 2.5\% (mean inline/\texttt{geesmv} ratio 1.02, standard
...
Kenward-Roger\textsuperscript{\citeproc{ref-kenward1997small}{39}} or Satterthwaite\textsuperscript{\citeproc{ref-satterthwaite1946}{40}} small-sample degrees-of-freedom correction
...
\texttt{analysis/data/quick-sim/08-test-replicates.rds}; the Morris
ADEMP summary is at \texttt{08-test-summary.txt}. With 1000 power
...
the Mancl-DeRouen 2001\textsuperscript{\citeproc{ref-mancl2001covariance}{34}} correction inline
...
than nominal and consistent with \texttt{corCAR1} degrees-of-freedom
...
drivers are at \texttt{analysis/scripts/}. We list the exact paths
...
data-generating process under the mean-moderation architecture via \texttt{generateData()}
