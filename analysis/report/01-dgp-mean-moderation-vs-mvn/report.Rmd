...
interaction is represented as treatment-state-dependent correlation
in a joint multivariate normal (MVN) distribution. Holding the
...
\emph{mean moderation}, adds an interaction term to the outcome
...
it [@simon2010; @freidlin2014]. The second, \emph{covariance
...
third, \emph{combined} architecture activates both channels
...
between treatment states estimates that participant's individual
treatment effect. An \emph{aggregated} N-of-1 trial pools many such
...
the differential in the correlation structure between treatment
states and weakens the signal that the analysis model detects. A
...
convergence rate. The number of replicates per cell is calibrated
...
($0.745 \to 0.754 \to 0.723$), within one MCSE and consistent with
no architecture-by-carryover interaction in that design. These
figures fall below the 40-60\% relative-loss range suggested by
earlier exploratory runs for the MVN differential-correlation DGP,
but confirm their qualitative finding of substantial,
...
moderation. Both mechanisms compress the between-state treatment
contrast and are shared by the two architectures. What mean
...
noise structure is independent of drug state. The interaction term
therefore has reduced variance (\texttt{Dbc} has less contrast
between drug states), but its signal-to-noise ratio degrades only
modestly.
...
is only a proxy for a hidden responder state that it indexes
...
treatment effects should state explicitly whether the interaction
is generated through mean moderation, covariance moderation, or
both channels simultaneously; the biological rationale for the
chosen mechanism; whether any carryover model operates on the
interaction signal consistently with that choice; and, when the
mechanism is uncertain, sensitivity analyses under the alternative
architecture. These requirements specialize the Data-generating
...
of the loss: restricting the analysis to on-drug points, weighting
...
results are calibrated to a single reference application
