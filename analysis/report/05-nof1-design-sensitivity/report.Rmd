...
\begin{claudecode}
Each sweep sources `00-common.R` for fixtures, sets a seed via
...
trajectories and the `corCAR1` residual structure absorbs the
...
shrinkage $1 / (1 + (k - 1) \rho)$, even when `corCAR1` is
...
correlation and the parallel-group analysis fits `corCAR1`, the
...
decaying `Dbc` in the analysis absorbs carryover correctly) but
...
an lme with an unstructured residual; fitting `corCAR1` at only
...
`corCAR1` fitted absorb serial correlation efficiently, while
...
correlation via `corCAR1` at the aggregated level. Our parallel
...
Sweep S8 was conducted with the analysis-side `corCAR1` always
enabled. A complementary with-versus-without-`corCAR1`
factorial would be informative and would require adding an
`op$use_corCAR1` option to `lme_analysis` followed by rerunning
the sweep.
...
between-subject analysis that fits `corCAR1` on a small sample,
...
high DGP AR(1) under analysis-side `corCAR1` misspecification,
...
`parallel::mclapply` in 191 seconds, well inside the 30-minute
wall-time budget. Convergence was 100% across all 7,500 fits.
Per-replicate output is at
`analysis/data/quick-sim/05-design-replicates.rds` with the
Morris ADEMP summary at `05-design-summary.txt`. With 500
...
`corCAR1` at small per-subject timepoint counts; the modest but
...
calls `set.seed(42 + sweep_id)` before constructing its
parameter grid, so the parameter grid and any pre-loop draws
are reproducible per sweep. At the replicate level, each driver
sets `set.seed(20260507)` at the top of the replicate loop and
uses `parallel::mc.set.seed` inside `mclapply`; per-replicate
