...
| `02-summarise-grid.R` | Aggregate replicate-level results into power and type-I-error grids; writes `output/02-grid-summary.rds` and `analysis/data/02-grid-summary.rds` |
| `03-render-figures.R` | Generate PDF figures into `analysis/figures/` |
...
Rscript analysis/scripts/carryover-sensitivity/01-run-factorial.R --smoke

# Development run (50 replicates per cell, minutes)
Rscript analysis/scripts/carryover-sensitivity/01-run-factorial.R --dev
...
- Summarised grid: `output/02-grid-summary.rds` and mirrored into
  `analysis/data/02-grid-summary.rds` (small; committed).
- Figures: `analysis/figures/02-*.pdf` (committed).
