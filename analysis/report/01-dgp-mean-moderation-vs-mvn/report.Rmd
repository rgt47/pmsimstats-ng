...
formulations [@pmsimstats-paper03] locates mean moderation and
covariance moderation as two points on a five-point spectrum
spanning pure mean moderation, combined mean-and-covariance
moderation, location-scale (distributional) moderation,
heterogeneous random-effects moderation, and the fully latent-class
generative form from which covariance moderation is derived as a
second-moment approximation, and it identifies three regimes in
which that approximation breaks down. When the biomarker is a
...
OL+BDC and smallest in CO, follows from a further property of
covariance moderation worth making explicit in mechanistic terms.
The architecture places the entire biomarker-treatment signal in a
correlation that is estimated fresh, from currently observed
drug-exposure state, at every timepoint, with no persistent
per-participant memory of biomarker-linked responder status once
off-drug attenuation has run its course. A genuinely latent-class
mechanism does not share this property: the class label is fixed
for each participant across the whole trial and is, in principle,
recoverable from the full within-subject trajectory even at
timepoints where the instantaneous drug-exposure contrast has
decayed to near zero [@pmsimstats-paper03]. Covariance moderation is
therefore best read as characterizing the power available to the
standard continuous-interaction analysis model of Section 2.1 when
the true mechanism is close to its covariance-only limit, not as an
upper bound on what any analysis could recover from a latent-class
truth. Whether a class-aware analysis can close part of that gap in
practice is bounded by mixture-identifiability limits that bind hard
at the $N \in [30, 150]$ range typical of N-of-1 biomarker
applications [@pmsimstats-paper03], so the design implication drawn
in Section 4.4, that designs with long off-drug windows carry the
largest architecture-dependent risk, should be read as a caution for
trials analyzed with standard mixed-model methods rather than as a
statement that latent-class biology makes long off-drug windows safe
by construction.
