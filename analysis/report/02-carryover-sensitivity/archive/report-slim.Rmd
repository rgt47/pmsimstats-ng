...
carryover (E1); an exposure-weighted continuous indicator that
assumes a parametric decay and half-life (E2, the current default);
and the binary indicator augmented by a lagged just-off-drug term
(E3, the classical crossover device). We embed them in an
...
**Results.** The exposure-weighted specification E2 does not
...
blinded-discontinuation phase, where its advantage over E1 is about
10 percentage points (e.g.\ 0.860 vs 0.766 at the reference cell).
Under the classical crossover design E2 is markedly inferior (0.488
vs 0.830 for E1), and under the mean-moderation architecture (A) it
is marginally dominated throughout; across the grid E2 is the
most powerful specification in only 19% of cells. Where E2 does
...
where E2 with a cluster-robust standard error is recommended, but
...
binary (E1), exposure-weighted (E2), and lagged-augmented (E3) --
across decay forms and both architectures.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item E1 ignores carryover; E2 commits to a parametric decay and
half-life; E3 estimates carryover non-parametrically via a lagged
term.
...
carryover (**E1**, the strict repeated-measures analogue); an
exposure-weighted continuous indicator that commits to a parametric
decay form and half-life (**E2**, the current default); and the
binary indicator augmented by a lagged just-off-drug term that
estimates carryover non-parametrically (**E3**, the classical
...
\item E1, E2, E3 differ only in how $X$ is constructed.
\end{itemize}
:::
...
::: {.bullets data-latex=""}
\textbf{E2 uses a continuous exposure-weighted predictor committing
...
::: {.bullets data-latex=""}
\textbf{E3 adds a lagged just-off-drug term but, because that term
is only a nuisance main effect, it is structurally E1 plus a
control variable.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
...
to E1.
\item E3 therefore differs from E1 only through variance $L$
...
references.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item E2 is effectively a different model for each $\hat{t}_{1/2}$.
\item The mis-specification risk falls entirely on off-drug points.
\item E1 and E3 carry no half-life parameter, so they are invariant
...
7. **Non-convergence rate** per cell, defined as the proportion
   ...
   the bm:X coefficient was not identifiable (e.g.\ E1 on OL
...
E2-vs-E1 can be compared trial by trial (a paired test).
...
This is why we can later compare E2 and E1 with a *paired* test --
...
$\theta_{\text{true}} = -c_{bm}\sigma_{BR} = -3.6$, but E1/E3
estimate a different coefficient ($\text{bm:}D_{it}$).
\item So E1/E3's apparent bias and under-coverage are an
...
grows, but E2's lead is confined to Architecture B's non-crossover
designs.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item All three are near-identical at $t_{1/2} = 0$.
\item E2's advantage emerges at $t_{1/2} = 0.5$ and grows under
Architecture B in the Hybrid and OL+BDC designs; E1 and E3 are
indistinguishable.
\item The advantage does not appear under the CO design (where E2 is
...
specification E2 retains a modest but consistent advantage
...
through $t_{1/2} = 1.0$ weeks; E1 and E3 are effectively
indistinguishable. This advantage is confined to those strata: it
does not appear under the crossover (CO) design, where E2 is in
fact dominated (Section 5), nor under Architecture~A. The figure
therefore shows that the E2 advantage, where present, is not an
...
$0.860 \pm 0.016$.
\item Same row, other columns: penalty of E1/E3 under an exponential
DGP.
...
::: {.bullets data-latex=""}
\textbf{Block S6 re-fits E2 with a CR2 cluster-robust SE to correct
...
fixes the calibration. Table \@ref(tab:tab-s6) re-fits E2 with both
...
the E2 advantage is a comparison among correctly-sized tests.
\end{itemize}
:::
...
where all three specifications hold nominal $\alpha$, the E2
...
\item The E2$-$E1 gap is held or widened, at the cost of far fewer
...
power. Crucially, the E2-minus-E1 gap is held or slightly widened,
...
the crossover design or Architecture~A where E2 does not lead.
:::
...
(range 0.766-0.964).
\item E1 and E3 are invariant to the assumed half-life (average
0.840).
...
narrows E2's advantage as off-drug observations are dropped.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item At 30\% missing-completely-at-random (MCAR) dropout all three fall to about 0.12-0.15, effectively
tied.
\item The E2$-$E1 gap shrinks from $\approx 0.10$ (no dropout) to
$\approx 0.03$ (30\% MCAR).
...
\item E2$-$E1 peaks at $+0.102$ at $c_{bm} = 0.45$, shrinking to
$+0.042$ at $c_{bm} = 0.60$.
...
::: {.bullets data-latex=""}
\textbf{S5: the E2 advantage is constant across the
$\rho \times t_{1/2}$ grid, confirming additive (not interactive)
effects.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item E2$-$E1 $= 0.096$ at both $\rho = 0.5$ and $\rho = 0.8$ when
$t_{1/2} = 1.0$.
...
::: {.bullets data-latex=""}
\textbf{A higher-precision paired-McNemar rerun confirms E2's lead
is real (p $= 6.6 \times 10^{-17}$ at the strongest cell).}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item 24-cell OL+BDC rerun at 600 trials/cell, all converging.
\item Paired McNemar (same trials) at $N = 70$, half-life 1.0: E2
beats E1 by 13.7 points, $p = 6.6 \times 10^{-17}$.
\item E3 recovers only part of E2's advantage; Type I stayed within
$[0.003, 0.053]$.
\end{itemize}
:::
...
half-life 1.0) E2 beats E1 by 13.7 points (0.737 vs 0.873) with
McNemar $p = 6.6 \times 10^{-17}$: the lead is real, not noise. The
shape-free method E3 recovers only part of E2's advantage, and only
...
::: {.bullets data-latex=""}
\textbf{E2's advantage is real but conditional: highest in only
19\% of cells, dominated under CO and under Architecture A.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Best cell (Arch B, Hybrid): E2 beats E1/E3 by $\approx 10$
points ($\approx 5$ MCSEs).
\item E2 is the best method in only 68 of 360 non-null cells; under
...
::: {.orig data-latex=""}
The headline is that E2's advantage is real but **conditional**.
At its best cell (Architecture B, Hybrid design, $N = 70$,
$c_{bm} = 0.45$, $t_{1/2} = 1.0$) E2 beats E1 and E3 by about 10
percentage points of power (0.860 vs 0.766 and 0.770) -- about five
MCSEs, so not luck. But E2 is the best method
in only 68 of the 360 cells with a real effect (19%). In the
crossover design it is much *worse* (0.488 vs 0.830 for E1), and
...
The ranking "E2 best" therefore holds only under Architecture B in
...
checks.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Decay shape: E2 never drops below E1/E3 even when the assumed
form is wrong (worst case the linear truth).
\item Assumed half-life: a two- to four-fold error moves E2 only
from 0.83 to 0.77, never below E1/E3.
\item Autocorrelation: lifts all three equally, so E2's 5-6 point
...
method falls to 0.12-0.25 and they become indistinguishable. E2
...
::: {.bullets data-latex=""}
\textbf{E2's advantage exists only under Architecture B, because
only there does carryover erode a correlation for E2 to recover.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Architecture B is more carryover-sensitive under every decay
form and Weibull shape.
\item Under Architecture A, E2 is marginally negative everywhere
(e.g.\ Hybrid, $t_{1/2} = 1.0$: 0.960 vs E1 0.972).
\item Under A the effect stays proportional, so there is no
attenuated correlation to recover and the binary E1 is preferable.
\end{itemize}
:::
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item E2 power 0.860 reproduces Hendrickson's 0.82; the new content
is the E1/E2/E3 comparison.
\item The Jones-Kenward expectation that a shape-free E3 beats E2
...
::: {.orig data-latex=""}
Our matched-decay (E2) power at the headline cell, $0.860 \pm
...
of E1, E2, and E3, which Hendrickson did not do. The crossover
tradition of Jones and Kenward [-@jones2014crossover] predicted
that a shape-free lagged term (E3) should beat a parametric
predictor (E2) when the decay shape is guessed wrong; our S2 result
does not support this in the range we tested, because E2's penalty
for a wrong half-life is too small for E3 to overtake it. Senn's
...
1. **E2 helps, but only conditionally.** The exposure-weighted
   predictor gives the highest power -- about 10 points over E1 --
   *only* under Architecture B in the Hybrid and OL+BDC designs,
   ...
   Architecture A, E2 has no advantage and E1 is at least as good.
   Where E2 is used, report it with a CR2 cluster-robust standard
...
3. **The shape-free method E3 is essentially E1.** Because its
   lagged term is only a nuisance control, E3 tracks E1 closely and
   never overtakes E2 in the settings we tested.
