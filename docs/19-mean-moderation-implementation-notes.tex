...
To address the zero-base problem, the formulation was changed to an additive
...
2025 tidyverse codebase (\texttt{pmsimstats2025/analysis/
scripts/pm\_functions.R}, initial commit \texttt{957c8bd};
this codebase has since been moved to an external archive at
\texttt{\textasciitilde/Dropbox/prj/alz/10-pmsimstats-ng/archive/2025/}
and is no longer part of the working repository):
%
\begin{equation}
  \text{BR}_{it}^{\text{mod}} = \text{BR}_{it}^{\text{base}}
...
\item \textbf{Centering.} The standardized biomarker
  ...
  differential BR. This is the correct behavior: the intent is to model
  an \emph{interaction} (differential response by
...
conditional expectation, the formulation ensures that $c_{bm} = 0.45$
...
\subsection{Known limitation: parameter validation}

The \texttt{validateParameterGrid()} function, called at the
start of \texttt{generateSimulatedResults()}, constructs
sigma matrices using \texttt{buildSigma()} to check for
positive definiteness before the simulation begins. This
check currently uses the Architecture~B covariance structure
regardless of the \texttt{dgp\_architecture} setting. Under
Architecture~A with $c_{bm} > 0.45$, the function will
report spurious PD failures and print a warning, even though
the actual simulation runs without issue because the BM-BR
correlation block is omitted from the sigma matrix used for
data generation.

This warning can be safely ignored when running
Architecture~A. A future revision should pass
\texttt{dgp\_architecture} to
\texttt{validateParameterGrid()} so that the PD check uses
the correct sigma structure.

\section{Positive definiteness and the $c_{bm}$ ceiling}

\subsection{The constraint under Architecture B}

Under Architecture~B, $c_{bm}$ enters the off-diagonal
entries of the covariance matrix as a correlation between the
biomarker and each BR timepoint column. Combined with the
within-factor autocorrelation ($\rho = 0.8$ in the
publication), cross-factor correlations ($c_{\text{cf1t}} =
0.2$, $c_{\text{cfct}} = 0.1$), and the full set of
timepoint columns, the matrix becomes non-positive-definite
when $c_{bm}$ exceeds approximately 0.45. The Hendrickson
publication used $c_{bm} \in \{0, 0.3, 0.6\}$, but the
$c_{bm} = 0.6$ condition fails PD checks for 7 of 9 trial
paths at zero carryover and for all 9 paths at higher
carryover values. The revised analysis in
\texttt{04-revised-power-analysis.tex} was forced to reduce the
grid to $c_{bm} \leq 0.45$ and the autocorrelation to 0.7.

\subsection{Architecture A removes the constraint}

Under Architecture~A, the BM-BR correlation block is absent
from the covariance matrix. The parameter $c_{bm}$ enters
only in the post-draw additive moderation step and does not
affect the eigenvalue structure of $\Sigma$. Testing
$c_{bm}$ from 0 to 0.95 in increments of 0.05 across all
four trial designs and three carryover half-lives:

\begin{center}
\begin{tabular}{lcc}
\toprule
& \textbf{Arch B (MVN)} & \textbf{Arch A (mean mod)} \\
\midrule
First PD failure & $c_{bm} = 0.50$ & None through 0.95 \\
Feasible ceiling & $\sim$0.45 & Unconstrained \\
Paths failing at 0.60 & 7/9 & 0/9 \\
Paths failing at 0.95 & 9/9 & 0/9 \\
\bottomrule
\end{tabular}
\end{center}

This is a direct consequence of removing the BM-BR entries
from $\Sigma$: the matrix dimension and sparsity pattern are
the same, but the off-diagonal entries that drove the
smallest eigenvalues negative are gone.

\subsection{Hendrickson publication parameters under
  Architecture A}

With the PD constraint removed, it is possible to evaluate the original
Hendrickson publication parameter grid: $c_{bm} \in \{0,
0.3, 0.6\}$, autocorrelation $\rho = 0.8$, carryover
$t_{1/2} \in \{0, 0.1, 0.2\}$ weeks, $N \in \{35, 70\}$.
Results at 50 replicates per cell, no censoring, analysis
model without carryover term:

\begin{center}
\begin{tabular}{llccc}
\toprule
\textbf{Design} & $N$ & $t_{1/2} = 0$ & $t_{1/2} = 0.1$
  & $t_{1/2} = 0.2$ \\
\midrule
\multicolumn{5}{l}{\textit{Power at $c_{bm} = 0.6$}} \\
...
\midrule
\multicolumn{5}{l}{\textit{Power at $c_{bm} = 0.3$}} \\
N-of-1 & 70 & 0.48 & 0.38 & 0.50 \\
OL+BDC & 70 & 0.36 & 0.36 & 0.34 \\
CO     & 70 & 0.26 & 0.20 & 0.28 \\
\midrule
\multicolumn{5}{l}{\textit{Type I error ($c_{bm} = 0$)}} \\
All designs & 35--70 & \multicolumn{3}{c}{0.00--0.08
  (nominal)} \\
\bottomrule
\end{tabular}
\end{center}
...
\item \textbf{Carryover resilience is confirmed.} Power is
  stable across all three carryover half-lives for every
  design. The variation (e.g., N-of-1 at $N = 70$: 0.98,
  0.96, 0.96) is within Monte Carlo error at 50 replicates.

\item \textbf{Design ranking is preserved.} N-of-1 $>$
  OL+BDC $>$ CO $>$ OL, consistent with the original
  Hendrickson findings and the revised Architecture~B
  analysis.

\item \textbf{$c_{bm} = 0.6$ is now evaluable.} The N-of-1
  design at $N = 70$ achieves 0.96--0.98 power, confirming
  that the original publication's target effect size is
  well-powered under this design. This result was previously
  unobtainable because the parameter failed PD validation
  under Architecture~B.

\item \textbf{OL design has low power} even at $c_{bm} =
  0.6$, because the all-on-drug design has no off-drug
  contrast for the \texttt{bm:Dbc} interaction term.

\item \textbf{Type I error is controlled} at 0--8\%
  across all conditions, consistent with the nominal 5\%
  level given Monte Carlo variability at 50 replicates.
...
{\footnotesize
Rendered on 2026-04-09 at 21:49 PDT.\\
