...
For each alternative we compute power, Type I error, and the
sampling distribution of the biomarker-treatment interaction
estimate at trial-relevant sample sizes against simulated data drawn
from the alternative family but analyzed under the standard
linear-mixed model.
...
inflation, and the estimator was unbiased there (mean
$\hat\beta_{bm:D}$ within 0.004 of zero in every null cell). At the
alternative the mean interaction estimate was family-invariant
($-0.230$ to $-0.239$ across the eight cells, a spread within Monte
Carlo noise) with a near-constant empirical standard error, so the
power separation is a precision effect rather than a shift in the
estimate. Bias at the alternative is not quantified: the
matched-anchor calibration fixes the marginal effect size across
families but yields no closed-form true $\beta_{bm:D}$ on the outcome
scale to difference against.
