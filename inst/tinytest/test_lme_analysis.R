...

{
  ...
  expect_true('beta' %in% names(result),
              info = 'lme_analysis result has beta column')
  expect_true('betaSE' %in% names(result),
              info = 'lme_analysis result has betaSE column')
  expect_true('p' %in% names(result),
              info = 'lme_analysis result has p column')

  if (!is.na(result$p)) {
    expect_true(result$p >= 0 && result$p <= 1,
                info = 'lme_analysis p-value is in [0, 1] (OL)')
  }
}

{
  ...
  expect_true('beta' %in% names(result),
              info = 'lme_analysis CO: result has beta column')
  if (!is.na(result$p)) {
    expect_true(result$p >= 0 && result$p <= 1,
                info = 'lme_analysis p-value is in [0, 1] (CO)')
  }
}
