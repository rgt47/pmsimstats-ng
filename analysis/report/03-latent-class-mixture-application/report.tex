...
regression assumptions of identical agents were known to fail. Heckman
and Singer\textsuperscript{\citeproc{ref-heckmansinger1984}{26}}, in an
...
DeSarbo\textsuperscript{\citeproc{ref-wedeldesarbo1995}{29}} generalised
the approach to generalised linear models, and Jedidi and
...
modelling strategies and presented an applied analysis of psychiatric
comorbidity.
...
latent class indicator. Zhang and
...
\begin{orig}
This paper has four aims. The first is to formalise the latent-class and
...
regression mixtures); a biomarker-moderation spectrum section organises
...
references the five-study simulation programme described in the
companion simulation plan, which constitutes the empirical core of the
present line of work and is the next deliverable in the programme.
...
the realised gap below the bound or, in adverse identifiability regimes,
reverse its sign.
...
an analysis-strategy programme (class-aware mixture analyses where
...
biologically more plausible than a continuous one, designs that maximise
the per-participant contrast and minimise carryover may gain power that
...
sub-study, and a deviation-log specification. Subsequent production runs
are diagnosed against these pre-registered estimands rather than against
any post-hoc wrapper choice.
...
cost ratio, consistent with the smoke-test estimate). \texttt{lcmm}
convergence rate is 96\%; the four non-converging replicates encountered
numerical issues during the gridsearch initialisation.
...
The gating coefficient is centred at zero (median \(-0.10\), sign
...
finite-sample identifiability artefact predicted by
the\textsuperscript{\citeproc{ref-bauercurran2003}{1}}
and\textsuperscript{\citeproc{ref-lubkemuthen2005}{2}} overextraction
...
hours per arm on 8 cores. The full programme is feasible on a single
...
continues to favour \texttt{ng=1} and the conditional procedure does not
fire.
