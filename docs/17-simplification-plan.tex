...
\begin{center}
\fbox{\parbox{0.95\textwidth}{\textbf{Status: Exploratory; not
adopted (2026-04-09).} This document explores simplification
paths for the simulation framework (alternative covariance
structures, summary-statistics replacements for the full LME
fit, and so on). The 2026-04-08 merge took a different path
from the proposals here: instead of simplification, it added
the Architecture~A (mean moderation) DGP alongside the existing
Architecture~B (MVN) DGP and reorganized the codebase into the
\texttt{implementations/} directory. The references to
\texttt{pmsimstats2025} in this document point to the
pre-merge exploratory pipeline, which has been moved to an
external archive. This document is retained as a record of
considered alternatives and may inform future refactor
decisions.}}
\end{center}
