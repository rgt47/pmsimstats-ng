...
```{r matched-baseline-placeholder, eval = FALSE}
# Pending simulation run.
# Load from ../data/02-matched-baseline.rds and render as a table
# parallel to the Architecture B power table in docs/02 Section 3.1.
```

## Cost of analysis-model mis-specification

*(Pending. Off-diagonal cells quantify the power penalty of
...
## Cost of DGP mis-specification under a fixed analysis

*(Pending. Holding the analysis specification fixed at A2
...
## Robustness of the lagged-treatment specification

*(Pending. The A3 column of the grid quantifies whether
estimating carryover as a nuisance is more robust than
assuming a specific parametric form. Classical crossover-trial
intuition [@jones2014crossover] predicts A3 is more robust to
...
*(Pending. Full grid under Architecture A versus Architecture
...
*(Pending. $c_{bm} = 0$ grid. Target: all cells nominal.
