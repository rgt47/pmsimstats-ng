...
title: "Carryover representation and inference for biomarker-treatment
  interaction tests under differential-correlation moderation: a
  simulation comparison of nine analysis specifications"
...
Blocks S7-S10 (Section 3.6). A short-code layer, `G1`-`G9`, is used
for the full nine-way comparisons in Blocks S7-S10 and is defined in
`spec-labels.R`; the Tier 1 three and Tier 2 discussion elsewhere in
this manuscript refer to them by their `E`-prefixed stored codes
(`E1`, `E2`, ...) or by name, per Table \@ref(tab:tab-spec-summary)
below.
...
and the fit with the lowest AIC [-@akaike1974new] is kept, mirroring
...
extension of the paired t-test Senn, Julious and Araujo
[-@senn2014four] found to outperform carryover-adjusted mixed models
...
standard error (Bell and McCaffrey [-@bell2002bias], via `clubSandwich`, paired with
Satterthwaite degrees of freedom) restores nominal size without
disturbing the ranking among G1-G3. That correction, `lme+CR2` in
...
`E5`/`E6` were replaced with two specifications answering different
open questions about the same underlying problem, the analyst's
unknown true decay half-life. `E7` asks whether estimating the
half-life from the data, by AIC selection over a candidate grid
(reusing the logic of `characterize_carryover()`,
`R/carryover_analysis.R`, already used for real-data analysis in this
...
estimation avoids needing the assumption at all. `E9` asks a more
basic question raised directly by the wider literature: is a
conditional mixed model even necessary. Senn, Julious and Araujo
[-@senn2014four] compared four analysis strategies for ordinary (no
...
when carryover is specifically suspected. `E9` is a direct
...
This remains a bounded comparison. `E7` and `E9` were chosen because
...
```{r fig-heatmap-type1, echo = FALSE, fig.cap = "Same data as the boxplot above, as an annotated heatmap of the mean Type I error per design x specification cell (diverging scale centered at nominal 0.05: blue = conservative, red = anti-conservative).", out.width = "60%"}
knitr::include_graphics("../../figures/02xs-heatmap-hendrickson-d.pdf")
```
...
```{r tab-type1-cr2-g9, echo = FALSE}
suppressPackageStartupMessages({library(dplyr)})
s6g9 <- readRDS(here::here(
  'analysis/scripts/carryover-sensitivity/output/18-sensitivity-s6-g9.rds'))
kappa_tab <- s6g9$results |>
  filter(cell == 'Reference (N=70)', arm == 'null') |>
  group_by(spec) |>
  summarise(kappa = mean(std_error, na.rm = TRUE) / sd(estimate, na.rm = TRUE),
           .groups = 'drop')
tab9 <- s6g9$summary |>
  filter(cell == 'Reference (N=70)', arm == 'null') |>
  left_join(kappa_tab, by = 'spec') |>
  mutate(Code = g_code[spec],
        Method = unname(g_labels[spec])) |>
  mutate(o = match(Code, paste0('G', 1:9))) |>
  arrange(o) |>
  transmute(Code, Method,
           `Type I error` = sprintf('%.3f', power),
           `$\\kappa$` = sprintf('%.2f', kappa),
           `Non-conv` = sprintf('%.3f', non_convergence_rate))
knitr::kable(tab9, booktabs = TRUE, escape = FALSE,
  caption = 'Type I error and calibration ratio $\\kappa$ for all nine
...
```{r fig-heatmap-sens-s6-g9-null, echo = FALSE, fig.cap = "Type I error for all nine G1-G9 specifications across the five S6 stress cells (diverging scale centered at nominal 0.05; preliminary $n_{sim} = 100$).", out.width = "90%"}
knitr::include_graphics("../../figures/02xs-heatmap-sens-S6-g9-null.pdf")
```
...
```{r fig-heatmap-type1-cr2, echo = FALSE, fig.cap = "Same data as the dot plot above, as two annotated heatmaps (model-based, CR2) sharing one diverging scale centered at nominal 0.05.", out.width = "48%", fig.show = "hold"}
knitr::include_graphics(c("../../figures/02xs-heatmap-sens-S6-model.pdf",
                          "../../figures/02xs-heatmap-sens-S6-cr2.pdf"))
```
...
```{r fig-heatmap-sens-s3, echo = FALSE, fig.cap = "Block S3: power by dropout rate and analysis specification, faceted by dropout mechanism, same data as the line plot above. The MAR arm has no $0\\%$ cell (Section 3.6); it shares the MCAR reference cell.", out.width = "80%"}
...
mostly adds estimation noise. `E5`/`E6` were replaced by two
specifications addressing the same open problem, the analyst's
unknown decay half-life, from different angles (Section 2.4): `E7`
(AIC-selected half-life) and `E9` (paired-difference regression).
...
already-established Section 3.3 null replicated once more). `G4`'s
...
immediate neighbor (`r round(100 * mean(readRDS(here::here('analysis/scripts/carryover-sensitivity/output/09-sensitivity-s7.rds'))$results$best_t_half[readRDS(here::here('analysis/scripts/carryover-sensitivity/output/09-sensitivity-s7.rds'))$results$spec == 'E7'] %in% c(1.0, 2.0), na.rm = TRUE))`% of replicates choose $1.0$ or $2.0$ weeks), close enough to
the truth that `G4` slightly exceeds `G3`'s own oracle-half-life
power at this cell. That `G5`, which discards the repeated-measures
structure, the correlation model, and any carryover representation
entirely, still beats `G1`/`G2` is the more striking result, echoing
Senn, Julious and Araujo's [-@senn2014four] finding that simplicity is
...
```{r fig-heatmap-sens-s8b, echo = FALSE, fig.cap = "Block S8, Panel B (Architecture B, covariance moderation): power by carryover half-life (rows) and analysis specification (columns). Same data as the Architecture B rows of the table above. Not directly comparable to Panel A in absolute terms (Section 3.6).", out.width = "80%"}
...
every design examined under mean moderation (Section 4.6); `E7`
inherits that same architecture-sensitivity since it is built on
Exposure-weighted's formula. `E9`, built on nothing but a raw
...
to `E7`. `E9` is the one specification in this table whose advantage,
...
`G4` in isolation. `G5`'s milder reversal has no comparable mechanical
...
### S10: does a CR2 correction change the ranking among G1-G5?
...
Section 3.6's power results, this means `G9`'s status as the
...
- **Covariance moderation (Architecture B), Hybrid or OL+BDC
  ...
  default: it has been through the full robustness battery `E7`/`E9`
  have not (next point).
- **`E7` (AIC-selected half-life) and `E9` (paired-difference) are
  promising at this same cell, not yet validated defaults.** Both
  beat Exposure-weighted itself at the Hybrid, Architecture B
  reference cell (Blocks S7-S8: `E7` $0.850$, `E9` $0.842$, `E2`
  $0.836$, at $t_{1/2} = 1.0$), and `E7` in particular removes the
  need to assume a half-life at all. Neither has been tested against
  decay-shape mis-specification, dropout, or under CR2 the way `E2`
...
- **Crossover (CO) design, any architecture.** Use Unadjusted.
  ...
  Block S9 shows `E7` and `E9` fail here too, `E7` catastrophically.
  `E7`'s AIC half-life selection systematically drifts to the largest
  ...
  widely spaced off-drug observations. `E9` also underperforms
...
- **Architecture uncertain, or known to be mean moderation
  ...
  Exposure-weighted or `E7`. Both specifications' advantage under
  covariance moderation reverses, marginally, under mean moderation
  (Block S8). `E9` is the one specification whose performance does
...
- **Minimize dropout before optimizing specification choice.** It is
  ...
  specification, including `E7`/`E9`, has been shown to offer any
  protection against it; their behavior under dropout is untested.

### Why E7 and E9 succeed at Hybrid but fail under CO
...
`E5`/`E6` (Section 4.6). `E7`'s failure is mechanical: with only a
...
of half-life error when the half-life is *fixed* by the analyst;
under CO, `E7` reproduces that same damaging error through the
estimation procedure itself, on nearly every replicate. This is a
concrete, testable prediction: `E7` should be reliable whenever a
...
`E9`'s failure has no comparable mechanical account and is milder.
...
`E9` against the within-cell decomposition of `E2`'s advantage to
confirm.
...
at all, and neither `E7` nor `E9` was tested against a DGP built to
...
both `E7` and `E9` are candidates worth validating further within
...
specifications addressing the same open problem, the analyst's
unknown decay half-life, from different angles: `E7` (AIC-selected
half-life) and `E9` (a paired-difference regression with no
carryover representation at all). Both beat Unadjusted at the
Hybrid, Architecture B reference cell, `E7` even edging out
Exposure-weighted itself (Block S7), but neither survives a change of
trial design or architecture (Blocks S8-S9): `E7`'s AIC selection
fails specifically under sparse off-drug sampling (Section 4.5), and
`E9`'s advantage appears to depend on the repeated-measures structure
...
moderation. Exposure-weighted's advantage, and now `E7`'s, are
...
specification within our existing pipeline. Tang and Landes
[-@tang2020ttests] propose formula-based $t$-tests that correct a
...
natural refinement `G5` lacks (`G5` discards within-patient
correlation entirely); their tests have no biomarker-moderation term
and would need a `diff ~ bm` extension with a serial-correlation-
aware variance to enter our comparison. Schmid and Yang
[-@schmid2022bayesian] describe a Bayesian hierarchical alternative
to the frequentist mixed-model and sandwich-estimator family
spanning `G1`-`G9`, with partial pooling of individual-level
...
extension of their model is a separate undertaking. Daza
...
from `G1`-`G9`'s explicit exponential-decay or lag covariates:
...
7. **Carryover terms interacted with the biomarker do not help; two
   ...
   half-life, differently: `E7` estimates it by AIC selection; `E9`
   ...
   at the Hybrid, Architecture B reference cell (Blocks S7-S8; `E7`
   $0.850$, `E9` $0.842$, `E2` $0.836$ at $t_{1/2} = 1.0$). Neither
   result generalizes. Block S8 shows both reverse, like
   Exposure-weighted itself, under Architecture A. Block S9 shows
   both fail under the crossover design: `E9` mildly, `E7`
...
S9 CO-design block
(`14-run-sensitivity-s9.R`, `output/14-sensitivity-s9.rds`), and the
...
and S11, which sets $c_{bm} = 0$). `E5`/`E6`
...
replaced by `E7` (AIC-selected half-life, reusing the candidate-grid
logic of `characterize_carryover()`, `R/carryover_analysis.R`) and
`E9` (paired-difference regression). The `E1`-`E3`, `E7`, `E9`
...
in S10's table and figure; `G8` is `E2`+CR2, reused directly from
...
for `G8` and Panel S6), and Panel D of `12-hendrickson-heatmap.R`
converts the Tier 1 Type I error boxplot to a heatmap from the same
`02-grid-summary.rds` used for Figures
