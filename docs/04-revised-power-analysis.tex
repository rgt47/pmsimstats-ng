...
\newcommand{\doctitle}{Revised Power Analysis for
  Aggregated N-Of-1 Trial Designs:
  Correcting the Carryover Sensitivity
  Estimate and Covariance Structure}
\newcommand{\shorttitle}{Revised Power Analysis}
...
\textbf{Architecture scope.} This document describes the revised
power analysis under the MVN differential correlation approach
(Architecture~B), in which the biomarker-treatment interaction is
encoded in the covariance structure. The positive definiteness
constraints, carryover sensitivity findings, and parameter ceilings
reported here are specific to Architecture~B. Under Architecture~A
(direct mean moderation), the PD constraint is removed and power is
stable across carryover levels. For a comparison of both DGP
architectures, see \texttt{02-dgp-mean-moderation-vs-mvn.tex}. For
implementation details of Architecture~A, see
...
An audit of the simulation code reveals that this dramatic
power drop is primarily an artifact of how the simulation
assigns correlations during off-drug periods, not a genuine
consequence of carryover at those half-lives. The analysis also identified
...
The analysis began by extracting the original simulation code and
...
The simulation was instrumented to count how often the
...
Source: \textasciitilde/prj/alz/10-pmsimstats-ng/pmsimstats-ng/docs/04-revised-power-analysis.tex}
