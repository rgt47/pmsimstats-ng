...
per-replicate results so that Monte Carlo standard errors can be
...
Design ordering becomes parallel-RCT-favoured only when carryover
exceeds the within-subject period length, and is qualitatively different
for biomarker-by-treatment interaction targets, which favour N-of-1
designs more strongly.
...
incompletely characterised. That is, the published literature provides
...
pmsimstats-ng tidyverse pipeline that operationalises this design in R,
...
the biomarker-by-treatment interaction examined in the original
Hendrickson study and in a separate companion paper.
...
components, although the section headers are organised by topic rather
than relabelled to the ADEMP acronym.
...
\begin{orig}
The simulation study has three aims. First, to characterise the power
...
sufficient replication to provide Monte Carlo standard errors below 1\%
per Morris et al.\textsuperscript{\citeproc{ref-morris2019using}{1}}.
...
render time with Morris Monte Carlo standard errors. Sweep scripts are
in \texttt{analysis/scripts/nof1-design-sensitivity/}.
...
crossover), labelled `Nof1' in the sweep artifacts and figures below.
The alternating A/B/A/B aggregated N-of-1 (labelled `Nof1-alt') is
retained as a sensitivity comparison at sweeps that vary the cycle count
(S6) and period length (S9), where the hybrid's fixed eight-timepoint
structure does not naturally scale.
...
that is not erased by post-discontinuation washout, whereas the
...
slightly favours the RCT because each additional timepoint contributes
...
errors of the nominal 0.05 for the hybrid N-of-1 (0.036 to 0.056 across
the four cells). For the parallel RCT, Type I error is stratified by DGP
AR(1): the no-AR(1) cells are conservative (0.018 to 0.024) while the
high-AR(1) cells are inflated above nominal (0.112 to 0.126), with MCSE
0.014 to 0.015, so the inflation is statistically robust. Under the
alternative, the hybrid is saturated at 1.000 in all four cells; the
parallel RCT power is 0.986 to 0.990 when no AR(1) is present, and 0.766
to 0.772 when AR(1) \(\rho = 0.7\) is present.
...
crossovers should be analysed without serial-correlation structure,
...
Third, sensitivity to carryover depends on design flavour. The hybrid's
...
detecting a continuous biomarker-by-treatment interaction. At \(N = 35\)
...
period-length effects for the main drug effect. A key qualitative
...
modelling artefact. The 0.11 to 0.13 Type I error rates for the parallel
...
replicates per cell the Monte Carlo standard error on a power estimate
near \(0.3\) is approximately \(0.020\).
...
biomarker-by-treatment interactions than for the main effect, and the
reader should be cautious about generalising from one estimand to the
other. Third, the two-period crossover should not be analysed with
...
that comparison generalises along twelve parameter axes for the hybrid
