...
\item Huynh-Feldt and Greenhouse-Geisser corrections recover nominal Type I only conditionally; the conventional threshold favours Greenhouse-Geisser when $\epsilon < 0.75$ (Blanca et al., 2023).
...
conventional rule favouring Greenhouse-Geisser when
...
the biomarker-by-treatment interaction estimand, the aggregated
...
procedure. @blackston2019comparison compare aggregated N-of-1
trials against parallel and cross-over designs by simulation and
find the aggregated design more efficient, but analyse a single
mixed model and do not vary the test procedure. @schork2023precision
and @zucker2010combining synthesize the broader N-of-1 design
...
linear-mixed analysis with `corCAR1` avoids both costs, modelling
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item The five procedures are strict RM-ANOVA (dichotomised biomarker), a continuous-biomarker within-subject contrast, linear-mixed with \texttt{corCAR1}, GEE with naive sandwich, and GEE with Mancl-DeRouen small-sample correction.
\item The continuous-biomarker contrast separates the cost of dichotomisation from the cost of the test procedure proper; the four continuous-biomarker arms share a common interaction estimand.
...
@morris2019. The primary estimand is the biomarker-treatment
...
continuous biomarker (the dichotomisation-free analogue of the
...
median-dichotomised biomarker and so targets a dichotomised
estimand on a different scale. The continuous-biomarker contrast
is included specifically to separate the cost of dichotomisation
...
locate the procedures' behaviour in the standard-error
...
::: {.orig data-latex=""}
The dominant message is that the biomarker should be analysed as
...
study isolates, while modelling choices among the
...
requires no covariance modelling. Strict RM-ANOVA remains useful
...
artefact of its residual over-rejection rather than a real gain,
...
::: {.bullets data-latex=""}
\textbf{The fourth resolved question is that the linear-mixed conservatism is a degrees-of-freedom artefact a Kenward-Roger correction would remove.}
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
...
\item Sphericity violation under AR(1) bites only when the full multi-timepoint series is analysed by classical RM-ANOVA without a degrees-of-freedom correction; the linear-mixed model with \texttt{corCAR1} sidesteps both issues.
\end{itemize}
:::
...
`corCAR1` avoids by modelling the AR(1) structure directly. Both
...
data-generating process under Architecture A via `generateData()`
