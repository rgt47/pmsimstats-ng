...
\textbf{Architecture scope.} The $\lambda_{\text{cor}}$ decay
mechanism derived here is specific to Architecture~B (MVN
differential correlation), in which the biomarker-BR correlation
decays exponentially during off-drug periods. Under Architecture~A
(direct mean moderation), the biomarker-treatment interaction is in
the mean structure and does not involve correlation decay; the
$\lambda_{\text{cor}}$ parameter is unused. For a comparison of
...
A 200-replicate simulation with $\lambda_{cor} = 1$
