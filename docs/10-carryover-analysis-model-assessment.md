...
offers two options -- a continuous drug indicator (`Dbc`) that
bakes carryover into the treatment variable, and a separate `tsd`
covariate -- but the choice between them (or whether to include
carryover at all) has not been systematically evaluated.

---

## 2. The 2x2 Factorial Framework
...
|---|---|---|
| **DGP: no carryover** | Baseline (Type I error check) | Cost of unnecessary adjustment |
| **DGP: carryover present** | Cost of ignoring carryover | Correct specification |
...
- Type I error (rejection rate when $c_{bm} = 0$)
- Power (rejection rate when $c_{bm} > 0$)
- Bias of the treatment effect estimate
...
### 3.1 The definitive reference

Senn (2002), *Cross-over Trials in Clinical Research*, 2nd
edition, provides the authoritative treatment. The key conclusions
for simulation design:

1. **The two-stage procedure is invalid.** The older approach --
...
2. **The modern consensus:** If carryover is scientifically
...
3. **For simulation studies:** Generate data under a range of
...
- Jones & Kenward (2014), *Design and Analysis of Cross-Over
...
- Zucker et al. (1997), 'Combining single patient (N-of-1)
...
- Araujo et al. (2016), 'Understanding variation in sets of
...
## 4. How This Maps to pmsimstats

### 4.1 Carryover in the DGP
...
### 4.2 Carryover in the analysis model

The current codebase offers two approaches:

**Approach A: Continuous drug indicator (`Dbc`).** The binary
on/off-drug indicator is replaced with a continuous version:

- On drug: $Dbc = 1$
- Off drug: $Dbc = (1/2)^{s \cdot t_{sd} / t_{1/2}}$

The model formula becomes:
`Sx ~ bm + t + Dbc + bm:Dbc`, with the interaction `bm:Dbc` as
the test statistic.

This approach conflates treatment status and carryover into a
single variable. The `bm:Dbc` coefficient captures both the
...
(including residual exposure),' then `Dbc` is appropriate. If
...
**Approach B: Separate carryover covariate (`tsd`).** A linear
time-since-discontinuation term is added to the model:

`Sx ~ bm + t + Db + bm:Db + tsd`

where `Db` is binary (on/off drug) and `tsd` absorbs the
carryover effect. The interaction `bm:Db` then estimates the
...
**Approach C: Ignore carryover.** The model uses only the binary
drug indicator:
...
### 4.3 The non-standard element

The `Dbc` parameterization (Approach A) is creative but
...
The `Dbc` approach would require explicit justification in a
methods paper, ideally supported by the factorial simulation
comparison described below.

---

## 5. Proposed Simulation Study
...
**DGP factor:** $t_{1/2} \in \{0, 0.25, 0.5, 1.0, 2.0\}$ weeks

**Analysis factor:** Three models:

1. **Ignore:** `Sx ~ bm + t + Db + bm:Db`
   (binary drug indicator, no carryover term)
2. **Separate:** `Sx ~ bm + t + Db + bm:Db + tsd`
   (binary drug indicator + linear carryover covariate)
3. **Continuous:** `Sx ~ bm + t + Dbc + bm:Dbc`
   (exponential-decay drug indicator, current default)

**Fixed parameters:** $N = 70$, $\rho = 0.7$, all four trial
designs.

**Biomarker correlation:** $c_{bm} \in \{0, 0.3, 0.45\}$
...
- **Type I error** (at $c_{bm} = 0$): should be $\leq 0.05$
- **Power** (at $c_{bm} > 0$): higher is better
- **Bias:** $E[\hat{\beta}] - \beta_{true}$
- **RMSE:** $\sqrt{E[(\hat{\beta} - \beta_{true})^2]}$
- **Coverage:** proportion of 95% CIs containing $\beta_{true}$

### 5.3 Expected findings

Based on the literature and the DGP structure:

- **Ignore (model 1)** will have correct Type I error at
...
- **Separate (model 2)** will have correct Type I error across
  all carryover values (the `tsd` term absorbs the carryover
...
- **Continuous (model 3)** will have correct Type I error if the
  `Dbc` decay rate matches the DGP. Power behavior depends on
...
- Model 1: `op$carryover_t1half = 0, op$simplecarryover = FALSE`
- Model 2: `op$carryover_t1half = 0, op$simplecarryover = TRUE`
- Model 3: `op$carryover_t1half = [matching DGP value]`

The simulation grid would need to cross the DGP carryover
parameter with the analysis model choice. This requires a
modification to `generateSimulatedResults()` (or the 2025
equivalent) to accept multiple analysis parameter sets per
DGP configuration, which is already supported via the
`analysisparams` argument.
...
1. **Does the `Dbc` parameterization control Type I error?** If
   it does not, the power estimates reported in the paper are
   unreliable.

2. **Is the power gain from modeling carryover worth the
