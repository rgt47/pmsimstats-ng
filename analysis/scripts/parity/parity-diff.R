...
#
# Usage:
#   Rscript analysis/scripts/parity/parity-diff.R
#   Rscript analysis/scripts/parity/parity-diff.R <baseline.rds> <current.rds>
...
baseline_path <- if (length(args) >= 1) {
  ...
  'analysis/scripts/parity/parity_baseline.rds'
}
current_path <- if (length(args) >= 2) {
  args[2]
} else {
  'analysis/scripts/parity/parity_report.rds'
}
