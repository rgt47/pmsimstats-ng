...
buildSigma<-function(modelparam,respparam,blparam,trialdesign,makePositiveDefinite=TRUE,lambda_cor=NA,verbose=FALSE,dgp_architecture="mvn"){
  ...
  for(cc in cl){
    ...
    if(nP>1){
      for(p in 1:(nP-1)){
        for(p2 in (1+p):nP){
          n1<-paste(trialdesign$timeptnames[p],cc,sep=".")
          n2<-paste(trialdesign$timeptnames[p2],cc,sep=".")
    ...
    for(c2 in setdiff(cl,cc)){
      for(p in 1:nP){
        n1<-paste(trialdesign$timeptnames[p],cc,sep=".")
        n2<-paste(trialdesign$timeptnames[p],c2,sep=".")
      ...
      if(nP>1){
        for(p in 1:(nP-1)){
          for(p2 in (1+p):nP){
            n1<-paste(trialdesign$timeptnames[p],cc,sep=".")
            n2<-paste(trialdesign$timeptnames[p2],c2,sep=".")
            tg<-abs(weeks[p2] - weeks[p])
