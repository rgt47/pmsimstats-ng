...
Architecture B (MVN differential correlation), where the
...
roughly two Monte Carlo standard errors below the logistic and
...
\textbf{The two mechanisms together place the modGompertz
...
framework's modGompertz default is robust by exact construction
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item Sensitivity to modGompertz is small relative to carryover
  specification and DGP-architecture sensitivity.
\item Under a covariance-channel DGP the modGompertz choice is
...
mean-moderation DGP the modGompertz default is the lowest-power
...
using the pmsimstats framework can adopt the modGompertz default
...
\texttt{generateData} (both default to the original behaviour, so
...
\emph{Rendered on 2026-06-17 at 09:31 PDT.}\\
