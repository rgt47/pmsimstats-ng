...
In the published code $\text{Cor}(BM, BR_t)$ is set to the constant
$c_{bm}$ at every timepoint at which the response mean is nonzero,
with no dependence on drug state and no off-drug decay. The
drug-state-dependent, exponentially decaying correlation that defines
covariance moderation in Section 2.2.2 is therefore not a feature of
the published implementation; it was introduced into that repository
in 2024. Architecture B as studied here is accordingly a mechanism
developed from Hendrickson et al.'s stated rationale, that the
biomarker is informative because it shares variance with
drug-responsive physiology, rather than a mechanism recovered from
...
the response mean at off-drug timepoints, and this paper retains that
functional form. The 2024 revision inserts an additional undocumented
...
with the formula \texttt{Sx \textasciitilde{} bm + Db + t + bm*Db +
(1|ptID)}, a random intercept with no residual correlation structure,
in which \texttt{Db} is a binary on-drug indicator. We use
