...
<!--
  ...
  the linear DGP decay form, and drops the M1 (binary on-drug)
  analysis specification, keeping only M2 (exposure-weighted) and
  M3 (lagged-treatment). See report.Rmd / report-slim.Rmd for the
  ...
  4. Render the filtered Tier 1 figures (Architecture B, no linear
     decay, M2/M3 only; writes analysis/figures/02xs-*.pdf):
...
commits to a parametric decay form and half-life (**M2**, the
...
(**M3**, the classical crossover device of Jones and Kenward
...
because M2 and M3 see the *same* trials, they can be compared with a
...
In the crossover design it is much *worse* (0.488 vs 0.830 for M3).
...
here. **Assumed half-life (Fig. \@ref(fig:fig-sens-s2)).** M3 does
...
about 0.83 to 0.77, never below M3. **Autocorrelation
