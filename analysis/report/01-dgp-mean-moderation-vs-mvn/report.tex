...
whether the treatment effect is moderated by the biomarker value.
Simulation studies are the principal tool available for evaluating the
statistical power of proposed trial designs to detect such interactions,
and these simulations necessarily require a data-generating process
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item HTE literature [@rothwell2005; @kent2020path]
...
notably by Rothwell\textsuperscript{\citeproc{ref-rothwell2005}{1}} and
synthesized more recently by Kent et
al.\textsuperscript{\citeproc{ref-kent2020path}{2}}, distinguishes
...
\item Wang and Schork [@wang2019] and Schork [@schork2022] also use
  ...
  et al. [@hendrickson2020], suggesting the MVN parameterization is a
...
Simon\textsuperscript{\citeproc{ref-simon2010}{3}} employs this approach
throughout the simulation frameworks he develops for
biomarker-stratified designs; Freidlin and
Korn\textsuperscript{\citeproc{ref-freidlin2014}{4}} adopt it for
adaptive enrichment trials; Renfro and
Sargent\textsuperscript{\citeproc{ref-renfro2017}{5}} use it throughout
...
biomarker-by-treatment interaction estimation in randomized trials with
prognostic covariates, specifically the study by Haller and
Ulm\textsuperscript{\citeproc{ref-haller2018}{6}}, likewise adopts the
standard regression specification with explicit interaction terms. Among
N-of-1 simulation studies, Zucker et
al.\textsuperscript{\citeproc{ref-zucker1997}{7}}, Araujo et
al.\textsuperscript{\citeproc{ref-araujo2016}{8}}, and Duan et al.
...
Schork\textsuperscript{\citeproc{ref-wang2019}{9}} and
Schork\textsuperscript{\citeproc{ref-schork2022}{10}}, whose
...
Hendrickson et al.\textsuperscript{\citeproc{ref-hendrickson2020}{11}}
...
pmsimstats simulation framework for N-of-1 clinical
trials\textsuperscript{\citeproc{ref-hendrickson2020}{11}}. The original
publication code uses the MVN differential correlation approach, an
unusual choice that, as we shall demonstrate here, has substantive
consequences for power estimation under carryover. A comprehensive audit
of the codebase subsequently revealed that a parallel tidyverse
reimplementation of the same simulation had inadvertently adopted the
standard direct mean moderation approach. Both implementations produced
plausible power estimates, yet they yielded qualitatively different
conclusions about how carryover effects influence statistical power.
Tracing this discrepancy to its root cause revealed the architectural
difference described in this paper, which has not, to the best of our
knowledge, been discussed explicitly in the simulation methodology
literature.
...
Hendrickson\textsuperscript{\citeproc{ref-hendrickson2020}{11}}
...
equation of Kent et al.\textsuperscript{\citeproc{ref-kent2020path}{2}}:
...
al.\textsuperscript{\citeproc{ref-hendrickson2020}{11}} publication code
...
relative-loss range that the broader N-of-1 simulation literature
reports for direct-mean-moderation DGPs, though they reproduce its
qualitative finding, namely a modest erosion that leaves power
essentially intact.
...
The OL+BDC loss is approximately 11 times the matched Architecture-A
OL+BDC loss (2.8\%). It falls below the 40-60\% relative-loss range that
the broader N-of-1 simulation literature reports for the MVN
differential-correlation DGP, but it confirms the qualitative finding,
namely a substantial and strongly design-dependent erosion.
Two-proportion \(z\)-tests on the B-versus-A absolute power-loss gap
give \(z = 7.64\) (\(p < 10^{-13}\)) at OL+BDC and \(z = 3.96\)
(\(p < 10^{-4}\)) at Hybrid; the CO contrast is not statistically
distinguishable from zero (\(z = 0.29\)) because the CO design's
within-subject AR(1) structure dominates the carryover-mediated
correlation decay channel that Architecture B uses to encode the
...
9.3 percentage points (Architecture\textasciitilde B; \(z = 7.4\),
\(p < 10^{-12}\)) and 1.7 percentage points
(Architecture\textasciitilde A; \(z = 2.0\), \(p \approx 0.05\)). The
qualitative ordering matches Section\textasciitilde3.1's \(N = 35\)
result. The absolute magnitudes are compressed because \(N = 70\) at the
prazosin-PTSD calibration places both architectures within 2-5
percentage points of the unit ceiling at \(t_{1/2} = 0\), mechanically
truncating the loss either architecture's curve can absorb. We note that
reading the narrative loss-magnitude claim against the \(N = 70\)
robustness numbers (3\% and 9\% relative loss for
Architecture\textasciitilde B versus 0.4\% and 1.8\% for
Architecture\textasciitilde A) requires acknowledging this saturation
effect; the \(N = 35\) production results in Section\textasciitilde3.1
...
\begin{orig}
We ran the same factorial simulation on the
Architecture\textasciitilde C grid. The design matrix is identical to
Section\textasciitilde3.1: CO, Hybrid, and OL+BDC designs; \(N = 35\);
\(t_{1/2} \in \{0, 0.5, 1.0\}\) weeks; 1000 replicates per cell; A2
analysis specification. The parameter panel is
\(c_{bm,A} \times c_{bm,B} \in
\{0, 0.22, 0.45\}^2\) (nine cells per design--carryover combination).
The boundary cells \((c_{bm,A} = 0.45, c_{bm,B} = 0)\) and
\((c_{bm,A} = 0, c_{bm,B} = 0.45)\) reproduce Sections 3.1's
Architecture\textasciitilde A and Architecture\textasciitilde B results
respectively, providing the internal validation gate.
...
Second, interior cells are uniformly super-additive relative to the
boundary-row and boundary-column averages. At (\(c_{bm,A} =
0.22\), \(c_{bm,B} = 0.22\)) in the Hybrid design, power is 0.567,
whereas the arithmetic mean of the corresponding pure-architecture
values (0.198 for pure A, 0.152 for pure B) is only 0.175. The joint
...
carryover (\(t_{1/2} = 0\)) and one-week (\(t_{1/2} = 1.0\)) panels for
the CO design, increasing \(c_{bm,B}\) from 0 to 0.45 (fixing
\(c_{bm,A} = 0\)) degrades power from 0.591 to 0.287 -- a 51\% relative
loss. The corresponding loss along the \(c_{bm,A}\) axis (fixing
\(c_{bm,B} = 0\)) is from 0.548 to 0.280 -- a 49\% loss, roughly
comparable. However, at the equal-weight interior cell
(\(c_{bm,A} = c_{bm,B} = 0.45\)), the Hybrid design retains 0.991 power
even under one-week carryover, whereas both pure architectures at
\(c_{bm} = 0.45\) individually fall well short of that ceiling
(Architecture\textasciitilde A: 0.714; Architecture\textasciitilde B:
0.527).

Fourth, design choice remains consequential under
Architecture\textasciitilde C. At the full-signal, one-week-carryover
cell, Hybrid (0.991) substantially outperforms OL+BDC (0.960) and
especially CO (0.751). The design ranking established in
...
washout period. That proportionality is precisely what Architecture A
encodes.
...
strategies (Section 6) or by explicit mixture modeling is an open
question.
...
Senn\textsuperscript{\citeproc{ref-senn2016mastering}{12}} has argued
that the apparent `personal element in response to treatment' is often a
variance-component artifact rather than a genuine biomarker-by-treatment
...
al.\textsuperscript{\citeproc{ref-raskind2013}{13}}, whose
...
colleagues\textsuperscript{\citeproc{ref-raskind2013}{13}--\citeproc{ref-raskind2007}{15}}
...
\begin{orig}
The Hendrickson et
al.\textsuperscript{\citeproc{ref-hendrickson2020}{11}} DGP uses
...
\begin{orig}
Architecture C is appropriate when there is independent evidence that
both the pharmacodynamic-scaling pathway and the co-regulatory coupling
pathway are biologically active for the same biomarker-drug pair. Such
evidence might include mechanistic assay data (e.g., the drug enhances
biomarker-outcome correlation in vitro, consistent with the B channel,
while receptor occupancy scales with biomarker level, consistent with
the A channel) or replication across independent trials that are
...
parameters where either Architecture A or Architecture B requires only
one, and calibrating both parameters independently demands more prior
information than is typically available in the trial-design phase. The
appropriate use cases are: (a) formal sensitivity analyses that ask how
power estimates change across the \((c_{bm,A}, c_{bm,B})\) grid when the
channel structure is genuinely uncertain; and (b) evaluation studies
designed to determine which channel is dominant for a given
biomarker-drug pair, using the simulation as a diagnostic instrument
rather than a planning tool.

In the prazosin-PTSD context, Architecture C captures the possibility
that SBP simultaneously governs the expected prazosin effect at the mean
level (Architecture A, consistent with the pharmacological argument that
noradrenergic tone determines the therapeutic window) and alters the
degree of coupling between SBP and CAPS change during active drug
exposure (Architecture B, consistent with a signaling-pathway
interpretation). We regard Architecture C as the appropriate framework
for evaluating the prazosin-SBP interaction if and when mechanistic data
become available to calibrate both channels separately.
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parskip}{0pt}
\item ADEMP [@morris2019simulation] prescribes five elements:
...
Crowther\textsuperscript{\citeproc{ref-morris2019simulation}{17}}, which
...
extensive. Rothwell\textsuperscript{\citeproc{ref-rothwell2005}{1}} sets
...
al.\textsuperscript{\citeproc{ref-kent2020path}{2}}, which distinguishes
...
\item N-of-1 simulation studies [@zucker1997; @araujo2016;
  @duan2013] use hierarchical random-effects
...
Simon\textsuperscript{\citeproc{ref-simon2010}{3}} employs it throughout
the simulation frameworks he develops for biomarker-stratified designs;
Freidlin and Korn\textsuperscript{\citeproc{ref-freidlin2014}{4}} adopt
it for adaptive enrichment trials; Renfro and
Sargent\textsuperscript{\citeproc{ref-renfro2017}{5}} use it for basket
and umbrella trials; and it underlies the platform trial literature
throughout. Haller and
Ulm\textsuperscript{\citeproc{ref-haller2018}{6}}, in simulation work on
biomarker-by-treatment interaction estimation in randomized trials with
...
al.\textsuperscript{\citeproc{ref-araujo2016}{8}}, and Duan et
al.\textsuperscript{\citeproc{ref-duan2013}{18}}, use hierarchical
...
Schork\textsuperscript{\citeproc{ref-wang2019}{9}} and
Schork\textsuperscript{\citeproc{ref-schork2022}{10}}, adopts the same
Architecture A specification, even though this work shares senior
authorship with Hendrickson et
al.\textsuperscript{\citeproc{ref-hendrickson2020}{11}}. It appears,
...
Rizopoulos\textsuperscript{\citeproc{ref-rizopoulos2012}{19}}, Bayesian
...
Senn\textsuperscript{\citeproc{ref-senn2002}{20}} and Jones and
Kenward\textsuperscript{\citeproc{ref-jones2014crossover}{21}},
...
Lumley\textsuperscript{\citeproc{ref-sturdevant2021carryover}{22}} on
...
without engaging with how carryover interacts with
biomarker-by-treatment effect signals. The standard recommendation,
namely to always include carryover in the analysis model or to use
adequate washout, applies to both architectures but has different power
implications under each. The finding that Architecture B is more
sensitive to carryover suggests that crossover designs with short
washout periods may be less suitable for detecting correlation-based
biomarker interactions than previously recognized.
...
Simon\textsuperscript{\citeproc{ref-simon2010}{3}} and Freidlin and
Korn\textsuperscript{\citeproc{ref-freidlin2014}{4}}, use Architecture A
...
washout phase erodes the BM-BR correlation, so the biomarker-treatment
...
Sargent\textsuperscript{\citeproc{ref-renfro2017}{5}}, introduce an
...
\item This recommendation parallels the advice in Section 5.3
  and is consistent with the PATH Statement [@kent2020path].
...
Statement\textsuperscript{\citeproc{ref-kent2020path}{2}}, which itself
...
personalized studies. \emph{Harvard Data Science Review}. Published
online 2022.}
...
\emph{Rendered on 2026-06-13 at 17:08 PDT.}\\
\emph{Source:
