...
\item Grid: $c_{bm,A} \times c_{bm,B} \in \{0, 0.22, 0.45\}^2$;
  boundary rows/columns reproduce the Section 3.1 baselines.
...
\begin{orig}
We ran the same factorial simulation on the Architecture~C grid.
The design matrix is identical to Section~3.1: CO, Hybrid, and
OL+BDC designs; $N = 35$; $t_{1/2} \in \{0, 0.5, 1.0\}$ weeks;
1000 replicates per cell; A2 analysis specification. The
parameter panel is $c_{bm,A} \times c_{bm,B} \in
\{0, 0.22, 0.45\}^2$ (nine cells per design--carryover
combination). The boundary cells $(c_{bm,A} = 0.45, c_{bm,B} = 0)$
and $(c_{bm,A} = 0, c_{bm,B} = 0.45)$ reproduce Sections 3.1's
Architecture~A and Architecture~B results respectively, providing
the internal validation gate.
\end{orig}
...
channel). The boundary column $c_{bm,B} = 0$ reproduces
pure-Architecture A results; the boundary row $c_{bm,A} = 0$
reproduces pure-Architecture B results.}
...
0.00 & 0.040 & 0.085 & 0.287 \\
0.22 & 0.087 & 0.277 & 0.540 \\
0.45 & 0.280 & 0.523 & 0.751 \\
\addlinespace
\multicolumn{4}{l}{\textit{Hybrid}} \\
0.00 & 0.025 & 0.152 & 0.527 \\
0.22 & 0.198 & 0.567 & 0.901 \\
0.45 & 0.714 & 0.929 & 0.991 \\
\addlinespace
\multicolumn{4}{l}{\textit{OL+BDC}} \\
0.00 & 0.028 & 0.087 & 0.309 \\
0.22 & 0.174 & 0.414 & 0.719 \\
0.45 & 0.584 & 0.835 & 0.960 \\
\bottomrule
\end{tabular}
\end{table}
...
Hybrid design, power is 0.567. The additive baseline on the
probability scale -- the power expected if the two channels
contributed independently -- is $p(0.22,0) + p(0,0.22) - p(0,0)
= 0.198 + 0.152 - 0.025 = 0.325$. The observed value of 0.567
exceeds this benchmark by 0.242 percentage points, indicating
genuine super-additivity: the joint signal compounds rather than
aggregates.

Third, the covariance channel ($c_{bm,B}$) governs carryover
sensitivity more than the mean-shift channel. Comparing the no-
carryover ($t_{1/2} = 0$) and one-week ($t_{1/2} = 1.0$) panels
for the CO design, increasing $c_{bm,B}$ from 0 to 0.45 (fixing
$c_{bm,A} = 0$) degrades power from 0.591 to 0.287 -- a 51\%
relative loss. The corresponding loss along the $c_{bm,A}$ axis
(fixing $c_{bm,B} = 0$) is from 0.548 to 0.280 -- a 49\% loss,
roughly comparable. However, at the equal-weight interior cell
($c_{bm,A} = c_{bm,B} = 0.45$), the Hybrid design retains 0.991
power even under one-week carryover, whereas both pure
architectures at $c_{bm} = 0.45$ individually fall well short of
that ceiling (Architecture~A: 0.714; Architecture~B: 0.527).

Fourth, design choice remains consequential under Architecture~C.
At the full-signal, one-week-carryover cell, Hybrid (0.991)
substantially outperforms OL+BDC (0.960) and especially CO
(0.751). The design ranking established in Section~3.2 is
