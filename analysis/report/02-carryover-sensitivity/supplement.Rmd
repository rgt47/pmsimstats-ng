...
2%). Panel C (`G1`-`G3` only, decay-shape axis) needs no new
...
```{r fig-hendrickson-c-archA, echo = FALSE, fig.cap = "Architecture A counterpart to Figure 3: decay-shape axis, power by trial design (columns), $N$ (row blocks), and analysis specification (rows within block), across the four DGP decay-shape levels (exponential, Weibull $k \\in \\{0.7, 1.0, 1.5\\}$) at $c_{bm} = 0.45$, $t_{1/2} = 1.0$ weeks, Architecture A ($n_{sim} = 500$).", out.width = "95%"}
