...
<!--
  ...
  This is a narrative rewrite of report-extra-slim.Rmd: the same
  scope (Architecture B, exponential and Weibull decay, M2 versus
  M3), the same data, the same tables and figures, but with the
  paired bullets/rgt scaffolding removed and the surviving prose
  rewritten as continuous narrative. It reuses the filtered figures
  and cell-level summaries produced for report-extra-slim.Rmd
  ...
  4. Filtered Tier 1 figures (Architecture B, no linear decay, M2/M3
     only; writes analysis/figures/02xs-*.pdf), shared with
     report-extra-slim.Rmd:
  ...
  5. Filtered Tier 2 sensitivity figures (writes
     analysis/figures/02xs-sens-S1..S5.pdf), shared with
     report-extra-slim.Rmd:
  ...
  5c. Filtered S6 type-I figure (writes
      analysis/figures/02xs-type1-cr2.pdf), shared with
      report-extra-slim.Rmd:
  ...
  6. Knit this Rmd (this step; ~1 min; no simulation code runs):
       Rscript -e "rmarkdown::render('analysis/report/02-carryover-sensitivity/report-extra-slim-rgt.Rmd')"
...
knitr::opts_chunk$set(
  ...
  warning = FALSE,
  message = FALSE,
  fig.path = "../../figures/02xsrgt-cache-",
  cache.path = "_cache_xsrgt/"
)
library(tibble)
library(dplyr)
library(knitr)
...
**Results.** M2 attains the highest power only in the two designs
...
design M2 is markedly inferior (0.488 vs 0.830 for M3, which tracks
the binary indicator closely). Where M2 leads, the lead appears
...
Architecture B we recommend the exposure-weighted predictor M2,
reported with a cluster-robust standard error, for the Hybrid and
OL+BDC designs; the lagged-treatment specification M3 is preferable,
...
binary on-drug specification (M1) or Architecture A (mean
moderation); the full-scope evaluation is in the companion report.Rmd
...
a parametric decay form and half-life, which we call M2 and which is
...
functional form, which we call M3, following the classical
...
model, and to determine which of the two candidate strategies, M2 or
M3, performs better across the grid of data-generating decay forms
...
`implementations/tidyverse/R/functions.R`. It corresponds to
...
Under M2, $X = \text{Dbc}_{it}$, a continuous exposure-decayed
...
identical regardless of $\hat{t}_{1/2}$. That is, M2 is in effect a
...
Under M3, $X = D_{it}$ is augmented with a lagged-treatment indicator
...
[-@jones2014crossover]. One structural feature of M3 bears directly
...
actually tested under M3 remains $\text{bm:}D_{it}$, the same
coefficient tested under the binary on-drug specification (which we
call M1 and do not evaluate further in this manuscript). M3 is best
understood, then, as M1 with a carryover nuisance control added,
rather than as a fundamentally different model of the interaction.
Because it carries no half-life parameter, M3 is invariant to the
analyst's assumed half-life by construction, in contrast to M2. This
...
Architecture B, the two non-linear decay forms, and the M2 and M3
...
M2 minus M3 contrast is estimated with reduced Monte Carlo variance
...
(`analysis/data/02-sensitivity-summary.rds` and
...
figures used here are produced by `03-render-figures-extra-slim.R`,
...
Architecture B, the exponential and Weibull decay forms, and the M2
and M3 specifications. The Monte Carlo standard error on power does
...
columns: these are computed against M2's calibrated target,
$\theta_{\text{true}} = -c_{bm}\sigma_{BR} = -3.6$, while M3
estimates a different coefficient, $\text{bm:}D_{it}$. M3's apparent
...
completeness but are not directly comparable between M2 and M3.
...
```{r fig-power-by-spec, echo = FALSE, fig.cap = "Power versus carryover half-life under the exponential DGP, Architecture B, stratified by analysis specification. M2 retains a modest advantage over M3 in the Hybrid and OL+BDC designs at $t_{1/2} = 1.0$ weeks, but is dominated by M3 under the crossover (CO) design.", out.width = "95%"}
...
the half-life increases. M2 retains a modest but consistent
...
where M2 is in fact dominated, a point we return to in Section 4.
...
M2's decaying predictor was designed to recover, so M2 has little
...
the M2 column is the matched reference, with power
$0.860 \pm 0.016$. The same row's M3 column gives the power penalty
of the lagged specification when the data-generating decay is
exponential, while the other rows of the M2 column give the cost of
...
process we examined ($k = 0.7$), M2's advantage over M3 narrows but
...
specifications, it does not disturb the M2-versus-M3 comparison, and
...
lab <- c('Reference, $N=70$', 'Small $N=35$',
         ...
         '30\\% MCAR dropout')
## The pre-computed s6$gaps object is the M2-M1 gap inherited from
## the full-scope (report.Rmd) diagnostic run; this manuscript's
## comparison is M2 vs M3, so the M2-M3 gap is computed directly
...
tab <- s6$summary |>
  ...
  transmute(
    ...
    `M2$-$M3 model`   = sprintf('%+.3f', gap_mod_a3),
    `M2$-$M3 CR2`     = sprintf('%+.3f', gap_cr2_a3),
    `CR2 df`          = sprintf('%.0f', cr2_df))
knitr::kable(tab, booktabs = TRUE, escape = FALSE,
  caption = 'Cluster-robust recalibration of M2 (Power columns) and
...
process. Table \@ref(tab:tab-s6) reports the M2 minus M3 gap, which
...
the M2-versus-M1 contrast reported in the full-scope grid. That gap
...
to $+0.036$, though M2 still leads); and at 30% MCAR dropout it
collapses from a small M2 lead under the model-based test
...
M2's advantage is driven by information carried in the off-drug
...
should not be extended to the crossover design, where M2 does not
...
$\rho = 0.5$, M2 power is $0.798 \pm 0.018$ against M3's
$0.738 \pm 0.020$; at $\rho = 0.9$, M2 reaches $0.956 \pm 0.009$
against M3's $0.910 \pm 0.013$ (point estimate plus or minus MCSE,
$n_{sim} = 500$). The M2 advantage over M3, roughly 0.06 in absolute
...
```{r fig-sens-s2, echo = FALSE, fig.cap = "Power of M2 and M3 as the analyst's assumed half-life departs from the true DGP half-life. M3 is invariant to the assumed half-life by construction; M2's power degrades with mis-specification.", out.width = "80%"}
knitr::include_graphics("../../figures/02xs-sens-S2.pdf")
```

M2 appears fairly robust to a mismatch between the analyst's assumed
...
between 0.008 and 0.019. M3, which does not consume the half-life
...
erode M2's advantage over M3 at either true half-life. It would
appear, then, that M2 remains a defensible choice even when the true
...
increases. At the MCAR baseline ($N = 70$, no dropout), M2 gives
$0.866 \pm 0.015$ and M3 gives $0.764 \pm 0.019$; at an MCAR dropout
rate of 0.30, these collapse to $0.148 \pm 0.016$ and
$0.124 \pm 0.015$ respectively. M2's advantage narrows steadily as
dropout worsens: the absolute M2 minus M3 gap runs about 0.10 at
...
M2's advantage originating in the exposure-weighted contrast
contributed by off-drug observations; when those observations are
preferentially lost, M2's edge contracts toward M3's. At low to
moderate dropout, that is, at a rate of 0.20 or below, the M2
...
$0.028 \pm 0.007$ for M2 and $0.030 \pm 0.008$ for M3; power then
rises through the mid-range and saturates once $c_{bm} \geq 0.45$,
reaching $0.958 \pm 0.009$ for M2 and $0.916 \pm 0.012$ for M3 at
$c_{bm} = 0.6$. The M2 advantage is largest at intermediate effect
...
converging. Because M2 and M3 are fit to the same trials, we can
...
the sample sizes and half-lives examined. Paired McNemar tests on M2
versus M3 find M2 strictly ahead only at the highest-leverage cell
...
M2-versus-M3 gap at the intermediate cells with statistical clarity
...
The principal finding is that M2's advantage over M3 is real, but
conditional. On the one hand, at its best cell (Hybrid design,
$N = 70$, $c_{bm} = 0.45$, $t_{1/2} = 1.0$), M2 exceeds M3 by about
...
design M2 performs considerably worse (0.488 versus 0.830 for M3).
...
signal that M2's decaying predictor was designed to recover, so M2
...
recommendation that M2 is to be preferred therefore holds only in
the two non-crossover designs studied here, and should not be
extended to a crossover trial.

## What the robustness checks add

Three checks probe whether M2's advantage, where it exists, is
...
true form is Weibull, M2 still does not fall below M3, within the
range of decay shapes we examined. On the assumed half-life (Figure
\@ref(fig:fig-sens-s2)): M3 does not use a half-life and is
therefore unaffected, while M2 does, yet mis-specifying the
half-life by a factor of two or four moves M2's power only from
about 0.83 down to 0.77, never below M3. On autocorrelation (Figure
\@ref(fig:fig-sens-s1)): stronger serial correlation raises both
specifications by roughly the same amount, so M2's lead of five to
six points holds regardless of correlation strength. Taken together,
these checks suggest that where M2's advantage exists, it is not a
...
between 0.12 and 0.25 and become essentially indistinguishable. M2
...
choice between M2 and M3 ever could.
...
M2-versus-M3 contrast, in the interest of a self-contained
evaluation of manageable length. The full-scope companion
(report.Rmd and report-slim.Rmd)
shows that the M2 advantage documented here does not carry over to
Architecture A, where the biomarker moderates the mean response
directly and M2 is marginally dominated in every design we examined;
nor is the binary on-drug specification M1 dominated by M2 outside
...
comparison against M1, should consult that manuscript rather than
...
lead one to expect that a shape-free lagged term such as M3 should
outperform a parametric predictor such as M2 once the decay shape is
mis-specified. Our S2 result does not support that expectation over
the range we examined, because M2's penalty for an incorrect
half-life appears too small for M3 to overtake it.
...
1. **M2 helps, but conditionally.** The exposure-weighted predictor
   achieves the highest power, about 10 points over M3, only in the
   ...
   those designs. Under the crossover design, M2 confers no
   advantage, and M3 is at least as good. Where M2 is used, we
   recommend reporting it with a CR2 cluster-robust standard error
   (Block S6).

2. **An approximate half-life is adequate, where M2 is used.**
   Mis-specifying the half-life by a factor of two to four moves
   M2's power only modestly, so a rough pharmacokinetic estimate is
...
4. **The interaction test is somewhat conservative, and the
   ...
   of power. The M2-versus-M3 gap is held or widened under CR2 at
...
5. **This is a narrowed slice of a larger study.** Architecture A,
   the binary specification M1, and linear decay all fall outside
...
`analysis/data/02-grid-summary.rds` and
`02-sensitivity-summary.rds`; the filtered figures used here are
produced by `03-render-figures-extra-slim.R`,
`04-sensitivity-figures-extra-slim.R`, and
`07-type1-cr2-figure-extra-slim.R` under
`analysis/scripts/carryover-sensitivity/`. No simulation code runs
